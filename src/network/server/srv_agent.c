...
void srv_shrink_agent_pool(agent_pool_t *agent_pool)
{
    ...
    if (agent_pool->shrink_hit_count > (long)AGENT_SHRINK_THRESHOLD(g_srv_inst->reactor_pool.agents_shrink_threshold)) {
        GS_LOG_DEBUG_INF("[agent_pool] begin with shrink extend agents ...");
        shrink_pool_core(agent_pool);
        GS_LOG_DEBUG_INF("[agent_pool] end of shrink extend agents.");
    }
}
...
static void close_extend_agent(agent_pool_t *agent_pool)
{
    ...
    GS_LOG_RUN_INF("[agent] close extend agents' thread, extended slot count: %u", slot_used_id);
...
static void free_extend_agent(agent_pool_t *agent_pool)
{
    ...
    agent_t *slot_agents = NULL;
    uint32 slot_used_id = CM_ALIGN_CEIL(agent_pool->extended_count, AGENT_EXTEND_STEP);
    GS_LOG_RUN_INF("[agent] free extend agents, extended slot count: %u", slot_used_id);
...
static void srv_shutdown_agent_pool(agent_pool_t *agent_pool)
{
    ...
    GS_LOG_RUN_INF("[agent] all agents' thread have been closed");
...
{
    GS_LOG_RUN_INF("[agent] begin to destroy agent pool");
...
static status_t allocate_slot(agent_pool_t *agent_pool)
{
    ...
    GS_LOG_DEBUG_INF("[agent] allocate extend slots count: %u", slot_count);
...
static status_t extend_agent_pool(agent_pool_t *agent_pool)
{
    ...
    GS_LOG_DEBUG_INF("[agent] extend agents, expansion_count: %u, slot_id: %u", expansion_count, slot_id);
...
void srv_process_free_session(session_t *session)
{
    ...
    GS_LOG_DEBUG_INF("[agent] free session %u successfully.", session->id);
    return;
}
...
static status_t srv_process_single_session(session_t *session)
{
    ...
    GS_RETURN_IFERR(cs_wait(session->pipe, CS_WAIT_FOR_READ, (int32)GS_POLL_WAIT, &ready));
    if (!ready) {
        return GS_SUCCESS;
    }
    GS_LOG_RUN_INF("begin srv_process_command");
...
void srv_detach_agent(session_t *session)
{
    ...
    if (agent == NULL) {
        GS_LOG_DEBUG_WAR("[agent] srv detach agent is NULL, sessid:%u", session->id);
        return;
    }
    agent_pool_t *agent_pool = &agent->reactor->agent_pool;
    ...
    GS_LOG_DEBUG_INF("[agent] detach session %u from agent %lu success, idle agent num %u.",
...
static void srv_detach_agent_and_set_oneshot(session_t *session, agent_t *agent)
{
    ...
    srv_detach_agent(session);
    CM_MFENCE;
    if (reactor_set_oneshot(session) != GS_SUCCESS) {
        GS_LOG_RUN_ERR("[agent] set oneshot flag of socket failed, session %u, reactor %lu",
...
static void srv_try_process_multi_sessions(agent_t *agent)
{
    ...
    for (;;) {
        ...
        if (cm_event_timedwait(&agent->event, CM_SLEEP_50_FIXED) == GS_SUCCESS) {
            break;
        }
    ...
    GS_LOG_DEBUG_INF("[agent] begin to process socket event session %u.", session->id);

    while (!agent->thread.closed) {
        ret = srv_process_single_session(session);
        if (ret != GS_SUCCESS) {
            GS_LOG_DEBUG_ERR("[agent] srv_process_single_session fail, sessionid:%u", session->id);
        }
        if (!session->is_reg) {
            return;
...
static void srv_agent_entry(thread_t *thread)
{
    ...
    cs_init_packet(&agent->recv_pack, 0);
    cs_init_packet(&agent->send_pack, 0);

    cm_set_thread_name("dcc_agent");
    GS_LOG_RUN_INF("[agent] agent thread started, tid:%lu, close:%u", thread->id, thread->closed);
    while (!thread->closed) {
        srv_try_process_multi_sessions(agent);
    }
    GS_LOG_RUN_INF("[agent] agent thread closed, tid:%lu, close:%u", thread->id, thread->closed);
...
{
    return cm_create_thread(entry, SIZE_M(8), agent, &agent->thread);
}

static status_t srv_create_agent(agent_t *agent)
{
    if (cm_event_init(&agent->event) != GS_SUCCESS) {
        GS_THROW_ERROR(ERR_CREATE_EVENT, cm_get_os_error());
        return GS_ERROR;
    }
    if (srv_start_agent(agent, srv_agent_entry) != GS_SUCCESS) {
        GS_LOG_RUN_ERR("[agent] create agent thread failed");
        srv_free_agent_res(agent);
        return GS_ERROR;
    }
    GS_LOG_DEBUG_INF("[agent] create agent(%lu) succeed", agent->thread.id);
    return GS_SUCCESS;
}
...
static status_t srv_try_create_agent(agent_pool_t *agent_pool, agent_t **agent)
{
    ...
    if (agent_pool->curr_count == agent_pool->optimized_count + agent_pool->extended_count) {
        cm_spin_lock(&agent_pool->lock_new, NULL);
        if (extend_agent_pool(agent_pool) != GS_SUCCESS) {
            GS_LOG_DEBUG_ERR("[agent] try to expand agent pool failed, current expanded count: %u.",
    ...
    *agent = OBJECT_OF(agent_t, node);
    if (srv_create_agent(*agent) != GS_SUCCESS) {
        srv_return_agent2blankqueue(*agent);
        *agent = NULL;
        return GS_ERROR;
    }
...
inline void srv_bind_sess_agent(session_t *session, agent_t *agent)
{
    ...
    GS_LOG_DEBUG_INF("[agent] srv bind sessid(%u) with agent(%lu)", session->id, agent->thread.id);
    ...
    *agent = NULL;
    for (;;) {
        ...
        if (*agent != NULL) {
            ...
            if (is_log == GS_TRUE) {
                GS_LOG_DEBUG_INF("[agent] srv attach agent recovery: session-id: %u", session->id);
            }
            return GS_SUCCESS;
        }
        ...
        if ((++count % SRV_TRY_ATTACH_AGENT_FAIL_LOG_THRESHOLD) == 0 && !is_log) {
            GS_LOG_DEBUG_WAR("[agent] system busy, wait for idle agent, session id %u active agent count %u, "
