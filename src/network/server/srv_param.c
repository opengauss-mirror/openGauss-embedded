...
static unit_t g_units[] = {
    ...
    [UNIT_TB]={{"TB", 2}, UNIT_T_MEM, SIZE_TB(1)},
    [UNIT_T]={{"T", 1}, UNIT_T_MEM, SIZE_TB(1)},
...
status_t get_param_name_by_id(uint32 param_id, char *param_name)
{
    ...
    GS_RETURN_IFERR(strcpy_s(param_name, MAX_PARAM_NAME_LEN + 1, g_parameters[param_id].name));
    return GS_SUCCESS;
}
...
status_t get_param_id_by_name(const char *param_name, uint32 *param_id)
{
    ...
    GS_THROW_ERROR(ERR_INVALID_PARAMETER_NAME, param_name);
    return GS_ERROR;
}
...
bool32 is_param_can_reloaded(uint32 param_id)
{
    ...
    return (g_parameters[param_id].notify != NULL
        && g_parameters[param_id].effect == EFFECT_IMMEDIATELY) ? GS_TRUE : GS_FALSE;
}
    ...
    const char *param_name, const char *param_value, dcc_param_t *param_id, param_value_t *out_value)
{
    ...
    if (ret == GS_ERROR || g_parameters[param_name_id].verify == NULL) {
        GS_THROW_ERROR(ERR_INVALID_PARAMETER_NAME, param_name);
        return GS_ERROR;
    }
    ...
    *param_id = (dcc_param_t) param_name_id;
    ...
    if (ret != GS_SUCCESS) {
        GS_THROW_ERROR(ERR_INVALID_PARAMETER_VALUE, param_name, param_value);
    }
    return ret;
}
...
{
    cm_latch_s(&latch, 0, GS_FALSE, NULL);
...
status_t srv_set_param(const char* param_name, const char* param_value)
{
    ...
    if (ret == GS_SUCCESS && g_parameters[param_id].notify != NULL) {
...
{
    GS_RETURN_IFERR(verify_param_string(param_id, param_value, out_value));
    GS_RETURN_IF_FALSE(cm_check_ip_valid(out_value->str_val));
    return GS_SUCCESS;
}
...
static status_t get_num_and_unit(char *param_value, char *num_part, uint32 num_len, text_t *unit_part)
{
    ...
    text_param.len = i;
    GS_RETURN_IFERR(cm_text2str(&text_param, num_part, num_len + 1));
    return GS_SUCCESS;
}
...
static status_t parse_param_with_units(const char *param_value, param_unit_t default_unit, uint64 *out_value)
{
    ...
    char num_part[GS_MAX_NUMBER_LEN + 1] = {0};
    MEMS_RETURN_IFERR(strcpy_s(param_val, MAX_PARAM_VALUE_LEN, param_value));
    GS_RETURN_IFERR(get_num_and_unit(param_val, num_part, GS_MAX_NUMBER_LEN, &text_unit));
    cm_trim_text(&text_unit);
    GS_RETURN_IFERR(cm_str2uint64(num_part, &size));
...
{
    CS_CHECK_NULL_PTR(min_val);
    CS_CHECK_NULL_PTR(max_val);
    char *token = NULL;
    char *next_token = NULL;
    if (range == NULL) {
        *min_val = 0;
        *max_val = GS_MAX_UINT64;
        return GS_SUCCESS;
    }
    ...
    GS_RETURN_IFERR(parse_param_with_units(token, default_unit, min_val));
    if (!left_close_itl && *min_val != GS_MAX_UINT64) {
        (*min_val)++;
    }
    text.str = next_token;
    ...
    GS_RETURN_IFERR(parse_param_with_units(text.str, default_unit, max_val));
...
status_t verify_param_uint64(dcc_param_t param_id, const char *param_value, param_value_t *out_value)
{
    ...
    if (g_parameters[param_id].unit != UNIT_NONE) {
        GS_RETURN_IFERR(parse_param_with_units(param_value, g_parameters[param_id].unit, &value));
    } else {
        GS_RETURN_IFERR(cm_str2uint64(param_value, &value));
    }
    GS_RETURN_IFERR(parse_integer_range(g_parameters[param_id].range, g_parameters[param_id].unit, &min_val, &max_val));
    if (value >= min_val && value <= max_val) {
        out_value->uint64_val = value;
        return GS_SUCCESS;
    }
    GS_THROW_ERROR(ERR_INVALID_PARAMETER_VALUE, g_parameters[param_id].name, param_value);
    return GS_ERROR;
}
...
status_t verify_param_log_level(dcc_param_t param_id, const char *param_value, param_value_t *out_value)
{
    ...
    if (ret != GS_SUCCESS || log_level > MAX_LOG_LEVEL) {
        return GS_ERROR;
    }
...
{
    uint32 enum_value;
    GS_RETURN_IFERR(valid_param_enum(g_parameters[param_id].range, param_value, &enum_value));
...
/*static inline status_t dcf_set_param_wrap(const char *param, const char *value)
{
    ...
    if (ret != GS_SUCCESS) {
        GS_LOG_RUN_ERR("[PARAM] set dcf param %s to value %s failed", param, value);
    }
    if (cm_str_equal(param, "SSL_PWD_PLAINTEXT")) {
        GS_LOG_RUN_INF("[PARAM] set dcf param %s value %s success", param, "***");
    } else {
        GS_LOG_RUN_INF("[PARAM] set dcf param %s value %s success", param, value);
    }
    return (ret == GS_SUCCESS) ? GS_SUCCESS : GS_ERROR;
}
