...
static status_t reactor_add_epoll_session(session_t *session)
{
    ...
    if (epoll_ctl(reactor->epollfd, EPOLL_CTL_ADD, fd, &ev) != 0) {
        GS_LOG_RUN_ERR("[reactor] register session to reactor failed, session %u, reactor %lu, active agent num %u",
    ...
    GS_LOG_DEBUG_INF("[reactor] register session %u to reactor %lu sucessfully, current session count %ld",
...
status_t reactor_register_session(session_t *session)
{
    ...
    status_t res = reactor_add_epoll_session(session);
    if(res == GS_SUCCESS) {
...
void reactor_unregister_session(session_t *session)
{
    ...
    if (epoll_ctl(reactor->epollfd, EPOLL_CTL_DEL, fd, NULL) != 0) {
        GS_LOG_RUN_ERR("[reactor] unregister session from reactor failed, session %u, reactor %lu",
    ...
    GS_LOG_DEBUG_INF("[reactor] unregister session %u from reactor %lu, current session count %d",
...
{
    if (cm_create_thread(reactor_entry, 0, reactor, &reactor->thread) != GS_SUCCESS) {
        GS_LOG_RUN_ERR("[reactor] failed to create reactor thread");
        return GS_ERROR;
    }
    return GS_SUCCESS;
}
...
static inline void reactor_init_kill_events(reactor_t *reactor)
{
    ...
    queue->w_pos = 0;
    queue->w_lock = 0;
    queue->count = avg_round_ceil(GS_MAX_SESSIONS, pool->reactor_count);
...
static inline status_t reactor_start(reactor_t *reactor, uint32 optimized_count, uint32 max_count)
{
    ...
    GS_RETURN_IFERR(srv_create_agent_pool(&reactor->agent_pool));
...
status_t reactor_create_pool(void)
{
    ...
    pool->roudroubin = 0;
    pool->roudroubin2 = 0;
    uint32 count = avg_round_ceil(GS_MAX_SESSIONS, pool->reactor_count);
...
static status_t reactor_handle_event(const reactor_t *reactor, session_t *sess)
{
    ...
    status_t status = srv_attach_agent(sess, &agent, GS_FALSE);
    if (status != GS_SUCCESS) {
        cm_get_error(&err_code, &message, NULL);
        GS_LOG_RUN_ERR("[reactor] attach agent failed, sid [%u], reactor %lu err_err_code %d",
    ...
    if (agent != NULL) {
        GS_LOG_DEBUG_INF("[reactor] receive message from session %u, attached agent %lu", sess->id, agent->thread.id);
...
void reactor_add_kill_event(session_t *sess)
{
    ...
    cm_spin_unlock(&kill_events->w_lock);
    GS_LOG_DEBUG_INF("[reactor] add session %u to kill event queue [w_pos %u, r_pos %u] success.",
...
static status_t reactor_deal_kill_events(reactor_t *reactor)
{
    ...
    while (r_pos != last_w_pos) {
        ...
        if (sess->agent != NULL) {
            if (sess->is_used != GS_TRUE) {
                srv_detach_agent(sess);
            }

            r_pos = (r_pos + 1) % kill_events->count;
            GS_LOG_DEBUG_INF("[reactor] deal kill events session[%u] agent is not null, "
        ...
        status = srv_attach_agent(sess, &agent, GS_FALSE);
        if (status != GS_SUCCESS) {
            GS_LOG_RUN_ERR("[reactor] deal kill events attach agent failed, "
        ...
        if (agent != NULL) {
            ...
            GS_LOG_DEBUG_INF("[reactor] attached agent to release session, "
...
static void reactor_wait4events(reactor_t *reactor)
{
    ...
    if (status != GS_SUCCESS) {
        GS_LOG_DEBUG_ERR("[reactor] reactor deal kill events failed");
        return;
    }
    ...
    if (nfds == -1) {
        if (errno != EINTR) {
            GS_LOG_RUN_ERR("[reactor] failed to wait for connection request");
        }
        return;
    }
    if (nfds == 0) {
    ...
    for (loop = 0; loop < nfds; ++loop) {
        ...
        if (reactor->status != REACTOR_STATUS_RUNNING) {
            if (reactor_set_oneshot(sess) != GS_SUCCESS) {
                GS_LOG_RUN_ERR("[reactor] set oneshot flag of socket failed, session %u, reactor %lu",
        ...
        if (ev->events & EPOLLRDHUP) {
            GS_LOG_RUN_ERR("[reactor] epoll wait event as EPOLLRDHUP, session id %u, reactor %lu",
                sess->id, reactor->thread.id);
...
static void reactor_entry(thread_t *thread)
{
    ...
    GS_LOG_RUN_INF("[reactor] reactor thread started");
    while (!thread->closed) {
    ...
    GS_LOG_RUN_INF("[reactor] reactor thread closed");
