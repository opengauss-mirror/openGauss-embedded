...
status_t cs_read_bytes_timeout(cs_pipe_t *pipe, char *buf, uint32 max_size, int32 *size, int timeout)
{
    int new_timeout = timeout > 0 ? timeout : GS_NETWORK_IO_TIMEOUT;
    bool32 ready = GS_FALSE;
    CM_POINTER(pipe);
    uint32 wait_event;
    if (VIO_WAIT(pipe, CS_WAIT_FOR_READ, new_timeout, &ready) != GS_SUCCESS || !ready) {
        return GS_ERROR;
    }
