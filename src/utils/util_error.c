...
const char *g_dcc_error_desc[DCC_ERROR_COUNT] = {
    ...
    [ERR_DCC_INVALID_PARAMETER_NAME]  = "Server parameter name %s was invalid",
    ...
    [ERR_DCC_LINE_SIZE_TOO_LONG]      = "The length of row %d in dcc_server.ini is too long",
    [ERR_DCC_FILE_NOT_EXIST]          = "config %s file %s does not exist",
    [ERR_DCC_FILE_SIZE_TOO_LARGE]     = "The size of config file %s is too large",
    [ERR_DCC_DUPLICATE_PARAMETER]     = "The parameter name in config file %s is duplicated",
...
void util_convert_exc_errno(void)
{
    ...
    if (err_code >= (int32)DCC_ERRNO_EXECUTOR_BEGIN && err_code < (int32)DCC_ERRNO_EXECUTOR_END) {
        GS_THROW_ERROR(ERR_DCF_INTERNAL, "");
    } else {
        GS_THROW_ERROR(ERR_STORAGE_INTERNAL, "");
    }
}
