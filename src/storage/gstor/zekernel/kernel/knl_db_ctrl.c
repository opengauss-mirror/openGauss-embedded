...
void db_init_logfile_ctrl(knl_session_t *session, uint32 *offset)
{
    ...
    for (i = 0; i < GS_MAX_LOG_FILES; i++) {
        db->logfiles.items[i].ctrl =
            (log_file_ctrl_t *)db_get_ctrl_item(db->ctrl.pages, i, sizeof(log_file_ctrl_t), *offset);
...
void db_init_datafile_ctrl(knl_session_t *session, uint32 *offset)
{
    ...
    for (i = 0; i < GS_MAX_DATA_FILES; i++) {
        db->datafiles[i].ctrl =
            (datafile_ctrl_t *)db_get_ctrl_item(db->ctrl.pages, i, sizeof(datafile_ctrl_t), *offset);
 ...
 */
void db_fetch_ctrlfile_name(text_t *files, text_t *name)
{
...
status_t db_save_arch_ctrl(knl_session_t *session, uint32 id)
{
    ...
    for (i = 0; i < db->ctrlfiles.count; i++) {
        ...
        page_id = db->ctrl.arch_segment + (id / count) % (CTRL_MAX_PAGE - db->ctrl.arch_segment);
        if (db_save_ctrl_page(session, ctrlfile, page_id) != GS_SUCCESS) {
            cm_spin_unlock(&db->ctrl_lock);
            return GS_ERROR;
        }
...
void db_update_sysdata_version(knl_session_t *session)
{
    ...
    GS_LOG_RUN_INF("[UPGRADE] system data version is update from %u to %u", db->ctrl.core.sysdata_version,
        CORE_SYSDATA_VERSION);
