...
idx_accessor_t g_invalid_index_acsor = { (knl_cursor_operator_t)db_invalid_cursor_operation,
                                         (knl_cursor_operator_t)db_invalid_cursor_operation, (knl_cursor_operator_t)db_invalid_cursor_operation };
...
void idx_decode_row(knl_session_t *session, knl_cursor_t *cursor, uint16 *offsets, uint16 *lens, uint16 *size)
{
    ...
    for (i = 0; i < index->desc.column_count; i++) {
        ...
        switch (column->datatype) {
            ...
            default:
                knl_panic_log(0,
                    "column's datatype is unknown, panic info: table %s index %s column datatype %u "
                    "page %u-%u type %u",
                    entity->table.desc.name, index->desc.name, column->datatype, cursor->rowid.file, cursor->rowid.page,
                    ((page_head_t *)cursor->page_buf)->type);
        }
    }
}
...
status_t idx_generate_dupkey_error(index_t *index, char *key)
{
    ...
    ret =
        snprintf_s(msg_buf, MAX_DUPKEY_MSG_LEN, MAX_DUPKEY_MSG_LEN - 1, ", index %s, duplicate key ", index->desc.name);
...
static status_t idx_recycle_index_pages(knl_session_t *session, index_recycle_item_t *item, idx_recycle_stats_t *stats)
{
    ...
    if (item->scn > min_scn) {
        if (!GS_INVALID_SCN(force_recycle_scn) && (cur_scn - item->scn) > force_recycle_scn) {
