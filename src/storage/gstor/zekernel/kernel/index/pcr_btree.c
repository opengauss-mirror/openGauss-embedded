...
static page_id_t pcrb_clean_copied_itl(knl_session_t *session, uint64 xid, page_id_t page_id)
{
    ...
    for (uint8 i = 0; i < page->itls; i++) {
        ...
        if (itl->xid.value == session->rm->xid.value) {
            ...
            knl_panic_log(txn_info.status == (uint8)XACT_END,
                "txn's status is abnormal, panic info: page %u-%u type "
                "%u txn status %u",
                page_id.file, page_id.page, ((page_head_t *)CURR_PAGE)->type, txn_info.status);
            itl->is_active = GS_FALSE;
...
static void pcrb_generate_itl_undo(knl_session_t *session, knl_cursor_t *cursor, btree_page_t *page, pcr_itl_t *itl,
    undo_data_t *undo)
{
    if (!session->is_timescale || session->rm->prev != GS_INVALID_ID16 || session->is_insert == GS_FALSE)
    {
    ...
    bool32 *changed)
{
    ...
    *changed = GS_TRUE;
    ...
    if (lock_itl(session, AS_PAGID(page->head.id), session->itl_id, part_loc, AS_PAGID(page->next), LOCK_TYPE_PCR_KX) !=
        GS_SUCCESS) {
        return GS_ERROR;
    }
...
static bool32 pcrb_find_update_pos(knl_session_t *session, btree_t *btree, knl_scan_key_t *scan_key,
    btree_path_info_t *path_info, btree_find_type type, bool32 *is_same, bool32 *compact_leaf)
{
    ...
    for (;;) {
        ...
        knl_panic_log(level == page->level,
            "the page's level is abnormal, panic info: page %u-%u type %u index %s "
            "level %u page_level %u",
            AS_PAGID(page->head.id).file, AS_PAGID(page->head.id).page, page->head.type, index->desc.name, level,
            page->level);
        ...
        knl_panic_log(page->head.type == PAGE_TYPE_PCRB_NODE,
            "page type is abnormal, panic info: page %u-%u type %u index %s", AS_PAGID(page->head.id).file,
            AS_PAGID(page->head.id).page, page->head.type, index->desc.name);
        knl_panic_log(page->seg_scn == seg->seg_scn,
            "the seg_scn of page and segment are not same, panic info: "
            "page %u-%u type %u index %s page seg_scn %llu seg seg_scn %llu",
    ...
    bool32 is_rebuild, bool32 *is_same, bool32 *changed, idx_conflict_info_t *conflict_info)
{
    ...
    for (;;) {
        *changed = GS_FALSE;
        ...
        btree_page_t *page = BTREE_CURR_PAGE;
        uint16 pct_size =
            ((is_rebuild || btree->is_shadow) && PCRB_MAX_COST_SIZE(key) <= DEFAULT_PAGE_SIZE - BTREE_PCT_SIZE(btree)) ?
...
static status_t pcrb_do_insert(knl_session_t *session, knl_cursor_t *cursor, idx_conflict_info_t *conflict_info)
{
    ...
    if (pcrb_enter_insert(session, cursor, &path_info, GS_FALSE, &is_same, &page_changed, conflict_info) !=
        GS_SUCCESS) {
        return GS_ERROR;
    }
    ...
    if (!session->is_timescale || session->rm->prev != GS_INVALID_ID16 )
    {
...
{
    btree_page_t *src_page = (btree_page_t *)cursor->page_buf;
    ...
    *need_wait = GS_FALSE;
    ...
    if (keys > 0) {
        if (!session->is_timescale || session->rm->prev != GS_INVALID_ID16) {
            ...
            pcr_itl_t *itl = pcrb_get_itl(BTREE_CURR_PAGE, session->itl_id);
            itl->undo_page = undo_page_info->undo_rid.page_id;
            itl->undo_slot = undo_page_info->undo_rid.slot;
            itl->ssn = (uint32)cursor->ssn;
        }
...
static status_t pcrb_prepare_batch_insert(knl_session_t *session, knl_cursor_t *cursor)
{
    ...
    if (SECUREC_UNLIKELY(btree->segment == NULL)) {
        if (IS_PART_INDEX(btree->index)) {
            knl_panic_log(cursor->index_part != NULL,
                "the index_part is NULL, panic info: page %u-%u type %u table %s index %s", cursor->rowid.file,
    ...
    errno_t ret =
        memcpy_sp((char *)page + page->free_begin, DEFAULT_PAGE_SIZE - page->free_begin, &min_key, sizeof(pcrb_key_t));
    knl_securec_check(ret);
...
static status_t pcrb_insert_into_sort_page(knl_session_t *session, btree_t *btree, btree_page_t *page, pcrb_key_t *key)
{
    rd_pcrb_insert_t rd_insert = {
        .ssn = 0,
        .is_reuse = GS_FALSE,
        .undo_page = INVALID_UNDO_PAGID
    };
    btree_path_info_t path_info;
    knl_scan_key_t scan_key;
...
status_t pcrb_batch_insert(knl_handle_t handle, knl_cursor_t *cursor)
{
    ...
    cm_pop(session->stack);
    cursor->row = org_row;
    knl_panic_log(status != GS_SUCCESS || batch_size == 0,
        "batch insert SUCCESS but batch_size is not zero, "
...
status_t pcrb_insert(knl_session_t *session, knl_cursor_t *cursor)
{
    ...
    if (SECUREC_UNLIKELY(btree->segment == NULL)) {
        if (IS_PART_INDEX(btree->index)) {
            knl_panic_log(cursor->index_part != NULL,
                "current index_part is NULL, panic info: page %u-%u type %u, "
                "table %s, index %s",
                cursor->rowid.file, cursor->rowid.page, ((page_head_t *)cursor->page_buf)->type,
                ((table_t *)cursor->table)->desc.name, ((index_t *)btree->index)->desc.name);
            if (btree_create_part_entry(session, btree, cursor->index_part) != GS_SUCCESS) {
...
status_t pcrb_insert_into_shadow(knl_session_t *session, knl_cursor_t *cursor)
{
    ...
    path_info.get_sibling = GS_FALSE;
    if (pcrb_enter_insert(session, cursor, &path_info, GS_TRUE, &is_same, &page_changed, &conflict_info) !=
        GS_SUCCESS) {
        return GS_ERROR;
    }
    ...
    btree_path_info_t *path_info, uint32 level)
{
    ...
    knl_panic_log((uint16)(page->free_end - page->free_begin) >= PCRB_COST_SIZE(key),
        "page's free size is abnormal, "
...
void pcrb_undo_itl(knl_session_t *session, undo_row_t *ud_row, undo_page_t *ud_page, int32 ud_slot)
{
    ...
    knl_panic_log(itl->xid.value == session->rm->xid.value,
        "xid of itl and rm are not same, panic info: "
        "page %u-%u type %u itl xid %llu rm xid %llu",
        AS_PAGID(page->head.id).file, AS_PAGID(page->head.id).page, page->head.type, itl->xid.value,
...
static void pcrb_undo_insert_key(knl_session_t *session, btree_t *btree, pcrb_key_t *ud_key, rd_pcrb_undo_t *redo)
{
    btree_page_t *page = BTREE_CURR_PAGE;
    ...
    pcr_itl_t *itl = pcrb_get_itl(page, key->itl_id);
    knl_panic_log(itl->xid.value == session->rm->xid.value,
        "xid of itl and rm are not same, panic info: page %u-%u "
        "type %u, index %s itl xid %llu rm xid %llu",
        AS_PAGID(page->head.id).file, AS_PAGID(page->head.id).page, page->head.type,
        ((index_t *)btree->index)->desc.name, itl->xid.value, session->rm->xid.value);
    itl->ssn = redo->ssn;
    ...
    knl_dictionary_t *dc)
{
    ...
    pcr_itl_t *itl = pcrb_get_itl(page, key->itl_id);
    knl_panic_log(itl->xid.value == session->rm->xid.value,
        "the xid of itl and rm are not same, panic info: "
        "page %u-%u type %u, index %s itl xid %llu rm xid %llu",
        AS_PAGID(page->head.id).file, AS_PAGID(page->head.id).page, page->head.type,
        ((index_t *)btree->index)->desc.name, itl->xid.value, session->rm->xid.value);
...
status_t pcrb_construct(btree_mt_context_t *ctx)
{
    ...
    while (!cursor.eof) {
        ...
        pct_size =
            (PCRB_COST_SIZE(key) > DEFAULT_PAGE_SIZE - BTREE_PCT_SIZE(btree)) ? (uint16)0 : BTREE_PCT_SIZE(btree);
...
void pcrb_get_txn_info(knl_session_t *session, btree_page_t *page, pcrb_key_t *key, txn_info_t *txn_info)
{
    if (key->itl_id == GS_INVALID_ID8) {
        ...
        knl_panic_log(key->itl_id < page->itls,
            "the key's itl_id is nore than page's itls, panic info: page %u-%u "
            "type %u itl_id %u itls %u",
            AS_PAGID(page->head.id).file, AS_PAGID(page->head.id).page, page->head.type, key->itl_id, page->itls);
        pcr_itl_t *itl = pcrb_get_itl(page, key->itl_id);
...
void pcrb_validate_page(knl_session_t *session, page_head_t *page)
{
    ...
    for (uint16 i = 0; i < copy_page->keys; i++) {
        ...
        knl_panic_log(*dir < copy_page->free_begin,
            "dir's offset is more than copy_page's free_begin, panic info: "
    ...
    for (uint16 i = 0; i < copy_page->keys; i++) {
        ...
        knl_panic_log(total_size <= copy_page->free_begin,
            "total_size is more than copy_page's free_begin, "
    ...
    knl_panic_log(copy_page->free_begin + copy_page->free_size == copy_page->free_end,
        ...
        "free_end %u",
        AS_PAGID(copy_page->head.id).file, AS_PAGID(copy_page->head.id).page, copy_page->head.type,
        AS_PAGID(page->id).file, AS_PAGID(page->id).page, page->type, copy_page->free_begin, copy_page->free_size,
        copy_page->free_end);
    CM_RESTORE_STACK(session->stack);
}
#endif
...
int32 pcrb_cmp_mtrl_colomn_data(knl_handle_t col1, knl_handle_t col2, gs_type_t type, uint16 *offset1, uint16 *offset2)
{
    ...
    static void *labels[] = {
        ...
        [GS_TYPE_I(GS_TYPE_SMALLINT)] = &&LABEL_INTEGER,
        [GS_TYPE_I(GS_TYPE_USMALLINT)] = &&LABEL_UINT32,
        [GS_TYPE_I(GS_TYPE_TINYINT)] = &&LABEL_INTEGER,
        [GS_TYPE_I(GS_TYPE_UTINYINT)] = &&LABEL_UINT32,
        [GS_TYPE_I(GS_TYPE_FLOAT)] = &&LABEL_REAL,
...
int32 pcrb_cmp_mtrl_column(knl_column_t *column, pcrb_key_t *key1, pcrb_key_t *key2, uint32 idx_col_id, uint16 *offset1,
    uint16 *offset2)
{
