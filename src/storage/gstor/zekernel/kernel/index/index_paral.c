...
void idx_part_paral_proc(thread_t *thread)
{
    ...
    while (!thread->closed) {
        ...
        if (ctx->index_cnt == 1) {
            ...
            if (db_fill_multi_indexes_paral(session, dc, ctx->indexes, ctx->index_cnt, ctx->paral_count, part_loc) !=
                GS_SUCCESS) {
                idx_set_err_msg(ctx->err_msg);
                thread->result = GS_ERROR;
                break;
...
static status_t idx_fill_segments(knl_session_t *session, knl_cursor_t *cursor, mtrl_sort_ctrl_t *sort_ctrl,
    idx_sort_worker_t *worker)
{
...
static status_t idx_fill_part_segments(knl_session_t *session, knl_cursor_t *cursor, mtrl_sort_ctrl_t *sort_ctrl,
    idx_sort_worker_t *worker)
{
    knl_part_locate_t part_loc;
...
static status_t idx_init_build_segment(knl_session_t *session, idx_sort_worker_t *worker, mtrl_sort_ctrl_t *sort_ctrl,
    knl_cursor_t *cursor)
{
...
static status_t idx_fill_muti_segments(knl_session_t *session, knl_cursor_t *cursor, mtrl_sort_ctrl_t *sort_ctrl,
    idx_build_worker_t *build_worker)
{
    char *key = NULL;
    mtrl_rowid_t rid;
    status_t status = GS_SUCCESS;
    ...
    for (;;) {
        ...
        for (uint32 i = 0; i < build_worker->index_count; i++) {
            ...
            if (mtrl_insert_row_parallel(&curr_idx_info->mtrl_ctx, curr_idx_info->seg_id, key, &sort_ctrl[i], &rid) !=
                GS_SUCCESS) {
                status = GS_ERROR;
                break;
            }
...
static status_t idx_init_multi_segment(knl_session_t *session, idx_build_worker_t *worker, mtrl_sort_ctrl_t *sort_ctrl,
    knl_cursor_t *cursor)
{
...
static status_t idx_fill_multi_part_segments(knl_session_t *session, knl_cursor_t *cursor, mtrl_sort_ctrl_t *sort_ctrl,
    idx_build_worker_t *worker)
{
    knl_part_locate_t part_loc;
...
void idx_init_construct_ctx(idx_paral_sort_ctx_t *paral_ctx, uint32 id1, uint32 id2, btree_mt_context_t *ctx)
{
...
static status_t idx_init_build_worker(idx_build_worker_t *worker, btree_t **btree)
{
    ...
    worker->idx_info =
        (idx_multi_info_t *)cm_push(worker->session->stack, worker->index_count * sizeof(idx_multi_info_t));
...
static void idx_init_construct_workers(idx_paral_sort_ctx_t *sort_ctx, idx_sort_worker_t *worker, uint32 index_id,
    uint32 paral_no)
{
...
status_t idx_construct_alloc_resource(knl_session_t *session, idx_paral_sort_ctx_t *sort_ctx)
{
    ...
    errno_t err = memset_sp(sort_ctx->workers, sizeof(idx_sort_worker_t) * sort_ctx->paral_count, 0,
        sizeof(idx_sort_worker_t) * sort_ctx->paral_count);
    knl_securec_check(err);
...
status_t idx_check_workers_status(idx_paral_sort_ctx_t *paral_ctx, uint8 index_id, uint8 build_count,
    idx_sort_phase_t phase1, uint8 *count)
{
    idx_sort_worker_t *worker = NULL;
...
status_t idx_switch_create_phase(knl_session_t *session, idx_paral_sort_ctx_t *paral_ctx, idx_sort_phase_t phase1,
    idx_sort_phase_t phase2)
{
...
static void idx_active_rebuild_worker(idx_paral_rebuild_ctx_t *ctx, uint32 worker_index, knl_part_locate_t part_loc,
    uint32 split_cnt)
{
    ...
    knl_part_locate_t *current_part_loc, bool32 *finish_stat, knl_parts_locate_t parts_loc)
{
    ...
    for (;;) {
        ...
        if (!IS_READY_PART(table_part) || !is_idx_part_existed(current_part_loc, parts_loc, GS_FALSE)) {
            current_part_loc->part_no++;
            continue;
        }
...
static status_t idx_split_table_segment_range(idx_paral_rebuild_worker_t *worker, knl_scan_range_t *split_range[],
    uint32 *range_cnt)
{
...
static bool32 idx_split_worker_range(idx_paral_rebuild_worker_t *worker, uint32 split_cnt)
{
    ...
    for (; range_index < split_cnt; range_index++) {
        ...
        if (range[range_index]) {
            range_cnt++;
            errno_t err = memset_sp(range[range_index], sizeof(knl_scan_range_t), 0, sizeof(knl_scan_range_t));
            knl_securec_check(err);
            continue;
        }
...
status_t idx_open_cursor(idx_paral_rebuild_worker_t *worker, knl_cursor_t *cursor, int32 range_index, bool32 reopen)
{
    idx_paral_rebuild_ctx_t *ctx = worker->ctx;
    knl_dictionary_t *dc = ctx->dc;
    knl_session_t *session = worker->session;
    ...
    for (;;) {
        ...
        GS_LOG_DEBUG_INF("lfile = %u, page= %u, vmid = %u", range->l_page.file, range->l_page.page, range->l_page.vmid);
        GS_LOG_DEBUG_INF("rfile = %u, page= %u, vmid = %u", range->r_page.file, range->r_page.page, range->r_page.vmid);
...
static status_t idx_rebuild_index_range_proc_entity(idx_paral_rebuild_worker_t *worker, knl_cursor_t *cursor,
    knl_dictionary_t *dc)
{
    knl_session_t *session = worker->session;
    int32 current_range_index;
