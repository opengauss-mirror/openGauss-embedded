...
typedef bool32 (*btree_recycle_check_t)(knl_session_t *session, btree_page_t *page, bool32 *is_sparse, bool32 *is_empty,
    knl_scn_t force_recycle_scn);
typedef void (*btree_recycle_t)(knl_session_t *session, btree_t *btree, page_id_t pagid, uint64 lsn,
...
status_t btree_dump_page(knl_session_t *session, page_head_t *page_head, cm_dump_t *dump)
{
    ...
    for (uint32 slot = 0; slot < page->keys; slot++) {
        ...
        cm_dump(dump, "\towscn/infinite/deleted/cleaned: %u/%u/%u/%u", key->is_owscn, key->is_infinite, key->is_deleted,
            key->is_cleaned);
    ...
    switch (type) {
        ...
        case GS_TYPE_UINT64:
            if (len == sizeof(uint32)) {
                *(uint64 *)CURR_KEY_PTR(key) = (uint64)(*(uint32 *)data);
...
void btree_compact_page(knl_session_t *session, btree_page_t *page, knl_scn_t min_scn)
{
    ...
    while ((char *)key < (char *)page + page->free_begin) {
        ...
        knl_panic_log(key->bitmap < page->keys,
            "bitmap is more than page's keys, panic info: page %u-%u type %u "
            "bitmap %u page keys %u",
...
static void btree_insert_new_node(knl_session_t *session, rowid_t *path, btree_key_t *insert_key, btree_key_t *new_key)
{
    ...
    if (dst_page->level != 0) {
        ...
        if (need_redo) {
            ...
            log_put(session, RD_BTREE_INSERT, &redo, (uint32)OFFSET_OF(rd_btree_insert_t, key), LOG_ENTRY_FLAG_NONE);
...
static void btree_split_normal(knl_session_t *session, btree_page_t *src_page, rowid_t *path, btree_key_t *insert_key,
    uint8 *itl_map, btree_key_t *new_key, bool32 use_pct, btree_t *btree)
{
    uint16 pos = 0;
    btree_dir_t *dir = NULL;
    btree_key_t *key = NULL;
...
static status_t btree_alloc_itl(knl_session_t *session, knl_cursor_t *cursor, btree_page_t *page, itl_t **itl)
{
    ...
    if (lock_itl(session, AS_PAGID(page->head.id), session->itl_id, part_loc, AS_PAGID(page->next), LOCK_TYPE_RCR_KX) !=
        GS_SUCCESS) {
        return GS_ERROR;
    }
    ...
    bool32 *is_wait, idx_conflict_info_t *conflict_info)
{
    ...
    *is_wait = GS_FALSE;
    ...
    if (cursor->isolevel == (uint8)ISOLATION_SERIALIZABLE && txn_info.status == (uint8)XACT_END &&
        cursor->query_scn < txn_info.scn) {
    ...
    undo_type_t type)
{
    ...
    if (is_same) {
        ...
        if (btree->is_shadow && type == UNDO_BTREE_INSERT && !old_key->is_deleted) {
            ...
            if (type == UNDO_BTREE_DELETE) {
                knl_panic_log(IS_SAME_ROWID(old_key->rowid, cursor->rowid),
                    "[BTREE]index %s try to delete a wrong key %u-%u-%u, cursor rid %u-%u-%u", btree->index->desc.name,
        ...
        knl_panic_log(page->free_size >= BTREE_COST_SIZE(key),
            "page's free_size is abnormal, panic info: page %u-%u "
            "type %u free_size %u, table %s, index %s btree cost size %lu",
            cursor->rowid.file, cursor->rowid.page, page->head.type, page->free_size, table->desc.name,
            index->desc.name, BTREE_COST_SIZE(key));
        undo.snapshot.is_xfirst = GS_TRUE;
        undo.snapshot.undo_page = INVALID_UNDO_PAGID;
        undo.snapshot.scn = 0;
        undo.snapshot.is_owscn = 0;
...
static bool32 btree_find_update_pos(knl_session_t *session, btree_t *btree, knl_scan_key_t *scan_key,
    btree_find_type type, btree_path_info_t *path_info, bool32 *is_same, bool32 *compact_leaf)
{
    btree_segment_t *seg = BTREE_SEGMENT(btree->entry, btree->segment);
    knl_tree_info_t tree_info;
    ...
    page_id_t page_id;
    uint16 cost_size =
        (type == BTREE_FIND_DELETE ? sizeof(itl_t) : BTREE_COST_SIZE((btree_key_t *)scan_key->buf) + sizeof(itl_t));
    bool32 cmp_rowid;
    ...
    for (;;) {
        ...
        page = BTREE_CURR_PAGE;
        knl_panic_log(level == page->level,
            "the btree's level is abnormal, panic info: page %u-%u type %u level %u "
            "page level %u, index %s",
            AS_PAGID(page->head.id).file, AS_PAGID(page->head.id).page, page->head.type, level, page->level,
            index->desc.name);
        if (page->is_recycled) {
            buf_leave_page(session, GS_FALSE);
            return GS_FALSE;
        ...
        knl_panic_log(page->head.type == PAGE_TYPE_BTREE_NODE,
            "page type is abnormal, panic info: page %u-%u type %u "
            "index %s",
            AS_PAGID(page->head.id).file, AS_PAGID(page->head.id).page, page->head.type, index->desc.name);
        knl_panic_log(page->seg_scn == seg->seg_scn,
            "seg_scn of page and segment are not same, panic info: "
            "page %u-%u type %u, index %s page seg_scn %llu seg_scn %llu",
            AS_PAGID(page->head.id).file, AS_PAGID(page->head.id).page, page->head.type, index->desc.name,
            page->seg_scn, seg->seg_scn);
...
static status_t btree_do_insert(knl_session_t *session, knl_cursor_t *cursor, idx_conflict_info_t *conflict_info)
{
    ...
    if (btree->segment == NULL) {
        if (IS_PART_INDEX(btree->index)) {
            knl_panic_log(cursor->index_part != NULL,
                "the index_part is NULL, panic info: page %u-%u type %u "
                "table %s index %s",
    ...
    if (btree_enter_insert(session, cursor, &path_info, GS_FALSE, &is_same, &compact_leaf, conflict_info) !=
        GS_SUCCESS) {
        return GS_ERROR;
    }
...
status_t btree_insert_into_shadow(knl_session_t *session, knl_cursor_t *cursor)
{
    ...
    if (btree_enter_insert(session, cursor, &path_info, GS_TRUE, &is_same, &compact_leaf, &conflict_info) !=
        GS_SUCCESS) {
        return GS_ERROR;
    }
...
static status_t btree_need_wait(knl_session_t *session, knl_cursor_t *cursor, btree_page_t *page, btree_dir_t *dir,
    bool32 *need_wait)
{
    itl_t *itl = NULL;
    txn_info_t txn_info;
    ...
    for (;;) {
        ...
        if (!(*is_same)) {
            ...
            knl_panic_log(0, "[BTREE] index %s cannot find the key %u-%u-%u to be deleted in page %u-%u",
                btree->index->desc.name, (uint32)key->rowid.file, (uint32)key->rowid.page, (uint32)key->rowid.slot,
                (uint32)AS_PAGID(page->head.id).file, (uint32)AS_PAGID(page->head.id).page);
        }
...
static void btree_get_recycle_part_orgscn(index_t *index, knl_part_locate_t part_loc, index_recycle_item_t *item)
{
    if (IS_PART_INDEX(index)) {
        ...
        if (IS_PARENT_TABPART(&part->desc)) {
            knl_panic_log(part_loc.subpart_no != GS_INVALID_ID32,
                "the subpart_no is invalid, panic info: table %s "
                "table_part %s index %s",
                table->desc.name, part->desc.name, index->desc.name);
    ...
    knl_dictionary_t *dc)
{
    ...
    knl_panic_log(is_same, "[BTREE] index %s cannot find the key %u-%u-%u for undo insert in page %u-%u",
        btree->index->desc.name, (uint32)ud_key->rowid.file, (uint32)ud_key->rowid.page, (uint32)ud_key->rowid.slot,
        (uint32)AS_PAGID(page->head.id).file, (uint32)AS_PAGID(page->head.id).page);

    dir = BTREE_GET_DIR(page, path_info.path[0].slot);
    key = BTREE_GET_KEY(page, dir);
    knl_panic_log(IS_SAME_PAGID(key->undo_page, AS_PAGID(ud_page->head.id)),
        "key's undo_page and ud_page are not "
        "same, panic info: page %u-%u type %u, ud_page %u-%u type %u index %s",
    ...
    knl_panic_log(key->undo_slot == ud_slot,
        "undo_slot is abnormal, panic info: page %u-%u type %u undo_slot %u, "
        "ud_page %u-%u type %u ud_slot %u, index %s",
        AS_PAGID(page->head.id).file, AS_PAGID(page->head.id).page, page->head.type, key->undo_slot,
        AS_PAGID(ud_page->head.id).file, AS_PAGID(ud_page->head.id).page, ud_page->head.type, ud_slot,
        ((index_t *)btree->index)->desc.name);
    knl_panic_log(dir->itl_id != GS_INVALID_ID8,
        "itl_id is invalid, panic info: page %u-%u type %u itl_id %u, "
        "ud_page %u-%u type %u, index %s",
        AS_PAGID(page->head.id).file, AS_PAGID(page->head.id).page, page->head.type, dir->itl_id,
        AS_PAGID(ud_page->head.id).file, AS_PAGID(ud_page->head.id).page, ud_page->head.type,
        ((index_t *)btree->index)->desc.name);
    itl = BTREE_GET_ITL(page, dir->itl_id);
    knl_panic_log(itl->xid.value == session->rm->xid.value,
        "the xid of itl and rm are not equal, panic info: "
        "page %u-%u type %u ud_page %u-%u type %u index %s itl xid %llu rm xid %llu",
        AS_PAGID(page->head.id).file, AS_PAGID(page->head.id).page, page->head.type, AS_PAGID(ud_page->head.id).file,
...
void btree_append_to_page(knl_session_t *session, btree_page_t *page, btree_key_t *key, uint8 itl_id)
{
    ...
    dir->offset = page->free_begin;
    dir->itl_id = itl_id;
    err = memcpy_sp((char *)page + page->free_begin, BTREE_PAGE_FREE_SIZE(page) - sizeof(btree_dir_t), key,
        (size_t)key->size);
    knl_securec_check(err);
    page->free_begin += (uint16)key->size;
    page->free_end -= sizeof(btree_dir_t);
    ...
    char **key_buf, btree_key_t *key, uint32 level)
{
    ...
    pct_size = (BTREE_COST_SIZE(key) > DEFAULT_PAGE_SIZE - BTREE_PCT_SIZE(btree) || vm_page->level > 0) ?
        (uint16)0 :
        BTREE_PCT_SIZE(btree);
    if (vm_page->free_begin + BTREE_COST_SIZE(key) + pct_size > vm_page->free_end) {
        ...
        if (parent_page[level + 1] != NULL) {
            ...
            log_atomic_op_end(session);
            if (GS_SUCCESS !=
                btree_construct_ancestors(session, btree, parent_page, key_buf, (btree_key_t *)min_key, level + 1)) {
                return GS_ERROR;
            }
        }
...
status_t btree_construct(btree_mt_context_t *ctx)
{
    ...
    int16 free_size = (int16)(DEFAULT_PAGE_SIZE - sizeof(btree_page_t) - sizeof(page_tail_t) - cipher_reserve_size -
        page->itls * sizeof(itl_t) - BTREE_RESERVE_SIZE);

    while (!cursor.eof) {
        key = (btree_key_t *)cursor.sort.row;
        pct_size =
            (BTREE_COST_SIZE(key) > DEFAULT_PAGE_SIZE - BTREE_PCT_SIZE(btree)) ? (uint16)0 : BTREE_PCT_SIZE(btree);
        if (free_size - (int16)pct_size - (int16)BTREE_COST_SIZE(key) < 0) { // key->size is 3900 at most now
            ...
            if (btree_construct_ancestors(session, btree, parent_page, key_buf, (btree_key_t *)dst_mkey, 0) !=
                GS_SUCCESS) {
                status = GS_ERROR;
                break;
            }
...
static void btree_get_parent_page(knl_session_t *session, btree_t *btree, knl_scan_key_t *scan_key, uint32 child_level,
    btree_path_info_t *path_info)
{
    btree_segment_t *seg = BTREE_SEGMENT(btree->entry, btree->segment);
    knl_tree_info_t tree_info;
...
static bool32 btree_is_recycled_page(knl_session_t *session, btree_page_t *page, bool32 *is_sparse, bool32 *is_empty,
    knl_scn_t force_recycle_scn)
{
    uint32 i;
    btree_dir_t *dir = NULL;
    ...
    *is_empty = GS_TRUE;
    ...
    total_size = (uint16)(session->kernel->attr.page_size - sizeof(itl_t) * page->itls - sizeof(page_tail_t) -
...
static status_t btree_coalesce_by_extents(knl_session_t *session, btree_t *btree, btree_coalesce_assist_t *assist,
    idx_recycle_stats_t *stats, knl_part_locate_t part_loc)
{
    page_id_t leaf_id;
    btree_page_t *page = NULL;
    uint64 lsn;
    ...
    for (;;) {
        ...
        if (extent_size == 0) {
            ...
            if (session->id == SESSION_ID_IDX_RECYCLE && session->stat.disk_reads > disk_reads) {
                 ...
                 */
                uint32 sleep_msecs = (uint32)((session->stat.disk_read_time - disk_read_time) / MICROSECS_PER_MILLISEC);
...
status_t btree_coalesce(knl_session_t *session, btree_t *btree, idx_recycle_stats_t *stats, knl_part_locate_t part_loc)
{
    ...
    GS_LOG_RUN_INF("%s index %s completed, total pages %u, empty pages %u, sparse pages %u, "
        ...
        GS_INVALID_SCN(stats->force_recycle_scn) ? "coalesce" : "force coalesce", btree->index->desc.name, total_pages,
        stats->empty_pages, stats->sparse_pages, stats->recycled_pages, stats->free_pages, time_used_ms);
...
void btree_validate_page(knl_session_t *session, page_head_t *page)
{
    ...
    for (uint8 j = 0; j < copy_page->itls; j++) {
        ...
        if (itl->is_active) {
            knl_panic_log(itl->scn == 0,
                "commit scn is abnormal, panic info: page %u-%u type %u, copy_page %u-%u "
                "type %u",
                AS_PAGID(page->id).file, AS_PAGID(page->id).page, page->type, AS_PAGID(copy_page->head.id).file,
                AS_PAGID(copy_page->head.id).page, copy_page->head.type);
            knl_panic_log(itl->xid.value != GS_INVALID_ID64,
                "itl xid is invalid, panic info: page %u-%u type %u, "
                "copy_page %u-%u type %u",
                AS_PAGID(page->id).file, AS_PAGID(page->id).page, page->type, AS_PAGID(copy_page->head.id).file,
                AS_PAGID(copy_page->head.id).page, copy_page->head.type);
        }
    }
    ...
    for (uint16 i = 0; i < copy_page->keys; i++) {
        ...
        knl_panic_log(dir->offset < copy_page->free_begin,
            "offset in dir is more than page's free_begin, panic info: "
            "page %u-%u type %u, copy_page %u-%u type %u dir offset %u free_begin %u",
            AS_PAGID(page->id).file, AS_PAGID(page->id).page, page->type, AS_PAGID(copy_page->head.id).file,
            AS_PAGID(copy_page->head.id).page, copy_page->head.type, dir->offset, copy_page->free_begin);
        knl_panic_log(dir->offset >= sizeof(btree_page_t) + space->ctrl->cipher_reserve_size,
            ...
            "cipher_reserve_size %u",
            AS_PAGID(page->id).file, AS_PAGID(page->id).page, page->type, AS_PAGID(copy_page->head.id).file,
            AS_PAGID(copy_page->head.id).page, copy_page->head.type, dir->offset, space->ctrl->cipher_reserve_size);

        uint8 itl_id = dir->itl_id;
        knl_panic_log(itl_id == GS_INVALID_ID8 || itl_id <= copy_page->itls,
            "itl_id is abnormal, panic info: "
            "page %u-%u type %u, copy_page %u-%u type %u itl_id %u copy_page's itls %u",
            AS_PAGID(page->id).file, AS_PAGID(page->id).page, page->type, AS_PAGID(copy_page->head.id).file,
            AS_PAGID(copy_page->head.id).page, copy_page->head.type, itl_id, copy_page->itls);
    }
    ...
    for (uint16 i = 0; i < copy_page->keys; i++) {
        ...
        knl_panic_log(key->size <= GS_MAX_KEY_SIZE - space->ctrl->cipher_reserve_size,
            "the key's size is abnormal, "
        ...
        total_size += key->size;
        knl_panic_log(total_size <= copy_page->free_begin,
            "total_size is more than page's free_begin, panic info: "
            "page %u-%u type %u, copy_page %u-%u type %u free_begin %u total_size %llu",
            AS_PAGID(page->id).file, AS_PAGID(page->id).page, page->type, AS_PAGID(copy_page->head.id).file,
    ...
    knl_panic_log(copy_page->free_begin + copy_page->free_size == copy_page->free_end,
        "copy_page's free size is "
    ...
    txn_info_t *txn_info)
{
    ...
    if (dir->itl_id == GS_INVALID_ID8) {
        ...
        knl_panic_log(dir->itl_id < page->itls,
            "dir's itl id is more than page's itls, panic info: page %u-%u "
            "type %u itl id %u itls %u",
            AS_PAGID(page->head.id).file, AS_PAGID(page->head.id).page, page->head.type, dir->itl_id, page->itls);
        itl = BTREE_GET_ITL(page, dir->itl_id);
