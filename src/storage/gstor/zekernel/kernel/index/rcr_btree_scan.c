...
int32 btree_cmp_column_data(void *col1, void *col2, gs_type_t type, uint16 *offset, bool32 is_pcr)
{
    ...
    static void *labels[] = {
        ...
        [GS_TYPE_I(GS_TYPE_UINT64)] = &&LABEL_UBIGINT,
...
int32 btree_compare_key(index_t *index, knl_scan_key_t *scan_key, btree_key_t *key, bool32 cmp_rowid, bool32 *is_same)
{
...
void btree_binary_search(index_t *index, btree_page_t *page, knl_scan_key_t *scan_key, btree_path_info_t *path_info,
    bool32 cmp_rowid, bool32 *is_same)
{
    ...
    knl_panic_log(page->level == 0 || page->keys > 0,
        "page level is not equal to zero and the keys is not more than "
        "zero, panic info: page %u-%u type %u index %s page_level %u page_keys %u",
        AS_PAGID(page->head.id).file, AS_PAGID(page->head.id).page, page->head.type, index->desc.name, page->level,
        page->keys);
    if (page->keys == 0) {
        *is_same = GS_FALSE;
    }
...
static status_t btree_enter_locate_page(knl_session_t *session, btree_search_t *search_info, uint8 level,
    page_id_t page_id, btree_page_t **page)
{
    if (search_info->read_root_copy && BTREE_ROOT_COPY_VALID(session->index_root)) {
        ...
        *page = (btree_page_t *)BTREE_GET_ROOT_COPY(session->index_root);
        ...
        if (level > 0) {
            if (buf_read_page(session, page_id, LATCH_MODE_S, ENTER_PAGE_NORMAL | ENTER_PAGE_HIGH_AGE) != GS_SUCCESS) {
        ...
        if (path_info->path[page->level].slot >= page->keys) {
            if (search_info->is_dsc_scan) {
                knl_panic_log(page->level == 0,
                    "page level is not equal to zero, panic info: page %u-%u type %u "
    ...
    bool32 *is_found)
{
    ...
    if (cursor->restrict_subpart) {
        knl_panic_log(IS_COMPART_TABLE(part_table),
            "part_table is not compart table, panic info: index %s table %s "
            "table_part %s page %u-%u type %u",
            ((index_t *)btree->index)->desc.name, table->desc.name, table_part->desc.name, cursor->rowid.file,
        ...
        knl_panic_log(subpart != NULL,
            "the subpart is NULL, panic info: page %u-%u type %u table %s table_part %s "
            "index %s",
            cursor->rowid.file, cursor->rowid.page, ((page_head_t *)cursor->page_buf)->type, table->desc.name,
            table_part->desc.name, ((index_t *)btree->index)->desc.name);
    ...
    btree_key_t *key, uint32 *part_id, uint32 *parent_partid, bool32 *is_found)
{
    ...
    if (ud_row->xid.value == cursor->xid) {
        if (ud_row->ssn < cursor->ssn) {
            ...
            knl_panic_log(ud_row->scn <= cursor->query_scn,
                "ud_row's scn is more than query_scn, panic info: "
    ...
    btree_key_t *key, uint32 *part_id, uint32 *parent_partid, bool32 *is_found)
{
    for (;;) {
        ...
        if (btree_check_visible_with_udss(session, cursor, btree, key, part_id, parent_partid, is_found) !=
            GS_SUCCESS) {
            return GS_ERROR;
        }
        ...
        if (cursor->snapshot.is_owscn) {
            tx_record_sql(session);
            GS_LOG_RUN_ERR("snapshot too old, detail: snapshot owscn %llu, query scn %llu", cursor->snapshot.scn,
                cursor->query_scn);
            GS_THROW_ERROR(ERR_SNAPSHOT_TOO_OLD);
            return GS_ERROR;
        }
...
static status_t btree_check_visible(knl_session_t *session, knl_cursor_t *cursor, btree_dir_t *dir, btree_key_t *key,
    bool32 *is_found)
{
    btree_t *btree;
    btree_page_t *page;
    ...
    if (txn_info.status == (uint8)XACT_END) {
        ...
        if (txn_info.is_owscn) {
            tx_record_sql(session);
            GS_LOG_RUN_ERR("snapshot too old, detail: key owscn %llu, query scn %llu", txn_info.scn, cursor->query_scn);
            GS_THROW_ERROR(ERR_SNAPSHOT_TOO_OLD);
            return GS_ERROR;
        }
...
static status_t btree_fetch_key_asc(knl_session_t *session, knl_cursor_t *cursor, bool32 *is_found)
{
    ...
    btree_page_t *page =
        (cursor->key_loc.page_cache == LOCAL_PAGE_CACHE) ? (btree_page_t *)cursor->page_buf : BTREE_CURR_PAGE;
    index_t *index = (index_t *)cursor->index;
    ...
    *is_found = GS_FALSE;
    ...
    while (cursor->key_loc.slot < page->keys) {
        ...
        if (!is_equal &&
            (cursor->key_loc.slot_end == INVALID_SLOT || cursor->key_loc.slot_end < cursor->key_loc.slot)) {
...
void btree_get_end_slot(knl_session_t *session, knl_cursor_t *cursor)
{
    ...
    index_t *index = (index_t *)cursor->index;
    knl_scan_key_t *end_key = cursor->index_dsc ? &range->l_key : (range->is_equal ? &range->l_key : &range->r_key);
    btree_dir_t *dir = NULL;
    btree_key_t *key = NULL;
    uint16 slot;
...
static status_t btree_locate_with_find(knl_session_t *session, knl_cursor_t *cursor, knl_scan_key_t *scan_key)
{
    ...
    search_info.seg_scn = (IS_PART_INDEX((cursor)->index) ? ((index_part_t *)(cursor)->index_part)->desc.seg_scn :
                                                            ((index_t *)(cursor)->index)->desc.seg_scn);
...
static status_t btree_enter_next_page(knl_session_t *session, knl_cursor_t *cursor, page_id_t *next_page,
    bool32 *check_next)
{
...
static status_t btree_locate_curr_page(knl_session_t *session, knl_cursor_t *cursor, knl_scan_key_t *scan_key)
{
    ...
    if (page->head.lsn == cursor->key_loc.lsn &&
        ...
        if (cursor->key_loc.slot >= page->keys) {
            knl_panic_log(0,
                "key's slot is bigger than page keys, panic info: slot %u page %u-%u type %u keys %u "
                "table %s",
                cursor->key_loc.slot, cursor->rowid.file, cursor->rowid.page, page->head.type, page->keys,
    ...
    for (i = 0; i < page->itls; i++) {
        ...
        if (itl->xid.value == session->rm->xid.value) {
            ...
            knl_panic_log(txn_info.status == (uint8)XACT_END,
                "current txn status is abnormal, panic info: page %u-%u "
                "type %u status %u",
                page_id.file, page_id.page, page->head.type, txn_info.status);
            itl->is_active = GS_FALSE;
...
static void btree_cleanout_itls(knl_session_t *session, knl_cursor_t *cursor, btree_page_t *page, bool32 *changed)
{
    itl_t *itl = NULL;
    txn_info_t txn_info;
    rd_btree_clean_itl_t redo;
...
void btree_init_match_cond(knl_cursor_t *cursor, bool32 is_pcr)
{
    ...
    if (cursor->scan_range.is_equal || cursor->skip_index_match || index->desc.column_count == 1 ||
        cursor->index_paral) {
    ...
    for (i = 0; i < index->desc.column_count - 1; i++) {
        if (l_key->flags[i] != SCAN_KEY_NORMAL || r_key->flags[i] != SCAN_KEY_NORMAL) {
            break;
        }

        column = dc_get_column(index->entity, index->desc.columns[i]);
        if (btree_cmp_column_data((void *)(l_key->buf + l_key->offsets[i]), (void *)(r_key->buf + r_key->offsets[i]),
            column->datatype, &offset, is_pcr) != 0) {
            break;
        }
    }
...
static void btree_init_fetch(knl_session_t *session, knl_cursor_t *cursor)
{
    ...
    if (SECUREC_UNLIKELY(cursor->index_ss)) {
        ...
        knl_panic_log(((index_t *)cursor->index)->desc.column_count > BTREE_MIN_SKIP_COLUMNS,
            "the column count is "
            "abnormal, panic info: page %u-%u type %u table %s index %s column count %u",
            ...
            ((index_t *)cursor->index)->desc.column_count);
        ...
        if (IS_INDEX_ONLY_SCAN(cursor)) {
            if (cursor->index_prefetch_row) {
                if (buf_read_page_asynch(session, GET_ROWID_PAGE(cursor->rowid)) != GS_SUCCESS) {
                    GS_LOG_DEBUG_WAR("failed to prefetch page file : %u , page: %llu", (uint32)cursor->rowid.file,
                        (uint64)cursor->rowid.page);
                }
            }
...
static void btree_get_sub_r_key(knl_session_t *session, index_t *index, btree_key_t *key, knl_scan_range_t *range,
    bool8 is_dsc)
{
    page_id_t page_id;
    btree_page_t *page = NULL;
