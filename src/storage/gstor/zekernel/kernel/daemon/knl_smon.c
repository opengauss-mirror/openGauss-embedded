...
void smon_detect_dead_lock(knl_session_t *session, uint8 *wait_marks, uint16 id, bool32 record_sql)
{
    ...
    for (;;) {
        if (wait_sid[id] != knl_get_rm_sid(session, current->wrmid) || curr_lsn[id] != current->curr_lsn) {
            CM_RESTORE_STACK(session->stack);
            return;
        }
        ...
        if (snapshot.rmid != current->wrmid || snapshot.xnum != current->wxid.xnum ||
        ...
        if (record_sql) {
            GS_LOG_TRACE("session id: %u, wait session: %u, wait rowid: %u-%u-%u", current->id,
                knl_get_rm_sid(session, current->wrmid), current->wrid.file, current->wrid.page, current->wrid.slot);
    ...
    for (i = GS_SYS_SESSIONS; i < GS_MAX_SESSIONS; i++) {
        ...
        if (se->wrmid != GS_INVALID_ID16 && w_marks[i] != 1) {
            ...
            if (smon_check_itl_waits(session, se, GS_FALSE)) {
                ...
                GS_LOG_RUN_ERR("smon found itl deadlock in session(%u), page_id : %u-%u", se->id, se->wpid.file,
                    se->wpid.page);
...
static void smon_check_undo_usage(knl_session_t *session, space_t *space, uint64 *used_pages,
    uint32 *usage_alarm_threshold, warn_name_t *warn_name)
{
    ...
    warn_name_t warn_name)
{
    ...
    if (session->kernel->is_coordinator) {
        GS_LOG_ALARM_CN(warn_name, "'space-name':'%s', 'alarm-threshold':'%d'}", space->ctrl->name,
            usage_alarm_threshold);
    ...
    warn_name_t warn_name)
{
#ifdef Z_SHARDING
    if (session->kernel->is_coordinator) {
        GS_LOG_ALARM_RECOVER_CN(warn_name, "'space-name':'%s', 'alarm-threshold':'%d'}", space->ctrl->name,
            usage_alarm_threshold);
    ...
    GS_LOG_ALARM_RECOVER(warn_name, "'space-name':'%s', 'alarm-threshold':'%d'}", space->ctrl->name,
        usage_alarm_threshold);
    space->alarm_enabled = GS_TRUE;
}
...
void smon_proc(thread_t *thread)
{
    ...
    while (!thread->closed) {
        if (session->kernel->attr.smon_loop_in_ignore_mode && count != 0 )  
        { // 20241101 吴锦锋  让数据库启动时进入忽略受损undo shrink相关文件的模式
            session->kernel->attr.smon_loop_in_ignore_mode = GS_FALSE;
        }
...
static bool32 smon_push_itl(knl_session_t *start_session, itl_t *item, knl_dlock_stack_t *stack_ptr, uint8 *w_marks)
{
...
static bool32 smon_push_pcr_itl(knl_session_t *session, pcr_itl_t *itl, knl_dlock_stack_t *dlock_stack, uint8 *w_marks)
{
    knl_session_t *next_session = NULL;
...
static bool32 smon_check_itl_waits(knl_session_t *session, knl_session_t *start_session, bool32 record_sql)
{
    ...
    while (!dlock_is_empty(stack_ptr)) {
        ...
        if (curr_session->status == SESSION_INACTIVE || start_session->status == SESSION_INACTIVE || curr_rm == NULL ||
            start_rm == NULL) {
            cm_pop(session->stack);
            return GS_FALSE;
        }
        ...
        if (curr_wxid.value == GS_INVALID_ID64 && IS_INVALID_PAGID(curr_wpid)) {
            ...
            if (IS_SAME_PAGID(curr_wpid, start_wpid)) {
                if (record_sql) {
                    GS_LOG_TRACE("session id: %u, wait page_id: %u-%u", curr_session->id, curr_wpid.file,
                        curr_wpid.page);
                    smon_record_deadlock(session, curr_session);
                }
                continue;
...
static bool32 smon_push_lock(knl_session_t *session, uint8 *w_marks, knl_dlock_stack_t *stack_lock, schema_lock_t *lock)
{
    knl_session_t *lock_session = NULL;
    uint16 sid;
    uint32 count = 0;
...
static bool32 smon_check_lock_waits(knl_session_t *session, knl_session_t *se, bool32 record_sql)
{
    ...
    while (!dlock_is_empty(stack_lock)) {
        ...
        if ((curr_session->wait.event == ENQ_TX_TABLE_S || curr_session->wait.event == ENQ_TX_TABLE_X) &&
            ...
            if (record_sql) {
                GS_LOG_TRACE("session id: %u, wait object id: %u-%u", curr_session->id, curr_session->wtid.uid,
                    curr_session->wtid.oid);
                smon_record_deadlock(session, curr_session);
            }
            ...
            if (record_sql) {
                GS_LOG_TRACE("session id: %u, wait session id: %u", curr_session->id,
                    knl_get_rm_sid(session, curr_wrmid));
                smon_record_deadlock(session, curr_session);
            }
