                                  ...
                                  uint16 old_rmid, knl_xa_xid_t *xa_xid)
{
    ...
    if (!g_knl_callback.add_xa_xid(cc_instance, xa_xid, rm->id, XA_PHASE1)) {
...
static inline void xa_recover_reset(knl_session_t *session, knl_rm_t *rm, uint16 old_rmid, uint16 rmid)
{
    ...
    g_knl_callback.release_rm(cc_instance, rmid);
}
...
status_t xa_recover(knl_session_t *session, tx_item_t *item, txn_t *txn, uint32 item_id)
{
    ...
    if (g_knl_callback.alloc_rm(cc_instance, &rmid) != GS_SUCCESS) {
...
status_t knl_xa_start(knl_handle_t session, xa_xid_t *xa_xid, uint64 timeout, uint64 flags)
{
    ...
    knl_rm_t *rm = se->rm;
    instance_t *cc_instance = se->kernel->server;
    ...
    if (!g_knl_callback.add_xa_xid(cc_instance, &rm->xa_xid, rm->id, XA_START)) {
        xa_reset_rm(rm);
        GS_THROW_ERROR(ERR_XA_DUPLICATE_XID);
...
static status_t convert_to_xa(knl_session_t *session, xa_xid_t *xa_xid)
{
    ...
    rm->uid = session->uid;
    if (!g_knl_callback.add_xa_xid(cc_instance, &rm->xa_xid, rm->id, XA_PHASE1)) {
        xa_reset_rm(rm);
        GS_THROW_ERROR(ERR_XA_DUPLICATE_XID);
        return GS_ERROR;
    }
...
static inline void xa_delete_xa_xid(knl_session_t *session, knl_xa_xid_t *xa_xid)
{
    instance_t *cc_instance = session->kernel->server;

    if (knl_xa_xid_valid(xa_xid)) {
        g_knl_callback.delete_xa_xid(cc_instance, xa_xid);
    }
}
...
static status_t xa_phase2(knl_session_t *session, xa_xid_t *xa_xid, bool32 commit, uint64 flags, knl_scn_t scn)
{
    ...
    if (from_suspend) {
        xa_delete_xa_xid(session, &rm->xa_xid);
        g_knl_callback.release_rm(cc_instance, org_rmid);
        return GS_SUCCESS;
    }
    ...
    if (xa_phase2_delete_ptrans(session, &curr_xa_xid, &ltid, rm->xa_rowid) != GS_SUCCESS) {
        ...
        xa_delete_xa_xid(session, &rm->xa_xid);
        g_knl_callback.release_rm(cc_instance, xa_rmid);
...
status_t knl_xa_status(knl_handle_t session, xa_xid_t *xa_xid, xact_status_t *status)
{
    ...
    rmid = g_knl_callback.get_xa_xid(cc_instance, &knl_xa_xid);
