...
void mtrl_init_context(mtrl_context_t *ctx, handle_t sess)
{
    ...
    g_knl_callback.init_vmc((knl_session_t *)sess, (handle_t)ctx);
...
static status_t mtrl_insert_into_segment(mtrl_context_t *ctx, mtrl_segment_t *segment, const char *row, uint16 row_size,
    bool32 is_part, mtrl_rowid_t *rid)
{
    mtrl_page_t *page;
    page = (mtrl_page_t *)segment->curr_page->data;
...
static status_t mtrl_merge_1cursor(mtrl_context_t *ctx, mtrl_segment_t *segment, mtrl_sort_cursor_t *cursor,
    mtrl_rowid_t *srid)
{
    mtrl_part_t part;
    mtrl_rowid_t rid;
...
static status_t mtrl_init_sort_cursor(mtrl_context_t *ctx, mtrl_segment_t *segment, mtrl_rowid_t *rid, uint32 level,
    mtrl_sort_cursor_t *cursor, vm_page_t *page)
{
...
status_t mtrl_open_sort_cursor(mtrl_context_t *ctx, mtrl_segment_t *segment, mtrl_rowid_t *rid, uint32 level,
    mtrl_sort_cursor_t *cursor)
{
    vm_page_t *page = NULL;
...
static status_t mtrl_merge_sort(mtrl_context_t *ctx, mtrl_segment_t *curr_seg, mtrl_segment_t *segment, uint32 level,
    mtrl_rowid_t *start_rid, bool32 *finished)
{
    uint32 parts;
...
status_t mtrl_sort_init(knl_session_t *session, index_t *index, knl_part_locate_t part_loc, mtrl_sort_ctrl_t *sort_ctrl)
{
...
status_t mtrl_fetch_savepoint(mtrl_context_t *ctx, uint32 sort_seg, mtrl_savepoint_t *savepoint, mtrl_cursor_t *cursor,
    row_addr_t *rows, uint32 count)
{
...
static status_t mtrl_ensure_page_size(mtrl_context_t *ctx, mtrl_segment_t *segment, uint16 size, bool32 need_close)
{
    mtrl_page_t *page = (mtrl_page_t *)segment->curr_page->data;
...
status_t mtrl_win_aggr_alloc(mtrl_context_t *ctx, uint32 seg_id, void **var, uint32 var_size, mtrl_rowid_t *rid,
    bool32 need_close)
{
    mtrl_page_t *page = NULL;
    mtrl_segment_t *segment = ctx->segments[seg_id];
