...
static void ctrl_restore_core_ctrl(knl_session_t *session, page_head_t *page, int handle)
{
    ...
    int64 offset =
        sizeof(page_head_t) + sizeof(datafile_header_t) + sizeof(datafile_ctrl_bk_t) + sizeof(space_ctrl_bk_t);
    ...
    log_file_ctrl_bk_t *logfile_ctrl_bk, bool32 need_restore_name)
{
    if (need_restore_name) {
        errno_t ret =
            memcpy_sp(logfile_ctrl->name, GS_FILE_NAME_BUFFER_SIZE, logfile_ctrl_bk->name, GS_FILE_NAME_BUFFER_SIZE);
        knl_securec_check(ret);
    }
...
static status_t ctrl_rebuild_parse_datafile(knl_session_t *session, knl_device_def_t *device)
{
    ...
    datafile_ctrl_bk_t *datafile_ctrl_bk =
        (datafile_ctrl_bk_t *)((char *)page + sizeof(page_head_t) + sizeof(datafile_header_t));
...
static status_t ctrl_recreate_ctrl_files(knl_session_t *session)
{
    ...
    while (file_name.len > 0) {
        ...
        if (cm_build_device(ctrlfile->name, ctrlfile->type, kernel->attr.xpurpose_buf, GS_XPURPOSE_BUFFER_SIZE,
            (int64)ctrlfile->blocks * ctrlfile->block_size, knl_io_flag(session), GS_FALSE,
            &ctrlfile->handle) != GS_SUCCESS) {
            return GS_ERROR;
        }
...
status_t ctrl_backup_static_core_items(knl_session_t *session, static_core_ctrl_items_t *items)
{
    ...
    errno_t ret =
        memcpy_sp((char *)page + offset, sizeof(static_core_ctrl_items_t), items, sizeof(static_core_ctrl_items_t));
    knl_securec_check(ret);
...
status_t ctrl_backup_log_ctrl(knl_session_t *session, uint32 id)
{
    ...
    if (cm_open_device(log_file->ctrl->name, log_file->ctrl->type, knl_redo_io_flag(session), &log_file->handle) !=
        GS_SUCCESS) {
        return GS_ERROR;
    }
...
status_t ctrl_backup_ctrl_info(knl_session_t *session)
{
    ...
    static_core_ctrl_items_t *core_ctrl_backup =
        (static_core_ctrl_items_t *)((char *)&kernel->db.ctrl.core + OFFSET_OF(core_ctrl_t, name));
    if (ctrl_backup_static_core_items(session, core_ctrl_backup) != GS_SUCCESS) {
        return GS_ERROR;
    }

    /* backup system table entries of core ctrl */
    sys_table_entries_t *system_entry =
        (sys_table_entries_t *)((char *)&db->ctrl.core + OFFSET_OF(core_ctrl_t, sys_table_entry));
