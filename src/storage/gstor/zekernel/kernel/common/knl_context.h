...
typedef struct st_knl_attr {
    ...
    bool32 synchronous_commit;
    bool32 enforced_ignore_all_redo_logs; // 20241210吴锦锋：增加读取是否忽略损坏文件的配置
    bool32 ignore_corrupted_logs;    // 20241022吴锦锋：增加读取是否忽略损坏文件的配置
...
#ifdef _BACKUP
    bak_context_t backup_ctx; // _BACKUP ??
#endif
