...
static status_t dbwr_init(knl_session_t *session)
{
    ...
    for (uint32 i = 0; i < ctx->dbwr_count; i++) {
        ...
        dbwr->fsync_time = KNL_NOW(dbwr->session);
...
static void ckpt_do_timed_task(knl_session_t *session, ckpt_context_t *ctx, date_t *clean_time, date_t *ckpt_time)
{
    ...
    if (ctx->queue.count >= attr->ckpt_interval ||
        ...
        if (ckpt_assign_timed_task(session, ctx, CKPT_TIMED_INC) == GS_SUCCESS) {
            ...
            GS_LOG_DEBUG_INF("line=%d,attr->ckpt_timeout = %d, ctx->queue.count =%d, ctx.group.count=%d \n", __LINE__,
                attr->ckpt_timeout, ctx->queue.count, ctx->group.count);
            ckpt_inc_checkpoint(session);
            *ckpt_time = KNL_NOW(session);
            ctx->timed_task = CKPT_MODE_IDLE;
        ...
        if (ckpt_assign_timed_task(session, ctx, CKPT_TIMED_CLEAN) == GS_SUCCESS) {
            date_t task_begin = KNL_NOW(session);
            GS_LOG_DEBUG_INF("line=%d,attr->page_clean_period = %d, *clean_time =%lld \n", __LINE__,
                attr->page_clean_period, *clean_time);
            ckpt_page_clean(session);
            *clean_time = KNL_NOW(session);
            ctx->timed_task = CKPT_MODE_IDLE;
    ...
    if (attr->page_clean_period != 0 && KNL_NOW(session) - (*clean_time) >= (date_t)attr->page_clean_period) {
        if (ckpt_assign_timed_task(session, ctx, CKPT_TIMED_CLEAN) == GS_SUCCESS) {
            date_t task_begin = KNL_NOW(session);
            ckpt_page_clean(session);
...
static void ckpt_copy_item(knl_session_t *session, buf_ctrl_t *ctrl, buf_ctrl_t *to_flush_ctrl)
{
    ...
    knl_panic_log(IS_SAME_PAGID(to_flush_ctrl->page_id, AS_PAGID(to_flush_ctrl->page->id)),
        ...
        AS_PAGID(to_flush_ctrl->page->id).file, AS_PAGID(to_flush_ctrl->page->id).page, to_flush_ctrl->page->type);
    ...
    ret = memcpy_sp(ctx->group.buf + DEFAULT_PAGE_SIZE * ctx->group.count, DEFAULT_PAGE_SIZE, to_flush_ctrl->page,
        DEFAULT_PAGE_SIZE);
    knl_securec_check(ret);
#ifdef _REPLICATION
...
status_t ckpt_prepare_pages(knl_session_t *session, ckpt_context_t *ctx)
{
    ...
    while (ctrl != NULL) {
        ...
        if (ctx->trigger_task != CKPT_TRIGGER_FULL &&
            log_cmp_point(&ctrl->trunc_point, &session->kernel->redo_ctx.curr_point) > 0) {
            break;
        }
    ...
    const char *zbuf)
{
    ...
    do {
        ...
        knl_panic_log(IS_SAME_PAGID(ctrl->page_id, AS_PAGID(page->id)),
            ...
            ctrl->page_id.file, ctrl->page_id.page, page->type, AS_PAGID(page->id).file, AS_PAGID(page->id).page);
...
static status_t ckpt_double_write(knl_session_t *session, ckpt_context_t *ctx)
{
    ...
    if (spc_write_datafile(session, df, &ctx->dw_file, offset, ctx->group.buf, ctx->group.count * DEFAULT_PAGE_SIZE) !=
        GS_SUCCESS) {
...
{
    core_ctrl_t *core = &session->kernel->db.ctrl.core;
    // GS_LOG_RUN_INF("ckpt_flush_prepare trigger_task = %d \n", ctx->trigger_task);
    if (ctx->trigger_task == CKPT_TRIGGER_FULL) {
        if (log_flush(session, &ctx->lrp_point, &ctx->lrp_scn) != GS_SUCCESS) {
            return GS_ERROR;
        }
    } else {
        cm_spin_lock(&session->kernel->redo_ctx.flush_lock, NULL);
        if (ctx->lrp_scn < session->kernel->redo_ctx.curr_scn) {
            ctx->lrp_scn = session->kernel->redo_ctx.curr_scn;
        }

        if (log_cmp_point(&ctx->lrp_point, &session->kernel->redo_ctx.curr_point) < 0)
        {
            ctx->lrp_point = session->kernel->redo_ctx.curr_point;
        }
    ...
    *saved_cnt = 0;
    ...
    for (uint16 i = begin; i < begin + sequent_cnt; i++) {
        ...
        knl_panic_log(IS_SAME_PAGID(ctrl->page_id, AS_PAGID(page->id)),
            ...
            ctrl->page_id.file, ctrl->page_id.page, ctrl->page->type, AS_PAGID(page->id).file, AS_PAGID(page->id).page,
            page->type);
...
static status_t ckpt_read_doublewrite_pages(knl_session_t *session)
{
    ...
    /* DEFAULT_PAGE_SIZE is 8192, ctx->group.count <= GS_CKPT_GROUP_SIZE(4096), can not cross bounds */
    if (spc_read_datafile(session, df, &ctx->dw_file, offset, ctx->group.buf, ctx->group.count * DEFAULT_PAGE_SIZE) !=
        GS_SUCCESS) {
...
status_t ckpt_clean_prepare_compress(knl_session_t *session, ckpt_context_t *ctx, buf_ctrl_t *head)
{
    ...
    for (i = 0; i < PAGE_GROUP_COUNT; i++) {
        errno_t ret = memcpy_sp(ctx->group.buf + DEFAULT_PAGE_SIZE * ctx->group.count, DEFAULT_PAGE_SIZE,
            ctrl_group[i]->page, DEFAULT_PAGE_SIZE);
        knl_securec_check(ret);
...
status_t ckpt_clean_prepare_normal(knl_session_t *session, ckpt_context_t *ctx, buf_ctrl_t *shift)
{
    ...
    errno_t ret = memcpy_sp(ctx->group.buf + DEFAULT_PAGE_SIZE * ctx->group.count, DEFAULT_PAGE_SIZE, shift->page,
        DEFAULT_PAGE_SIZE);
    knl_securec_check(ret);
...
static status_t ckpt_clean_single_set(knl_session_t *session, ckpt_context_t *ckpt_ctx, buf_set_t *set)
{
    ...
    for (;;) {
        ...
        // GS_LOG_RUN_INF("ckpt_ctx->group.count=%d,  page_list=%d,clean_cnt=%d \n", ckpt_ctx->group.count,
        // page_list.count, clean_cnt);
...
static void ckpt_page_clean(knl_session_t *session)
{
    ...
    for (uint32 i = 0; i < buf_ctx->buf_set_count; i++) {
        if (ckpt_clean_single_set(session, ckpt_ctx, &buf_ctx->buf_set[i]) != GS_SUCCESS) {
            KNL_SESSION_CLEAR_THREADID(session);
            GS_LOG_RUN_INF("[CKPT] ABORT INFO: flush page failed when clean dirty page \n");
