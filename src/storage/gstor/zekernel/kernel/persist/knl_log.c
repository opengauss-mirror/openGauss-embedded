...
static inline void log_buf_init(knl_session_t *session)
{
     ...
     */
    uint32 buffer_size =
        (ctx->buf_size / 2 - sizeof(log_batch_t) - sizeof(log_batch_tail_t) - sid_array_size) / ctx->buf_count -
        sizeof(log_part_t);
...
status_t log_switch_file(knl_session_t *session)
{
    ...
    knl_panic_log((next != ctx->active_file),
        "failed to switch log file, current file is %d, "
        "active file is %d, log free size is %llu",
        ctx->curr_file, ctx->active_file, ctx->free_size);
...
void log_flush_init(knl_session_t *session, uint32 batch_size)
{
    ...
    knl_panic_log(log_file_freesize(file) >= batch_size,
        "the log_file_freesize is smaller than batch_size, "
        "panic info: freesize %llu batch_size %u",
        log_file_freesize(file), batch_size);
}
...
status_t log_flush_to_disk(knl_session_t *session, log_context_t *ctx, log_batch_t *batch)
{
    ...
    if (cm_write_device(file->ctrl->type, file->handle, file->head.write_pos, batch, batch->space_size) != GS_SUCCESS) {
        GS_LOG_ALARM(WARN_FLUSHREDO, "'file-name':'%s'}", file->ctrl->name);
...
static inline void log_assemble_buffer(log_context_t *ctx, log_buffer_t *buf)
{
    ...
    *(log_part_t *)(ctx->logwr_buf + ctx->logwr_buf_pos) = part;
    ...
    errno_t ret = memcpy_sp(ctx->logwr_buf + ctx->logwr_buf_pos, ctx->logwr_buf_size - ctx->logwr_buf_pos, buf->addr,
        buf->write_pos);
    knl_securec_check(ret);
...
status_t log_decrypt(knl_session_t *session, log_batch_t *batch, char *plain_buf, uint32 plain_len)
{
    ...
    knl_panic_log(cipher_len - cipher_ctrl.cipher_expanded_size == plain_len,
        "the cipher_len is abnormal, "
        "panic info: cipher_len %u cipher_expanded_size %u plain_len %u",
        cipher_len, cipher_ctrl.cipher_expanded_size, plain_len);
...
static void log_encrypt(knl_session_t *session, log_context_t *ctx)
{
    ...
    status_t status =
        cm_kmc_encrypt(GS_KMC_KERNEL_DOMAIN, KMC_DEFAULT_ENCRYPT, plain_buf, plain_len, cipher_buf, &cipher_len);
    if (status != GS_SUCCESS) {
    ...
    knl_panic_log(sizeof(log_batch_t) + sizeof(cipher_ctrl_t) + cipher_len + sizeof(log_batch_tail_t) <=
        ctx->logwr_buf_size,
        "the plain_len is abnormal, panic info: plain_len %u logwr_buf_size %u", cipher_len, ctx->logwr_buf_size);
    cipher_ctrl_t *cipher_ctrl = (cipher_ctrl_t *)((char *)batch + sizeof(log_batch_t));
    ...
    errno_t ret = memcpy_sp((char *)batch + cipher_ctrl->offset, ctx->logwr_buf_size - cipher_ctrl->offset, cipher_buf,
        cipher_len);
    knl_securec_check(ret);
...
void log_proc(thread_t *thread)
{
    ...
    while (!thread->closed) {
        ...
        if ((g_timer()->now - flush_time) < LOG_FLUSH_INTERVAL && !flush_needed) {
            cm_sleep(1);
            continue;
        }
...
static status_t log_commit_flush(knl_session_t *session)
{
    ...
    {
        GS_LOG_RUN_INF("session->is_timescale=%d, prev=%d,session->is_insert = %d \n",
                       session->is_timescale, session->rm->prev, session->is_insert);
        if (log_flush(session, NULL, NULL) != GS_SUCCESS)
        {
            cm_spin_unlock(&ctx->commit_lock);
...
static void log_write(knl_session_t *session)
{
    ...
    log_group_t *group = (log_group_t *)session->log_buf;
    uint32 log_size = (!session->rm->need_copy_logic_log) ? group->size : (group->size + session->rm->logic_log_size);
    ...
    uint32 total_size = (!session->rm->need_copy_logic_log) ? group->size : (group->size + session->rm->logic_log_size);
...
bool32 log_can_recycle(knl_session_t *session, log_file_t *file, arch_log_id_t *last_arch_log)
{
    ...
    if (is_archive) {
        ...
        // 根据文件最新的scn获取时间(file->head.last)
        if (file->head.asn != GS_INVALID_ASN) {
            uint64 curr_scn = DB_CURR_SCN(session);
            timeval_t curr_tv;
            timeval_t file_tv;
            KNL_SCN_TO_TIME(curr_scn, &curr_tv, DB_INIT_TIME(session));
            KNL_SCN_TO_TIME(file->head.last, &file_tv, DB_INIT_TIME(session));
            if (curr_tv.tv_sec - file_tv.tv_sec < session->kernel->attr.redo_save_time &&
                session->kernel->redo_ctx.free_size > LOG_KEEP_SIZE(session->kernel) * 10) {
                return GS_FALSE;
            }
        }
    }
...
void log_recycle_file(knl_session_t *session, log_point_t *point)
{
    ...
    GS_LOG_DEBUG_INF("try to recycle log file with last_arch_log [%u-%u] active[%d] file [%u-%u]", last_arch_log.rst_id,
        last_arch_log.asn, ctx->active_file, file->head.rst_id, file->head.asn);
    ...
    while (LOG_POINT_FILE_LT(ctx->files[file_id].head, *point) || !DB_IS_PRIMARY(&session->kernel->db)) {
        ...
        GS_LOG_RUN_INF("recycle log file[%u] [%u-%u] rcy_point [%u-%u]", file_id, file->head.rst_id, file->head.asn,
            point->rst_id, point->asn);
...
static inline bool32 log_fileid_asn_mismatch(log_context_t *ctx, uint16 spec_fileid, uint32 spec_asn, uint32 next)
{
    ...
    if (spec_asn == next_asn && next != spec_fileid) {
        ...
        GS_LOG_RUN_ERR("[LOG] asn %u located in different fileid %u/%u on peer node and local node, "
            "perhaps the add/drop logfile has not been replayed",
            spec_asn, spec_fileid, next);
        return GS_TRUE;
    }
...
status_t log_switch_logfile(knl_session_t *session, uint16 spec_file_id, uint32 spec_asn, callback_t *callback)
{
    ...
    if (file->head.write_pos == CM_CALC_ALIGN(sizeof(log_file_head_t), file->ctrl->block_size)) {
        ...
        need_skip = GS_TRUE;
        GS_LOG_RUN_INF("[LOG] Switch log, need to skip file %u asn %u state %d", log->curr_file, file->head.asn,
            file->ctrl->status);
    }
...
static status_t log_get_first_batch_lfn(knl_session_t *session, log_file_t *logfile, uint64 *first_batch_lfn)
{
    ...
    if (cm_read_device(logfile->ctrl->type, logfile->handle, log_head_size, log_buf.aligned_buf, (int32)size) !=
        GS_SUCCESS) {
...
status_t log_check_asn(knl_session_t *session, bool32 force_ignorlog)
{
    ...
    // 20241028  吴锦锋   发现诚迈宕机打不开文件可能的引发点在本函数，
    //                   调用本函数的上层报错信息为：ERROR MSG:Invalid operation, check log asn failed when open database
    if (session->kernel->attr.ignore_corrupted_logs)
    {
        force_ignorlog = GS_TRUE;
    }
    ...
    if (ctx->active_file == ctx->curr_file) {
        ...
        if (!log_current_asn_is_correct(session, logfile, &first_batch_lfn)) {
            GS_LOG_RUN_ERR("[LOG] check asn of redo log %s failed, logfile [%u-%u/%llu], "
                ...
                logfile->ctrl->name, logfile->head.rst_id, logfile->head.asn, logfile->head.write_pos, first_batch_lfn,
                (uint64)rcy_point.rst_id, rcy_point.asn, (uint64)rcy_point.lfn);
            return GS_ERROR;
        }
        return GS_SUCCESS;
    }
    ...
    if (logfile->head.asn != last_active_asn + 1) {
        GS_LOG_RUN_ERR("[LOG] redo log asn are not continuous, %s asn: %u, previous log asn: %u", logfile->ctrl->name,
            logfile->head.asn, last_active_asn);
        return GS_ERROR;
    }
...
#else
        if (first_batch && (batch->head.point.asn != head.asn || batch->head.point.rst_id != head.rst_id)) {
            *offset = CM_CALC_ALIGN(sizeof(log_file_head_t), (uint32)head.block_size);
            cm_close_file(handle);
            GS_LOG_RUN_INF("[LOG] no need to repair file offset for %s, "
                "batch rstid/asn [%u/%u], file head rstid/asn [%u/%u]",
        ...
        while (remain_size >= sizeof(log_batch_t)) {
            if (remain_size < batch->space_size || !rcy_validate_batch(batch, tail) ||
                ...
                GS_LOG_RUN_INF("[LOG] log %s [%u-%u] offset %llu invalid batch size %u "
                    ...
                    batch->head.point.rst_id, batch->head.point.asn, (uint64)batch->head.point.lfn, batch->raft_index,
                    *latest_lfn);
...
status_t log_repair_file_offset(knl_session_t *session, log_file_t *file)
{
    ...
    if (log_get_file_offset(session, file->ctrl->name, &log_buf, (uint64 *)&file->head.write_pos, &latest_lfn,
        &file->head.last) != GS_SUCCESS) {
