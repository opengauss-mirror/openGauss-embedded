...
bool32 arch_need_archive(arch_proc_context_t *proc_ctx, log_context_t *redo_ctx)
{
    ...
    if (((proc_ctx->last_archived_log.rst_id > file->head.rst_id ||
        ...
        DB_IS_PRIMARY(&proc_ctx->session->kernel->db)) ||
        file->ctrl->archived) {
    ...
    const char *src_name, const char *arch_file_name)
{
    ...
    if (cm_build_device(tmp_arch_file_name, logfile->ctrl->type, session->kernel->attr.xpurpose_buf,
        GS_XPURPOSE_BUFFER_SIZE, CM_CALC_ALIGN(sizeof(log_file_head_t), logfile->ctrl->block_size), O_DSYNC, GS_FALSE,
...
status_t arch_record_archinfo(knl_session_t *session, uint32 dest_pos, const char *file_name, log_file_head_t *log_head)
{
    ...
    GS_LOG_RUN_INF("[ARCH] Record archive log file %s for log [%u-%u] start %u end %u", arch_ctrl->name,
        log_head->rst_id, log_head->asn, ctrl->core.archived_start, ctrl->core.archived_end);
    cm_spin_unlock(&proc_ctx->record_lock);
    return GS_SUCCESS;
}
...
void arch_do_archive(knl_session_t *session, arch_proc_context_t *proc_ctx)
{
    ...
    if (arch_archive_file(session, proc_ctx->arch_buf, logfile->ctrl->name, arch_file_name, logfile) == GS_SUCCESS) {
        // Update last archived log file id
        proc_ctx->last_file_id = proc_ctx->next_file_id;
        GS_LOG_RUN_INF("[ARCH] Archive log file[%u], restlog id is %u, asn is %u to %s", proc_ctx->next_file_id,
            logfile->head.rst_id, logfile->head.asn, arch_file_name);
    ...
    log_point_t *backup_rcy, uint64 target_size, knl_alterdb_archivelog_t *def)
{
    ...
    for (uint32 i = 0; i < arch_num; i++) {
        ...
        if (!cm_file_exist(arch_ctrl->name)) {
            ...
            GS_LOG_RUN_INF("archive file %s is cleaned, resetlog %u asn %u force %u start %u end %u", arch_ctrl->name,
                arch_ctrl->rst_id, arch_ctrl->asn, def->force_delete, ctrl->core.archived_start,
                ctrl->core.archived_end);
        }
...
static status_t arch_clean_arch_files(knl_session_t *session, arch_proc_context_t *proc_ctx, log_point_t *min_rcy_point,
    knl_alterdb_archivelog_t *def, uint64 max_arch_size)
{
    ...
    if (ignore_standby && !LOG_POINT_FILE_EQUAL(local_rcy_point, *min_rcy_point) &&
        ...
        if ((uint64)proc_ctx->curr_arch_size > GS_HWM_ARCHIVE_FILES_SIZE(max_arch_size)) {
            GS_LOG_DEBUG_ERR("failed to clean archive logfile ignore standby, local rcy_point [%u-%u/%u/%llu], "
                ...
                local_rcy_point.rst_id, local_rcy_point.asn, local_rcy_point.block_id, (uint64)local_rcy_point.lfn,
                proc_ctx->curr_arch_size, GS_HWM_ARCHIVE_FILES_SIZE(max_arch_size));
        }
    }
...
void arch_auto_clean(arch_proc_context_t *proc_ctx)
{
    ...
    if (DB_STATUS(session) != DB_STATUS_OPEN || session->kernel->attr.max_arch_files_size == 0 ||
        (uint64)proc_ctx->curr_arch_size < session->kernel->attr.max_arch_files_size ) {
        return;
    }
...
static status_t arch_init_single_proc_ctx(arch_context_t *arch_ctx, uint32 dest_id, knl_session_t *session)
{
    ...
    if (proc_ctx->arch_dest[0] != '\0' && proc_ctx->dest_status == STATE_ENABLE) {
        if (proc_ctx->arch_id > ARCH_DEFAULT_DEST) {
            GS_LOG_RUN_ERR("[ARCH] Multiple ARCHIVE_DEST not supported. ARCHIVE_DEST_%u is set.", proc_ctx->arch_id);
...
status_t arch_start(knl_session_t *session)
{
    ...
    for (i = 0; i < GS_MAX_ARCH_DEST; i++) {
        ...
        if (proc_ctx != NULL && proc_ctx->arch_dest[0] != '\0' && proc_ctx->enabled) {
            ...
            GS_LOG_RUN_INF("[ARCH] Start ARCH thread for ARCHIVE_DEST_%d[%s]", proc_ctx->arch_id, proc_ctx->arch_dest);
        }
    }
...
void arch_close(knl_session_t *session)
{
    ...
    for (i = 0; i < GS_MAX_ARCH_DEST; i++) {
        ...
        if (proc_ctx->arch_dest[0] != '\0' && proc_ctx->enabled) {
            ...
            CM_FREE_PTR(proc_ctx->arch_buf);
            GS_LOG_RUN_INF("[ARCH] Close ARCH thread for ARCHIVE_DEST_%d[%s]", proc_ctx->arch_id, proc_ctx->arch_dest);
        }
    }
}
...
static status_t arch_check_format(char *value, char *cur_pos, bool32 *has_asn, bool32 *has_thread_id,
    bool32 *has_rst_id)
{
    switch (*cur_pos) {
        case 's':
        case 'S': {
            if (*has_asn) {
                GS_THROW_ERROR_EX(ERR_INVALID_ARCHIVE_PARAMETER, "'%s' has repeated format '%c' for ARCHIVE_FORMAT",
                    value, *cur_pos);
                return GS_ERROR;
            }
        ...
        default: {
            // Invalid format.
            GS_THROW_ERROR_EX(ERR_INVALID_ARCHIVE_PARAMETER, "'%s' has wrong format '%c' for ARCHIVE_FORMAT", value,
                *cur_pos);
            return GS_ERROR;
        }
    }
...
void arch_get_dest_path(knl_session_t *session, uint32 id, arch_attr_t *arch_attr, char *path, uint32 path_size)
{
    ...
    if (id == 0) {
        ...
        print_num = sprintf_s(path, path_size, "[%s:%u] %s", arch_attr->service.host, arch_attr->service.port,
            proc_ctx->arch_dest);
        knl_securec_check_ss(print_num);
    } else {
        path[0] = '\0';
...
{
    if (tmp_attr->role_valid != arch_attr->role_valid && tmp_attr->role_valid != VALID_FOR_ALL_ROLES &&
...
status_t arch_check_dest_service(void *attr, arch_attr_t *arch_attr, uint32 slot)
{
    ...
    for (i = 1; i < GS_MAX_ARCH_DEST; i++) {
        ...
        if (strcmp(tmp_attr->service.host, arch_attr->service.host) == 0 &&
            tmp_attr->service.port == arch_attr->service.port && arch_dest_both_valid(tmp_attr, arch_attr)) {
...
status_t arch_process_existed_archfile(knl_session_t *session, const char *arch_name, log_file_head_t head,
    bool32 *ignore_data)
{
    database_ctrl_t *ctrl = &session->kernel->db.ctrl;
    arch_proc_context_t *proc_ctx = &session->kernel->arch_ctx.arch_proc[0];
...
static status_t log_try_get_file_offset(knl_session_t *session, log_file_t *logfile, aligned_buf_t *buf)
{
    ...
    if (cm_read_device(logfile->ctrl->type, logfile->handle, logfile->head.write_pos, buf->aligned_buf, size) !=
        GS_SUCCESS) {
    ...
    uint64 latest_lfn;
    if (log_get_file_offset(session, logfile->ctrl->name, buf, (uint64 *)&logfile->head.write_pos, &latest_lfn,
        &logfile->head.last) != GS_SUCCESS) {
        return GS_ERROR;
    }
    ...
    bool32 *is_continue)
{
    ...
    if (cm_file_exist(arch_file_name)) {
        ...
        if (ignore_data) {
            GS_LOG_RUN_INF("[ARCH] skip archive log file %s to %s which already exists", logfile->ctrl->name,
                arch_file_name);
            if (arch_archive_log_recorded(session, logfile->head.rst_id, logfile->head.asn, ARCH_DEFAULT_DEST)) {
                *is_continue = GS_TRUE;
                return GS_SUCCESS;
