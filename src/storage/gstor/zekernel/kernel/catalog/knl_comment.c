...
static status_t db_insert_syscomment(knl_session_t *session, knl_cursor_t *cursor, knl_comment_def_t *def)
{
    uint32 max_size;
    row_assist_t ra;
    table_t *table = NULL;
...
status_t db_delete_comment(knl_session_t *session, knl_comment_def_t *def)
{
    ...
    switch (def->type) {
        case COMMENT_ON_TABLE:
            ...
            knl_set_scan_key(INDEX_DESC(cursor->index), &cursor->scan_range.l_key, GS_TYPE_INTEGER, &def->uid,
                sizeof(uint32), COMMENT_USER_COLUMN_ID);
            knl_set_scan_key(INDEX_DESC(cursor->index), &cursor->scan_range.l_key, GS_TYPE_INTEGER, &def->id,
                sizeof(uint32), COMMENT_TABLE_COLUMN_ID);
            knl_set_key_flag(&cursor->scan_range.l_key, SCAN_KEY_LEFT_INFINITE, COMMENT_COLUMN_COLUMN_ID);

            knl_set_scan_key(INDEX_DESC(cursor->index), &cursor->scan_range.r_key, GS_TYPE_INTEGER, &def->uid,
                sizeof(uint32), COMMENT_USER_COLUMN_ID);
            knl_set_scan_key(INDEX_DESC(cursor->index), &cursor->scan_range.r_key, GS_TYPE_INTEGER, &def->id,
                sizeof(uint32), COMMENT_TABLE_COLUMN_ID);
