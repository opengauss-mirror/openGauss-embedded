    ...
    uint32 privid, uint32 grantee_id, uint32 grantee_type)
{
    ...
    if (grant_uid < GS_MAX_USERS && ctx->users[grant_uid] != NULL &&
        ...
        cm_list_for_each_safe(item, temp, &ctx->users[grant_uid]->grant_obj_privs)
        {
            ...
            if (grant_item->grantee_id == grantee_id && grant_item->priv_id == privid &&
                grant_item->grantee_type == grantee_type && grant_item->priv_item.objowner == grant_uid &&
...
void dc_revoke_objpriv_from_user_by_id(dc_context_t *ctx, dc_user_t *user, dc_obj_priv_item *priv_item, uint32 privid)
{
    dc_obj_priv_entry_t *entry = NULL;
    text_t obj_name;
...
void dc_revoke_objpriv_from_role_by_id(dc_context_t *ctx, dc_role_t *role, dc_obj_priv_item *priv_item, uint32 privid)
{
    text_t obj_name;
    dc_obj_priv_entry_t *entry = NULL;
...
status_t dc_load_objpriv(knl_session_t *session, dc_context_t *ctx, knl_cursor_t *cursor)
{
    ...
    if (grantee_type == 0) {
        ...
        if (!dc_find_objpriv_entry(&user->obj_privs, owner_id, &objname, objtype, &entry)) {
            if (dc_alloc_objpriv_entry(ctx, &user->obj_privs, user->memory, owner_id, &objname, objtype, &entry) !=
                GS_SUCCESS) {
                return GS_ERROR;
            }
        }
        ...
        if (!dc_find_objpriv_entry(&role->obj_privs, owner_id, &objname, objtype, &entry)) {
            if (dc_alloc_objpriv_entry(ctx, &role->obj_privs, role->memory, owner_id, &objname, objtype, &entry) !=
                GS_SUCCESS) {
                return GS_ERROR;
            }
        }
    ...
    if (GS_SUCCESS == dc_open_user_by_id(session, grant_uid, &grantor_user)) {
        if (dc_add_user_grant_objpriv(session, grantor_user, grantee_type, grantee_id, &entry->priv_item, privid) !=
            GS_SUCCESS) {
            return GS_ERROR;
        }
    }
...
status_t dc_load_role_priv(knl_session_t *session, dc_context_t *ctx, knl_cursor_t *cursor)
{
    ...
    if (grantee_type == 0) {
        ...
        if (dc_alloc_mem(&session->kernel->dc_ctx, user->memory, sizeof(dc_granted_role), (void **)&parent) !=
            GS_SUCCESS) {
            cm_spin_unlock(&ctx->lock);
            return GS_ERROR;
        }
        ...
        /* add to the list */
        if (dc_alloc_mem(&session->kernel->dc_ctx, role->memory, sizeof(dc_granted_role), (void **)&child) !=
            GS_SUCCESS) {
            cm_spin_unlock(&ctx->lock);
            return GS_ERROR;
        }
        ...
        if (dc_alloc_mem(&session->kernel->dc_ctx, role2->memory, sizeof(dc_granted_role), (void **)&parent) !=
            GS_SUCCESS) {
            cm_spin_unlock(&ctx->lock);
            return GS_ERROR;
        }
...
bool32 dc_find_user_grant_objpriv(dc_user_t *user, uint32 grantee_type, uint32 grantee_id, dc_obj_priv_item *priv_item,
    dc_grant_obj_priv **grant_obj_priv, uint32 priv_id)
{
    cm_list_head *item = NULL;
    dc_grant_obj_priv *entry = NULL;
    ...
    dc_obj_priv_item *priv_item, uint32 priv_id)
{
    ...
    if (!dc_find_user_grant_objpriv(user, grantee_type, grantee_id, priv_item, &grant_obj_priv, priv_id)) {
        if (dc_alloc_mem(ctx, user->memory, sizeof(dc_grant_obj_priv), (void **)&grant_obj_priv) != GS_SUCCESS) {
            return GS_ERROR;
        }
        ...
        err = strncpy_s(grant_obj_priv->priv_item.objname, GS_NAME_BUFFER_SIZE, priv_item->objname, objname_len);
        knl_securec_check(err);
...
bool32 dc_check_obj_priv_by_name(knl_session_t *session, text_t *curr_user, text_t *objuser, text_t *objname,
    ...
    object_type_t objtype, uint32 privid)
{
    ...
    while (eid != GS_INVALID_ID32) {
        ...
        if (owner == entry->priv_item.objowner && (uint32)objtype == entry->priv_item.objtype &&
...
bool32 dc_check_allobjprivs_with_option(knl_session_t *session, text_t *curr_user, text_t *objuser, text_t *objname,
...
void dc_collect_roles_objprivs(dc_role_t *role, uint32 *objprivs, dc_obj_priv_item *priv_item)
{
    ...
    cm_list_for_each(item, &role->parent)
    {
        ...
        if (dc_find_objpriv_entry(&dc_role->obj_privs, priv_item->objowner, &objname, priv_item->objtype, &entry)) {
...
void dc_collect_user_objpriv(dc_user_t *user, uint32 *objprivs, dc_obj_priv_item *priv_item)
{
    ...
    cm_list_for_each(item, &user->parent)
    {
        ...
        cm_str2text(priv_item->objname, &objname);
        if (dc_find_objpriv_entry(&role->obj_privs, priv_item->objowner, &objname, priv_item->objtype, &entry)) {
            *objprivs |= entry->priv_item.direct_grant;
        }
...
void dc_update_user_objpriv_info(dc_context_t *ctx, dc_user_t *user, dc_obj_priv_item *priv_item)
{
    ...
    if (!dc_find_objpriv_entry(&user->obj_privs, priv_item->objowner, &objname, priv_item->objtype, &entry)) {
        ...
        if (dc_alloc_objpriv_entry(ctx, &user->obj_privs, user->memory, priv_item->objowner, &objname,
            priv_item->objtype, &entry) != GS_SUCCESS) {
            return;
        }
    }
...
void dc_merge_role_objpriv_to_user(knl_session_t *session, dc_role_t *role, dc_user_t *user)
{
    ...
    for (eid = 0; eid < role->obj_privs.hwm; eid++) {
        ...
        if (entry != NULL && entry->valid && entry->priv_item.objowner != user->desc.id) {
            ...
            if (!dc_find_objpriv_entry(&user->obj_privs, entry->priv_item.objowner, &objname, entry->priv_item.objtype,
                &user_entry)) {
                /* allocate an object entry for the user */
                if (dc_alloc_objpriv_entry(&session->kernel->dc_ctx, &user->obj_privs, user->memory,
                    entry->priv_item.objowner, &objname, entry->priv_item.objtype, &user_entry) != GS_SUCCESS) {
                    return;
                }
            }
        ...
        if (user != NULL && user->status == USER_STATUS_NORMAL) {
            ...
            cm_list_for_each(item, &user->grant_obj_privs)
            {
                ...
                if (cm_str_equal(grant_obj_priv->priv_item.objname, oldname) &&
                    (grant_obj_priv->priv_item.objowner == uid) && (grant_obj_priv->priv_item.objtype == type)) {
    ...
    dc_obj_priv_entry_t **dc_entry)
{
    ...
    while (eid != GS_INVALID_ID32) {
        entry = DC_GET_OBJPRIV_ENTRY(obj_privs, eid);
        if (uid == entry->priv_item.objowner && obj_type == entry->priv_item.objtype &&
