...
status_t db_insert_sys_priv(knl_handle_t session, uint32 id, uint32 grantee_type, uint32 priv_id, uint32 admin_opt)
{
    uint32 max_size;
    row_assist_t ra;
...
status_t db_delete_sys_priv(knl_handle_t session, uint32 uid, uint32 type, uint32 priv_id)
{
    ...
    knl_set_scan_key(INDEX_DESC(cursor->index), &cursor->scan_range.l_key, GS_TYPE_INTEGER, (void *)&type, sizeof(type),
...
status_t db_update_sys_priv(knl_handle_t session, uint32 grantee_id, uint32 grantee_type, uint32 priv_id,
    uint32 admin_value)
{
    uint32 admin_opt;
    knl_cursor_t *cursor = NULL;
...
static status_t db_update_user_roles(knl_handle_t session, uint32 uid, uint32 grantee_type, uint32 rid,
    uint32 admin_opt)
{
    knl_cursor_t *cursor = NULL;
    row_assist_t ra;
...
status_t db_insert_user_privs(knl_handle_t session, uint32 uid, uint32 grantor_id, uint32 grantee_id, uint32 priv_type)
{
    row_assist_t ra;
    knl_cursor_t *cursor = NULL;
    knl_session_t *knl_session = (knl_session_t *)session;
...
status_t db_update_objname_for_priv(knl_handle_t session, uint32 uid, const char *oldname, text_t *newname, uint32 type)
{
    knl_cursor_t *cursor = NULL;
    row_assist_t ra;
    knl_session_t *knl_session = (knl_session_t *)session;
...
status_t db_delete_all_sysprivs(knl_handle_t session, uint32 uid, uint32 type)
{
    ...
    knl_set_scan_key(INDEX_DESC(cursor->index), &cursor->scan_range.l_key, GS_TYPE_INTEGER, (void *)&type, sizeof(type),
        IX_COL_SYS_PRIVS_001_GRANTEE_TYPE);
    knl_set_scan_key(INDEX_DESC(cursor->index), &cursor->scan_range.r_key, GS_TYPE_INTEGER, (void *)&type, sizeof(type),
        IX_COL_SYS_PRIVS_001_GRANTEE_TYPE);
...
static status_t db_insert_user_roles(knl_handle_t session, uint32 grantee_id, uint32 type, uint32 rid, uint32 admin_opt)
{
    uint32 max_size;
    row_assist_t ra;
    knl_cursor_t *cursor = NULL;
...
status_t db_drop_object_privs(knl_session_t *session, uint32 uid, const char *objname, uint32 type)
{
    ...
    knl_set_scan_key(INDEX_DESC(cursor->index), &cursor->scan_range.l_key, GS_TYPE_INTEGER, (void *)&type, sizeof(type),
        IX_COL_SYS_OBJECT_PRIVS_002_OBJECT_TYPE);
...
status_t db_delete_obj_priv_by_grantor(knl_handle_t session, uint32 root_grantor, uint32 grantor, uint32 grantor_type,
...
// 2024-9-8
// revoke user privileges from SYS_USER_PRIVS by priv_id
// conflict with enable_auto_inherit
status_t db_delete_user_privs_by_privid(knl_session_t *session, uint32 uid, uint32 grantee_id, obj_privs_id priv_id)
{
    knl_cursor_t *cursor = NULL;
    knl_session_t *knl_session = (knl_session_t *)session;
...
status_t db_grant_syspriv_to_user(knl_handle_t session, void *def, knl_holders_def_t *grantee, knl_priv_def_t *priv)
{
    uint32 uid;
...
status_t db_grant_syspriv_to_role(knl_handle_t session, void *def, knl_holders_def_t *grantee, knl_priv_def_t *priv)
{
    uint32 rid;
    dc_context_t *dc_ctx = NULL;
    dc_role_t *role = NULL;
...
status_t db_grant_objpriv_to_user(knl_handle_t session, void *def, knl_holders_def_t *grantee, knl_priv_def_t *priv)
{
    uint32 owner_uid;
    dc_user_t *user = NULL;
    knl_grant_def_t *grant_def = (knl_grant_def_t *)def;
    ...
    if (owner_uid == user->desc.id || (user->desc.id == DB_SYS_USER_ID && grant_def->priv_type != PRIV_TYPE_OBJ_PRIV)) {
        return GS_SUCCESS;
    }
    ...
    if (db_insert_object_privs(session, user->desc.id, grantee->type, priv->priv_id, &priv_item, grant_def->grant_opt,
        grant_def->grant_uid) != GS_SUCCESS) {
        return GS_ERROR;
    }
...
status_t db_grant_userpriv_to_user(knl_handle_t session, void *def, knl_holders_def_t *grantee, knl_priv_def_t *priv)
{
    uint32 grantee_id;
    dc_user_t *user = NULL;
    knl_grant_def_t *grant_def = (knl_grant_def_t *)def;
    ...
    if (db_insert_user_privs(session, user->desc.id, grant_def->grant_uid, grantee_id, priv->priv_id) != GS_SUCCESS) {
        return GS_ERROR;
    }
...
status_t db_grant_objpriv_to_role(knl_handle_t session, void *def, knl_holders_def_t *grantee, knl_priv_def_t *priv)
{
    uint32 rid;
    uint32 owner_uid;
    dc_context_t *dc_ctx = NULL;
    ...
    if (db_insert_object_privs(session, role->desc.id, grantee->type, priv->priv_id, &priv_item, grant_def->grant_opt,
        grant_def->grant_uid) != GS_SUCCESS) {
        return GS_ERROR;
    }
    ...
    if (dc_add_user_grant_objpriv(knl_session, grant_user, grantee->type, role->desc.id, &priv_item, priv->priv_id) !=
        GS_SUCCESS) {
        return GS_ERROR;
    }
...
status_t db_grant_userpriv_to_role(knl_handle_t session, void *def, knl_holders_def_t *grantee, knl_priv_def_t *priv)
{
    ...
    cm_list_for_each(item, &role1->child_roles)
    {
        ...
        if (child->granted_role == role2) {
            if (grant_def->admin_opt == 1 && child->admin_opt == 0) {
                ...
                return db_update_user_roles(session, role2->desc.id, (uint32)priv->priv_type, role1->desc.id,
                    grant_def->admin_opt);
            }
...
status_t db_grant_objprivs(knl_handle_t session, void *def, knl_holders_def_t *grantee, obj_privs_id *privset,
    uint32 count)
{
    uint32 i;
...
status_t db_grant_allprivs_to_user(knl_handle_t session, void *def, knl_holders_def_t *grantee, knl_priv_def_t *priv)
{
    uint32 priv_id;
    knl_priv_def_t priv_item;
    knl_grant_def_t *grant_def = (knl_grant_def_t *)def;
...
status_t db_grant_allprivs_to_role(knl_handle_t session, void *def, knl_holders_def_t *grantee, knl_priv_def_t *priv)
{
    uint32 priv_id;
    knl_priv_def_t priv_item;
    knl_grant_def_t *grant_def = (knl_grant_def_t *)def;
...
status_t db_revoke_syspriv_from_user(knl_handle_t session, void *def, knl_holders_def_t *revokee, knl_priv_def_t *priv)
{
    uint32 uid;
    knl_session_t *knl_session = (knl_session_t *)session;
    dc_context_t *ctx = &knl_session->kernel->dc_ctx;
    ...
    user = ctx->users[uid];
    if (!DC_HAS_SYS_PRIV(user->sys_privs, priv->priv_id)) {
        GS_THROW_ERROR(ERR_PRIVS_NOT_GRANT, T2S(&priv->priv_name), T2S_EX(&revokee->name));
        return GS_ERROR;
    }
...
status_t db_revoke_syspriv_from_role(knl_handle_t session, void *def, knl_holders_def_t *revokee, knl_priv_def_t *priv)
{
    uint32 rid;
    dc_role_t *role = NULL;
    knl_session_t *knl_session = (knl_session_t *)session;
...
status_t db_get_grantor_id(knl_handle_t session, uint32 grantee, uint32 grantee_type, assist_obj_priv_item_t *item,
    uint32 *up_grantor)
{
    knl_cursor_t *cursor = NULL;
    knl_session_t *knl_session = (knl_session_t *)session;
...
status_t db_revoke_objpriv_from_user(knl_handle_t session, void *def, knl_holders_def_t *revokee, knl_priv_def_t *priv)
{
    uint32 objowner;
    uint32 grantorid;
    dc_user_t *user = NULL;
...
status_t db_revoke_userpriv_from_user(knl_handle_t session, void *def, knl_holders_def_t *revokee, knl_priv_def_t *priv)
{
    uint32 grantee_id;
    dc_user_t *user = NULL;
    dc_user_priv_entry_t *entry = NULL;
    ...
    // 2024-9-8 why is GS_PRIV_INHERIT_PRIVILEGES??
    // return db_delete_user_privs_single(session, user->desc.id, grantee_id, GS_PRIV_INHERIT_PRIVILEGES);
    return db_delete_user_privs_by_privid(session, user->desc.id, grantee_id, priv->priv_id);
}

status_t db_revoke_objpriv_from_role(knl_handle_t session, void *def, knl_holders_def_t *revokee, knl_priv_def_t *priv)
{
    uint32 rid, objowner;
    dc_role_t *role = NULL;
    dc_context_t *ctx = &((knl_session_t *)session)->kernel->dc_ctx;
...
status_t db_revoke_userpriv_from_role(knl_handle_t session, void *def, knl_holders_def_t *revokee, knl_priv_def_t *priv)
{
    GS_THROW_ERROR(ERR_SQL_SYNTAX_ERROR, "invalid privilege on role");
    return GS_ERROR;
}

status_t db_revoke_role_from_user(knl_handle_t session, void *def, knl_holders_def_t *revokee, knl_priv_def_t *priv)
{
    uint32 uid;
    uint32 rid;
...
status_t db_revoke_role_from_role(knl_handle_t session, void *def, knl_holders_def_t *revokee, knl_priv_def_t *priv)
{
    bool32 granted = GS_FALSE;
    uint32 rid1, rid2;
    cm_list_head *item = NULL;
...
void dc_grant_objpriv_to_user(knl_handle_t session, void *def, void *privs, hold_t *h)
{
    ...
    /* the largest objtype is not larger than uint32 */
    if (dc_find_objpriv_entry(&user->obj_privs, grant_def->objowner, &grant_def->objname, (uint32)grant_def->objtype,
        &entry)) {
...
void dc_grant_userpriv_to_user(knl_handle_t session, void *def, void *privs, hold_t *h)
{
    ...
    if (dc_find_user_priv_entry(&user->user_privs, grantee->desc.id, &entry)) {
        ...
        GS_RETVOID_IFERR(dc_alloc_user_priv_entry(ctx, &user->user_privs, user->memory, grantee->desc.id, &entry));
    }
...
void dc_grant_objpriv_to_role(knl_handle_t session, void *def, void *privs, hold_t *h)
{
    ...
    /* check if the role has been already directly granted the object privilege */
    /* the largest objtype is not larger than uint32 */
    if (dc_find_objpriv_entry(&role->obj_privs, grant_def->objowner, &grant_def->objname, (uint32)grant_def->objtype,
        &entry)) {
        if (DC_HAS_OBJ_PRIV(entry->priv_item.direct_grant, p->id)) {
            return;
        }
...
bool32 dc_rela_role_to_user(knl_session_t *sess, dc_user_t *user, dc_role_t *role, knl_grant_def_t *grant_def)
{
    ...
    if (cm_list_is_empty(&role->child_users_free)) {
        if (dc_alloc_mem(&sess->kernel->dc_ctx, role->memory, sizeof(dc_user_granted), (void **)&user_grant) !=
            GS_SUCCESS) {
            return GS_FALSE;
        }
    } else {
    ...
    if (cm_list_is_empty(&user->parent_free)) {
        if (dc_alloc_mem(&sess->kernel->dc_ctx, user->memory, sizeof(dc_granted_role), (void **)&grant_role) !=
            GS_SUCCESS) {
            return GS_FALSE;
        }
    } else {
...
bool32 dc_rela_role_to_role(knl_session_t *sess, dc_role_t *role1, dc_role_t *role2, knl_grant_def_t *grant_def)
{
    cm_list_head *item = NULL;
    dc_granted_role *child = NULL;
    ...
    if (cm_list_is_empty(&role1->child_roles_free)) {
        if (dc_alloc_mem(&sess->kernel->dc_ctx, role1->memory, sizeof(dc_granted_role), (void **)&child) !=
            GS_SUCCESS) {
            return GS_FALSE;
        }
    } else {
    ...
    if (cm_list_is_empty(&role2->parent_free)) {
        if (dc_alloc_mem(&sess->kernel->dc_ctx, role2->memory, sizeof(dc_granted_role), (void **)&parent) !=
            GS_SUCCESS) {
            return GS_FALSE;
        }
    } else {
...
priv_proc_func find_priv_proc_function(knl_priv_proc_tab *proc_tab, uint32 count, priv_type_def priv_type,
...
status_t db_exec_grant_write_table(knl_handle_t session, knl_grant_def_t *def)
{
    ...
    for (i = 0; i < def->grantees.count; i++) {
        ...
        for (j = 0; j < def->privs.count; j++) {
            ...
            proc_func =
                find_priv_proc_function(g_grant_proc_func, PRIV_GRANT_PROC_FUNC_COUNT, priv->priv_type, grantee->type);
            if (proc_func == NULL) {
...
status_t db_exec_grant_update_dc(knl_session_t *session, knl_grant_def_t *def)
{
    ...
    if ((g_tls_error.code == ERR_GRANT_OBJ_EXCEED_MAX) || (g_tls_error.code == ERR_DC_BUFFER_FULL) ||
...
status_t db_exec_revoke_write_table(knl_session_t *session, knl_revoke_def_t *def)
{
    ...
    for (i = 0; i < def->revokees.count; i++) {
        ...
        for (j = 0; j < def->privs.count; j++) {
            ...
            proc_func = find_priv_proc_function(g_revoke_proc_func, PRIV_REVOKE_PROC_FUNC_COUNT, priv->priv_type,
                revokee->type);
            if (proc_func == NULL) {
                GS_LOG_RUN_ERR("[PRIV] failed to find priv proc function");
                return GS_ERROR;
    ...
    uint32 priv_id)
{
    ...
    if (db_insert_object_privs(session, user->desc.id, TYPE_USER, priv_id, &priv_item, 0, DB_SYS_USER_ID) !=
        GS_SUCCESS) {
        return GS_ERROR;
    }
...
static status_t db_grant_dirpriv_update_dc(knl_session_t *session, text_t *dir_name, dc_user_t *user, uint32 priv_id)
{
    ...
    if (dc_find_objpriv_entry(&user->obj_privs, DB_SYS_USER_ID, dir_name, OBJ_TYPE_DIRECTORY, &entry)) {
        ...
        /* add a entry for the object */
        if (dc_alloc_objpriv_entry(ctx, &user->obj_privs, user->memory, DB_SYS_USER_ID, dir_name, OBJ_TYPE_DIRECTORY,
            &entry) != GS_SUCCESS) {
            return GS_ERROR;
        }
    }
