...
bool32 dc_verify_user_password(knl_session_t *session, const text_t *user, const char *password)
{
    dc_context_t *ctx = &session->kernel->dc_ctx;

    for (uint32 i = 0; i < GS_MAX_USERS; i++) {
        if (!ctx->users[i]) {
            continue;
        }
        if (cm_text_str_equal_ins(user, ctx->users[i]->desc.name)) {
            uint32 check_uid = 0;
            if (dc_get_user_id(session, user, &check_uid) == GS_FALSE) {
                return GS_FALSE;
            }

            uint32_t ori_pswd_len = strlen(ctx->users[i]->desc.password);
            uint32_t client_pswd_len = strlen(password);
            //debug log
            /* char *str_ori_dgst = (char *)malloc((ori_pswd_len * 2) + 1);
            char *str_client_dgst = (char *)malloc((client_pswd_len * 2) + 1);
            for (int j = 0; j < ori_pswd_len; j++) {
                sprintf(&str_ori_dgst[j * 2], "%02x", ctx->users[i]->desc.password[j] & 0xFF);
            }
            for (int j = 0; j < client_pswd_len; j++) {
                sprintf(&str_client_dgst[j * 2], "%02x", password[j] & 0xFF);
            }
            GS_LOG_RUN_INF("ori user:%s, pswd:%s-%s size:%lu", user->str, str_ori_dgst, ctx->users[i]->desc.password, ori_pswd_len);
            GS_LOG_RUN_INF("cli user:%s, pswd:%s size:%lu", user->str, str_client_dgst, client_pswd_len); 
            free(str_ori_dgst);
            free(str_client_dgst); */
            return strcmp(ctx->users[i]->desc.password, password) == 0 ? GS_TRUE : GS_FALSE;
        }
    }
    return GS_FALSE;
}
