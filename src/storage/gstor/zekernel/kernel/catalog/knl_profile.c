...
#define FAILED_LOGIN_ATTEMPTS_DESC \
...
#define PASSWORD_REUSE_MAX_DESC \
    "Specify the number of password changes required before the current password can be reused"
#define PASSWORD_LOCK_TIME_DESC                                                                                    \
    "Specify the number of days an account will be locked after the specified number of consecutive failed login " \
    "attempts"
#define PASSWORD_GRACE_TIME_DESC \
    "Specify the number of days after the grace period begins during which a warning is issued and login is allowed"
#define SESSION_PER_USER_DESC \
    "Specify the number of days after the grace period begins during which a warning is issued and login is allowed"
...
static void profile_insert_into_buckets(knl_session_t *session, profile_array_t *profile_array, bucket_t *bucket,
    profile_t *profile)
{
...
status_t profile_get_param_limit(knl_session_t *session, uint32 profile_id, resource_param_t param_id, uint64 *limit)
{
    profile_t *profile = NULL;
...
status_t profile_alter(knl_session_t *session, knl_profile_def_t *def)
{
    ...
    for (uint32 i = FAILED_LOGIN_ATTEMPTS; i < RESOURCE_PARAM_END; i++) {
        if (GS_BIT_TEST(def->mask, GS_GET_MASK(i))) {
            knl_set_scan_key(INDEX_DESC(cursor->index), &cursor->scan_range.l_key, GS_TYPE_INTEGER, &i, sizeof(uint32),
                1);
            if (GS_SUCCESS != knl_fetch(session, cursor)) {
                CM_RESTORE_STACK(session->stack);
                cm_unlatch(&bucket->latch, NULL);
...
void rd_alter_profile(knl_session_t *session, log_entry_t *log)
{
    ...
    knl_open_sys_cursor(session, cursor, CURSOR_ACTION_SELECT, SYS_PROFILE_ID, IX_SYS_PROFILE_001_ID);
    knl_init_index_scan(cursor, GS_FALSE);
    knl_set_scan_key(INDEX_DESC(cursor->index), &cursor->scan_range.l_key, GS_TYPE_INTEGER, &rd->id, sizeof(uint32), 0);
    knl_set_key_flag(&cursor->scan_range.l_key, SCAN_KEY_LEFT_INFINITE, 1);

    knl_set_scan_key(INDEX_DESC(cursor->index), &cursor->scan_range.r_key, GS_TYPE_INTEGER, &rd->id, sizeof(uint32), 0);
    knl_set_key_flag(&cursor->scan_range.r_key, SCAN_KEY_RIGHT_INFINITE, 1);
