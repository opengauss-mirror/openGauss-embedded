...
#define DC_CACHED_SERIAL_VALUE(value, start)                                                              \
    (((value) - (start)) / GS_SERIAL_CACHE_COUNT * GS_SERIAL_CACHE_COUNT + ((start) == 0 ? 1 : (start)) + \
        GS_SERIAL_CACHE_COUNT)
...
#define DC_ENTRY_IS_MONITORED(entry) ((entry)->type == DICT_TYPE_TABLE || (entry)->type == DICT_TYPE_TABLE_NOLOGGING)
...
#define DC_RESET_SEQ_CURRVAL(entry, id) (DC_GET_SEQ_CURRVAL((entry), (id))->serial = KNL_INVALID_SERIAL_ID)
...
#define DC_TABLE_PART(dc) (DC_ENTITY(dc)->table.part_table)
/* common function */
...
int dc_convert_table_desc(knl_cursor_t *cursor, knl_table_desc_t *desc);
status_t dc_copy_column_data(knl_session_t *session, knl_cursor_t *cursor, dc_entity_t *entity, uint32 id, void *dest,
    bool32 is_reserved);
