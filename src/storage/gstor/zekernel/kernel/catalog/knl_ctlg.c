...
knl_column_t g_sys_table_columns[] = {
    ...
    { 27, "TIMESCALE",    0, SYS_TABLE_ID, GS_TYPE_INTEGER,   sizeof(uint32),  0,                         0, GS_TRUE,  0, { NULL, 0 } },
    { 28, "RETENTION",    0, SYS_TABLE_ID, GS_TYPE_VARCHAR,   GS_MAX_RETENTION_LEN,  0,                   0, GS_TRUE,  0, { NULL, 0 } },
    { 29, "EX_FLAG",      0, SYS_TABLE_ID, GS_TYPE_INTEGER,   sizeof(uint32),  0,                         0, GS_TRUE,  0, { NULL, 0 } },
};
...
// 2024-9-10
// base on g_sys_privs_def
static sys_privs_id g_basic_sys_privs[] = {
    ALTER_USER,
...
status_t db_write_systable(knl_session_t *session, knl_cursor_t *cursor, knl_table_desc_t *desc)
{
    ...
    (void)row_put_int32(&ra, desc->is_timescale);       // is_timescale
    (void)row_put_str(&ra, desc->retention_str);
    // (void)row_put_int32(&ra, desc->has_autoincrement);  // ex_flag
...
// 2024-9-10
// build more basic permissions on role CONNECT
// build more basic permissions on user PUBLIC
// basic permissions are defined in g_basic_sys_privs
status_t db_build_basic_sys_privs(knl_session_t *session, knl_cursor_t *cursor)
{
    uint32 max_size;
    row_assist_t ra;

    uint32 count = ELEMENT_COUNT(g_basic_sys_privs);
    // to user PUBLIC
    for (uint32 i = 0; i < count; i++) {
        /* grant some sys privs to connect */
        max_size = session->kernel->attr.max_row_size;
        row_init(&ra, cursor->buf, max_size, 4);
        (void)(row_put_int32(&ra, PUBLIC_USER_ID));                // grantee id
        (void)(row_put_int32(&ra, PRIVS_GRANTEE_TYPE_USER));       // grantee type : user
        (void)(row_put_int32(&ra, g_basic_sys_privs[i]));          // privilege
        (void)(row_put_int32(&ra, PRIVS_ADMIN_OPTION_FALSE));      // admin option
...
status_t db_build_ex_systables(knl_session_t *session)
{
    ...
    // 2024-9-10
    if (db_build_basic_sys_privs(session, cursor) != GS_SUCCESS) {
        CM_RESTORE_STACK(session->stack);
        return GS_ERROR;
    }
