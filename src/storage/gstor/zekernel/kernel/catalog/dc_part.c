...
status_t dc_alloc_part_table(knl_session_t *session, dc_entity_t *entity, knl_part_desc_t *desc, part_table_t **pointer)
{
...
status_t dc_alloc_table_part(knl_session_t *session, dc_entity_t *entity, part_table_t *part_table, uint32 id)
{
    ...
    if (group->entity[eid] == NULL) {
        if (dc_alloc_mem(&session->kernel->dc_ctx, entity->memory, sizeof(table_part_t), (void **)&part) !=
            GS_SUCCESS) {
            GS_THROW_ERROR(ERR_DC_BUFFER_FULL);
            return GS_ERROR;
        }
    ...
    part_decode_key_t **interval_key)
{
    ...
    if (dc_alloc_mem(&session->kernel->dc_ctx, entity->memory, sizeof(part_decode_key_t), (void **)&decoders) !=
        GS_SUCCESS) {
        return GS_ERROR;
    }
    ...
    knl_part_desc_t *desc)
{
    ...
    if (desc->binterval.size != 0) {
        if (part_decode_key_interval(session, entity, (part_key_t *)desc->binterval.bytes, &desc->interval_key) !=
            GS_SUCCESS) {
            return GS_ERROR;
        }
    ...
    if (CURSOR_COLUMN_SIZE(cursor, SYS_PARTOBJECT_COL_AUTO_ADDPART) != GS_NULL_VALUE_LEN) {
        desc->auto_addpart = *(uint32 *)CURSOR_COLUMN_DATA(cursor, SYS_PARTOBJECT_COL_AUTO_ADDPART);
    }
    if (CURSOR_COLUMN_SIZE(cursor, SYS_PARTOBJECT_COL_IS_CORSSPART) != GS_NULL_VALUE_LEN) {
        desc->is_crosspart = *(uint32 *)CURSOR_COLUMN_DATA(cursor, SYS_PARTOBJECT_COL_IS_CORSSPART);
    }
    ...
    knl_table_part_desc_t *desc, table_part_t **part, uint32 start_pos)
{
    ...
    *partnos = (*part)->part_no;
    ...
    (*part)->pnext = GS_INVALID_ID32;
    dc_part_table_insert_name(part_table, *part);

    return GS_SUCCESS;
}

static status_t dc_handle_spliting_part(knl_session_t *session, table_t *table, uint32 count, dc_entity_t *entity,
    knl_table_part_desc_t *desc, table_part_t **part)
{
...
static status_t dc_load_part_columns(knl_session_t *session, knl_cursor_t *cursor, dc_entity_t *entity)
{
    ...
    while (!cursor->eof) {
        ...
        knl_panic_log(desc.pos_id < part_table->desc.partkeys,
            "the pos_id is not smaller than partkeys, panic info: "
            "page %u-%u type %u table %s index %s pos_id %u partkeys %u",
    ...
    part_key_t *key)
{
    ...
    if (decoders->offsets == NULL) {
        if (dc_alloc_mem(&session->kernel->dc_ctx, entity->memory, sizeof(uint16), (void **)&decoders->offsets) !=
            GS_SUCCESS) {
            return GS_ERROR;
        }
    }
    ...
    if (decoders->lens == NULL) {
        if (dc_alloc_mem(&session->kernel->dc_ctx, entity->memory, sizeof(uint16), (void **)&decoders->lens) !=
            GS_SUCCESS) {
            return GS_ERROR;
        }
    }
 ...
 */
status_t dc_decode_part_key_group(knl_session_t *session, dc_entity_t *entity, uint32 partkeys, part_key_t *key,
    part_decode_key_t **groups, uint32 *groupcnt)
{
    part_decode_key_t *decoders = NULL;
    status_t status;
    ...
    knl_panic_log(key->column_count < GS_MAX_COLUMNS,
        "the column_count is reaching the maximum, panic info: "
        "table %s column_count %u",
        entity->table.desc.name, key->column_count);

    uint16 count = key->column_count / partkeys;
    uint16 remained = key->column_count % partkeys;
    knl_panic_log(remained <= 1, "remained is bigger than 1, panic info: table %s remained %u", entity->table.desc.name,
        remained);
...
status_t dc_get_table_part_desc(knl_session_t *session, knl_cursor_t *cursor, dc_entity_t *entity, uint32 partkeys,
    knl_table_part_desc_t *desc, bool32 is_reserved)
{
    part_key_t *key = NULL;
...
index_part_t *dc_get_index_part(knl_handle_t dc_entity, uint32 index_id, uint32 part_no)
{
    ...
    knl_panic_log(part_no < part_index->desc.partcnt,
        "the part_no is not smaller than part_index's partcnt, "
        "panic info: table %s index %s part_no %u partcnt %u",
        table->desc.name, index->desc.name, part_no, part_index->desc.partcnt);
...
static status_t dc_load_index_parts(knl_session_t *session, knl_cursor_t *cursor, index_t *index)
{
    ...
    knl_panic_log(part_pos == table->part_table->desc.partcnt,
        "curr part_pos is not equal to table's part count, "
        "panic info: page %u-%u type %u table %s index %s part_pos %u partcnt %u",
        cursor->rowid.file, cursor->rowid.page, ((page_head_t *)cursor->page_buf)->type, table->desc.name,
        index->desc.name, part_pos, table->part_table->desc.partcnt);
...
count is %d",
    ...
    knl_panic_log(not_ready_count <= GS_SPLIT_PART_COUNT,
        "not_ready_count abnormal, panic info: not_ready_count %u "
        "page %u-%u type %u table %s index %s",
        cursor->rowid.file, cursor->rowid.page, not_ready_count, ((page_head_t *)cursor->page_buf)->type,
    ...
    knl_panic_log(i == part_table->desc.partcnt,
        "part count is abnormal, panic info: page %u-%u type %u table %s "
        "index %s curr part_partcnt %u partcnt %u",
        cursor->rowid.file, cursor->rowid.page, ((page_head_t *)cursor->page_buf)->type, table->desc.name,
        ((index_t *)cursor->index)->desc.name, i, part_table->desc.partcnt);
...
status_t dc_load_shadow_index_part(knl_session_t *session, knl_cursor_t *cursor, dc_entity_t *entity)
{
    ...
    knl_panic_log(index != NULL && index->part_index != NULL,
        "index is NULL or part_index is NULL, panic info: "
        "page %u-%u type %u table %s index %s",
        cursor->rowid.file, cursor->rowid.page, ((page_head_t *)cursor->page_buf)->type, table->desc.name,
        ((index_t *)cursor->index)->desc.name);
    ...
    if (dc_alloc_mem(&session->kernel->dc_ctx, entity->memory, sizeof(shadow_index_t), (void **)&table->shadow_index) !=
        GS_SUCCESS) {
        return GS_ERROR;
    }
    ...
    if (spc_valid_space_object(session, desc.space_id)) {
        ...
        GS_LOG_RUN_ERR("[DC CORRUPTED] could not load index partition %s of table %s.%s, tablespace %s is offline",
            shadow_part->desc.name, user->desc.name, entity->table.desc.name, SPACE_GET(desc.space_id)->ctrl->name);
    }
...
static status_t dc_load_shwidx_part(knl_session_t *session, index_t *index, knl_cursor_t *cursor, uint32 *part_pos)
{
    ...
    if (!IS_PARENT_IDXPART(&part->desc)) {
        if (dc_load_index_part_segment(session, index->entity, part) != GS_SUCCESS) {
            GS_LOG_RUN_WAR("[DC] could not load index partition %s of table %s.%s, segment corrupted", part->desc.name,
                user->desc.name, index->entity->table.desc.name);
        }
    }
...
status_t dc_load_shadow_indexparts(knl_session_t *session, knl_cursor_t *cursor, index_t *index)
{
    ...
    GS_LOG_DEBUG_INF("load shadow index parts: uid: %d, tid: %d, iid: %d, index partcnt: %d", index->desc.uid,
        index->desc.table_id, index->desc.id, part_pos);
    knl_panic_log(part_pos == table->part_table->desc.partcnt,
        "table's part count is abnormal, panic info: "
        "page %u-%u type %u table %s index %s curr part_pos %u partcnt %u",
        cursor->rowid.file, cursor->rowid.page, ((page_head_t *)cursor->page_buf)->type, table->desc.name,
        index->desc.name, part_pos, table->part_table->desc.partcnt);
...
static status_t dc_load_interval_index_part(knl_session_t *session, knl_cursor_t *cursor, index_t *index, uint32 partno,
    bool8 is_shadow)
{
    knl_index_part_desc_t desc = { 0 };
    dc_entity_t *entity = index->entity;
    ...
    if (spc_valid_space_object(session, desc.space_id)) {
        ...
        entity->corrupted = GS_TRUE;
        GS_LOG_RUN_ERR("[DC CORRUPTED] could not load index partition %s of table %s.%s, tablespace %s is offline",
            part->desc.name, user->desc.name, entity->table.desc.name, SPACE_GET(desc.space_id)->ctrl->name);
    }
...
static status_t dc_alloc_part_lob_group(knl_session_t *session, dc_entity_t *entity, part_lob_t *part_lob, uint32 gid)
{
    int32 ret;
...
status_t dc_alloc_lob_part(knl_session_t *session, dc_entity_t *entity, part_lob_t *part_lob, uint32 id)
{
    ...
    if (group->entity[eid] == NULL) {
        if (dc_alloc_mem(&session->kernel->dc_ctx, entity->memory, sizeof(lob_part_t), (void **)&group->entity[eid]) !=
            GS_SUCCESS) {
            GS_THROW_ERROR(ERR_DC_BUFFER_FULL);
            return GS_ERROR;
        }
    ...
    lob_t *lob, uint32 partno)
{
    ...
    if (!spc_valid_space_object(session, desc.space_id)) {
        ...
        dc_user_t *user = session->kernel->dc_ctx.users[table->desc.uid];
        GS_LOG_RUN_ERR(
            "[DC CORRUPTED] could not load lob partition of column %s of table %s.%s, tablespace %s is offline",
...
status_t dc_load_interval_part(knl_session_t *session, knl_dictionary_t *dc, uint32 part_no)
{
    ...
    if (table->shadow_index != NULL && table->shadow_index->is_valid && IS_PART_INDEX(&table->shadow_index->index)) {
        if (dc_load_interval_index_part(session, cursor, &table->shadow_index->index, part_no, GS_TRUE) != GS_SUCCESS) {
            CM_RESTORE_STACK(session->stack);
            cm_unlatch(&entity->cbo_latch, NULL);
            return GS_ERROR;
        }
...
static status_t dc_init_lob_part(knl_session_t *session, dc_entity_t *entity, knl_lob_part_desc_t *desc, lob_t *lob,
    lob_part_t **part, uint32 pos)
{
...
status_t dc_load_lob_parts(knl_session_t *session, knl_cursor_t *cursor, dc_entity_t *entity, lob_t *lob)
{
    ...
    GS_LOG_DEBUG_INF("load lob parts: load lob part from lob part$, uid: %d, tid: %d, column id: %d", lob->desc.uid,
        lob->desc.table_id, lob->desc.column_id);
    knl_panic_log(i == table->part_table->desc.partcnt,
        "the table's part count is abnormal, panic info: "
        "page %u-%u type %u table %s index %s curr part count %u partcnt %u",
        cursor->rowid.file, cursor->rowid.page, ((page_head_t *)cursor->page_buf)->type, table->desc.name,
