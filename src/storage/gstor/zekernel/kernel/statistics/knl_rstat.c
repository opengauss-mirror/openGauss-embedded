...
#define STATS_CALC_EXTENT_NUM(blocks, extent_size) ((uint32)(((blocks) + (extent_size)-1) / (extent_size)))
...
#define STATS_GET_SAMPLE_EXT_STEP(stats_sampler) ((stats_sampler)->total_extents / (stats_sampler)->sample_extents)
...
#define STATS_NOT_DATA_PAGE(type) ((type) != PAGE_TYPE_HEAP_DATA && (type) != PAGE_TYPE_PCRH_DATA)
...
#define STATS_IS_NOT_SPECIFY_PART(stats_table, part_id) \
    ((stats_table)->single_part_analyze && (stats_table)->specify_part_id != (part_id))
#define STATS_GLOBAL_CBO_STATS_EXIST(entity) ((entity)->stat_exists && (entity)->cbo_table_stats->global_stats_exist)
...
#define IS_STATS_TABLE_TYPE(table)                                                                     \
    (((table)->desc.type == TABLE_TYPE_HEAP) || ((table)->desc.type == TABLE_TYPE_MEMORY_DISK_HEAP) || \
        ((table)->desc.type == TABLE_TYPE_NOLOGGING))
...
static inline int32 stats_compare_mtrl_row(mtrl_segment_t *segment, mtrl_row_t *row1, mtrl_row_t *row2)
{
    ...
    result = stats_compare_data_ex(STATS_CDATA(row1, 0), STATS_CSIZE(row1, 0), STATS_CDATA(row2, 0),
        STATS_CSIZE(row2, 0), column);
    return result;
}
...
status_t stats_init_report_file(knl_session_t *session, dc_entity_t *entity, stats_option_t *stats_option)
{
    ...
    ret = sprintf_s(stats_file_name, GS_MAX_FILE_NAME_LEN, STATS_REPORT_FORMAT, stats_file_dir, entity->entry->name,
        "TAB");
    knl_securec_check_ss(ret);
...
static status_t stats_values_convert_str(knl_column_t *column, text_t *res_value, text_t *value)
{
    ...
    switch (column->datatype) {
        ...
        case GS_TYPE_TIMESTAMP_TZ:
            if (cm_timestamp_tz2text((timestamp_tz_t *)res_value->str, NULL, value, STATS_MAX_BUCKET_SIZE) !=
                GS_SUCCESS) {
                return GS_ERROR;
            }
            break;
...
static void stats_write_report_col_tail(knl_session_t *session, stats_col_handler_t *stats_col, dc_entity_t *entity,
    ...
    stats_col_context_t *col_ctx, stats_tab_context_t *tab_ctx)
{
    ...
    if (table_stats->is_part) {
        column_handler->g_rid =
            table_stats->part_start_rid; // for record data position in temp table of every table part
    }
}
    ...
    uint8 *type)
{
    ...
    if (session->stat_sample) {
        if (buf_read_page(session, page_id, LATCH_MODE_S, ENTER_PAGE_NORMAL | ENTER_PAGE_SEQUENTIAL) != GS_SUCCESS) {
            return GS_ERROR;
        }
    } else {
...
static void stats_next_extent_size(stats_sampler_t *stat_sample, stats_table_t *tab_stats, heap_segment_t *segment,
    heap_page_t *page)
{
    ...
    if (stat_sample->extent_size != origin_ext_size) {
        ret = memset_sp(stat_sample->random_step, sizeof(uint32) * stat_sample->pages_per_ext, 0,
            sizeof(uint32) * stat_sample->pages_per_ext);
        knl_securec_check(ret);
    ...
    uint8 *type, stats_table_t *tab_stats)
{
    ...
    for (;;) {
        ...
        if (buf_read_page(session, extent, LATCH_MODE_S, ENTER_PAGE_NORMAL | ENTER_PAGE_SEQUENTIAL) != GS_SUCCESS) {
            return GS_ERROR;
        }
    ...
    stats_table_t *table_stats)
{
    ...
    errno_t ret = memset_s(&hist_assit, sizeof(stats_hist_assist_t) * STATS_HISTGRAM_MAX_SIZE, 0xFF,
        sizeof(stats_hist_assist_t) * STATS_HISTGRAM_MAX_SIZE);
    knl_securec_check(ret);
...
static status_t stats_delete_old_histgram(knl_session_t *session, knl_cursor_t *cursor,
    stats_hist_rowids_t *hist_buckets, stats_table_t *table_stats)
{
...
status_t stats_insert_row(knl_session_t *session, knl_cursor_t *cursor, mtrl_context_t *temp_ctx, uint32 seg_id,
    stats_table_t *table_stats)
{
    table_t *table = (table_t *)cursor->table;
    ...
    for (;;) {
        ...
        if (cursor->eof) {
            if (!STATS_IS_ANALYZE_TEMP_TABLE(table_stats)) {
                if (buf_read_page(session, page_id, LATCH_MODE_S, ENTER_PAGE_NORMAL | ENTER_PAGE_SEQUENTIAL) !=
                    GS_SUCCESS) {
                    mtrl_close_page(temp_ctx, segment->vm_list.last);
                    segment->curr_page = NULL;
                    cm_pop(session->stack);
...
static status_t stats_open_mtrl_rs_cursor(mtrl_rowid_t rid, mtrl_context_t *temp_ctx, mtrl_cursor_t *cursor,
    uint32 temp_seg)
{
    vm_page_t *page = NULL;
    ...
    double *sample_ratio, stats_table_t *table_stats)
{
    ...
    if (extents_count == STATS_MIN_SAMPLE_EXTENTS) {
        ...
        min_sample_blocks =
            (space->ctrl->extent_size < STATS_MIN_SAMPLE_BLOCKS) ? space->ctrl->extent_size : STATS_MIN_SAMPLE_BLOCKS;
    }
...
static status_t stats_make_index_key(knl_handle_t session, mtrl_cursor_t *cursor, index_t *idx, char *key_buf)
{
...
static void stats_estimate_total_idx_stats(stats_index_t *stats_idx)
{
    ...
    if (stats_idx->sample_ratio > GS_REAL_PRECISION) {
        stats_idx->info.comb_cols_2_ndv =
            stats_estimate_ndv(stats_idx->info.comb_cols_2_ndv, stats_idx->info.keys, stats_idx->sample_ratio);
        stats_idx->info.comb_cols_3_ndv =
            stats_estimate_ndv(stats_idx->info.comb_cols_3_ndv, stats_idx->info.keys, stats_idx->sample_ratio);
        stats_idx->info.comb_cols_4_ndv =
            stats_estimate_ndv(stats_idx->info.comb_cols_4_ndv, stats_idx->info.keys, stats_idx->sample_ratio);
        stats_idx->info.distinct_keys =
            stats_estimate_ndv(stats_idx->info.distinct_keys, stats_idx->info.keys, stats_idx->sample_ratio);
...
static void stats_estimate_tmp_leaf_page(knl_session_t *session, stats_index_t *stats_idx, stats_table_t *table_stats)
{
    ...
    estimate_leaf_page =
        (uint32)((key_cost_size + page_data_size * page_load_factor - 1) / (page_data_size * page_load_factor));
    ...
    btree_segment_t *segment)
{
    ...
    page_data_size = DEFAULT_PAGE_SIZE - sizeof(btree_page_t) - space->ctrl->cipher_reserve_size - sizeof(page_tail_t) -
        page->itls * itl_size;
    buf_leave_page(session, GS_FALSE);
...
static status_t stats_update_sys_column(knl_session_t *session, stats_col_handler_t *column_handler, bool32 is_dynamic)
{
    knl_cursor_t *cursor = column_handler->stats_cur;
    row_assist_t ra;
    knl_scan_key_t *key = NULL;
...
status_t stats_write_sys_hist_head(knl_session_t *session, stats_col_handler_t *column_handler,
    stats_table_t *table_stats, uint32 part_id, bool32 is_subpart)
{
    stats_part_table_t *part_stats = &table_stats->part_stats;
    knl_cursor_t *cursor = column_handler->stats_cur;
...
void stats_estimate_column_stats(stats_table_t *table_stats, stats_col_handler_t *column_handler)
{
    ...
    if (estimate_sample_ratio > GS_REAL_PRECISION) {
        column_handler->dist_num =
            stats_estimate_ndv(column_handler->dist_num, column_handler->total_rows, estimate_sample_ratio);
...
void stats_set_global_extreme_value(stats_col_handler_t *col_handler, cbo_stats_column_t *col_stats)
{
    ...
    result = stats_compare_data_ex(col_stats->low_value.str, col_stats->low_value.len, col_handler->min_value.str,
        col_handler->min_value.len, column);
    ...
    result = stats_compare_data_ex(col_stats->high_value.str, col_stats->high_value.len, col_handler->max_value.str,
        col_handler->max_value.len, column);
...
{
    col_handler->dist_num =
        col_handler->dist_num > col_stats->num_distinct ? col_handler->dist_num : col_stats->num_distinct;
    col_handler->null_num = col_handler->null_num > col_stats->num_null ? col_handler->null_num : col_stats->num_null;
    col_handler->total_rows =
        col_handler->total_rows > col_stats->total_rows ? col_handler->total_rows : col_stats->total_rows;
    ...
    stats_table_t *table_stats, dc_entity_t *entity)
{
    ...
    if (!global_stats_exist) {
        if (stats_write_sys_hist_head(session, column_handler, table_stats, GS_INVALID_ID32, is_subpart) !=
            GS_SUCCESS) {
            return GS_ERROR;
        }
    }
    ...
    stats_table_t *table_stats)
{
    ...
    if (IS_ANALYZE_ALL_PARTS(table_stats)) {
        if (stats_delete_histgrams(session, cursor, column, table_stats->is_dynamic, column_handler->is_nologging) !=
            GS_SUCCESS) {
            return GS_ERROR;
        }
    } else {
        if (is_subpart) {
            ...
            if (stats_delete_histgram_subpart_column(session, cursor, column, part_loc, column_handler->is_nologging) !=
                GS_SUCCESS) {
                return GS_ERROR;
            }
        } else {
            if (stats_delete_histgram_part_column(session, cursor, column, table_stats->part_stats.part_id,
...
status_t stats_push_cursor_from_vm(knl_handle_t handle, stats_vm_list *vm_list, knl_cursor_t **cursor)
{
    ...
    *cursor = (knl_cursor_t *)stats_push_stack_from_vm(page, session->kernel->attr.cursor_size);
    ...
    knl_panic_log((*cursor)->update_info.columns != NULL,
        "update_info's columns is NULL, panic info: page %u-%u type %u table %s index %s", (*cursor)->rowid.file,
    ...
    knl_panic_log((*cursor)->update_info.offsets != NULL,
        "update_info's offsets is NULL, panic info: page %u-%u type %u table %s index %s", (*cursor)->rowid.file,
        (*cursor)->rowid.page, ((page_head_t *)(*cursor)->page_buf)->type, ((table_t *)(*cursor)->table)->desc.name,
        ((index_t *)(*cursor)->index)->desc.name);

    (*cursor)->update_info.lens = (uint16 *)stats_push_stack_from_vm(page, ext_size);
    knl_panic_log((*cursor)->update_info.lens != NULL,
        "update_info's lens is NULL, panic info: page %u-%u type %u table %s index %s", (*cursor)->rowid.file,
        (*cursor)->rowid.page, ((page_head_t *)(*cursor)->page_buf)->type, ((table_t *)(*cursor)->table)->desc.name,
        ((index_t *)(*cursor)->index)->desc.name);
    ...
    switch (sample_type) {
        case STATS_AUTO_SAMPLE:
            ...
            if (estimate_blocks * DEFAULT_PAGE_SIZE > default_sample_size) {
                stats_table->estimate_sample_ratio =
                    ((double)default_sample_size) / (estimate_blocks * DEFAULT_PAGE_SIZE);
            }

            if (specify_part_esti_blocks * DEFAULT_PAGE_SIZE > default_sample_size) {
                stats_table->part_sample_ratio =
                    ((double)default_sample_size) / (specify_part_esti_blocks * DEFAULT_PAGE_SIZE);
            }
        ...
        if (!sub_part->heap.loaded) {
            if (dc_load_table_part_segment(session, dc->handle, (table_part_t *)sub_part) != GS_SUCCESS) {
                GS_LOG_RUN_WAR("[DC] could not load table partition %s of table %s.%s, segment corrupted",
                    sub_part->desc.name, session->kernel->dc_ctx.users[table->desc.uid]->desc.name, table->desc.name);
                cm_reset_error();
                continue;
            }
        }
...
uint32 stats_get_pages_from_tablepart(knl_session_t *session, knl_dictionary_t *dc, table_part_t *table_part)
{
    ...
    if (!table_part->heap.loaded) {
        if (dc_load_table_part_segment(session, dc->handle, table_part) != GS_SUCCESS) {
            GS_LOG_RUN_WAR("[DC] could not load table partition %s of table %s.%s, segment corrupted",
                table_part->desc.name, session->kernel->dc_ctx.users[table->desc.uid]->desc.name, table->desc.name);
            cm_reset_error();
            return estimate_blocks;
        }
    }
...
void stats_generate_specified_cols_list(dc_entity_t *entity, stats_cols_list_t *col_list, stats_table_t *table_stats,
    bool32 with_index_cols)
{
    table_t *table = &entity->table;
    index_t *idx = NULL;
    ...
    stats_cols_list_t *column_list)
{
    ...
    uint32 thread_num =
        (column_list->column_count > stats_max_paraller) ? stats_max_paraller : column_list->column_count;
    ...
    stats_part_table_t *part_stats, uint32 part_no, bool32 is_parent)
{
    ...
    global_stats->tab_info.sample_size =
        cbo_global_stats->sample_size + (part_stats->info.sample_size - cbo_part_stats->sample_size);
    ...
    global_stats->tab_info.empty_block =
        cbo_global_stats->empty_blocks + (part_stats->info.empty_block - cbo_part_stats->empty_blocks);
...
void stats_calc_global_part_stats(dc_entity_t *entity, stats_table_t *global_stats, uint32 part_no, bool32 is_parent)
{
    ...
    if (cbo_part_stats == NULL) {
        ...
        global_stats->tab_info.row_len =
            (cbo_global_stats->avg_row_len * cbo_global_stats->rows) + part_stats->info.row_len;
    } else {
...
status_t stats_update_empty_sys_indexparts(knl_session_t *session, index_t *idx, stats_index_t *index_stats,
    uint32 part_id)
{
...
static status_t stats_try_gather_empty_subpart(knl_session_t *session, knl_dictionary_t *dc,
    table_part_t *table_subpart, bool32 *is_empty, bool32 is_dynamic)
{
    stats_table_info_t tab_info;
    ...
    stats_tab_context_t *tab_ctx, bool32 *is_empty_subpart)
{
    ...
    if (stats_try_gather_empty_subpart(session, dc, table_subpart, is_empty_subpart, is_dynamic) != GS_SUCCESS) {
        return GS_ERROR;
    }
    ...
    stats_tab_context_t *tab_ctx, stats_part_table_t *sub_part)
{
    ...
    double ori_sample_ratio = STATS_IS_ANALYZE_SINGLE_PART(table_stats, table_stats->part_stats.part_id) ?
        table_stats->part_sample_ratio :
        table_stats->estimate_sample_ratio;
    ...
    stats_table_info_t lose_info)
{
    ...
    knl_set_scan_key(INDEX_DESC(cursor->index), key, GS_TYPE_STRING, table->desc.name, (uint16)strlen(table->desc.name),
...
static status_t stats_update_sys_table(knl_session_t *session, stats_table_t *tab_stats, knl_dictionary_t *dc)
{
    knl_cursor_t *cursor = NULL;
    row_assist_t ra;
    knl_scan_key_t *key = NULL;
...
status_t db_insert_sys_mon_mods(knl_session_t *session, knl_cursor_t *cursor, stats_table_mon_t *table_smon, uint32 uid,
    uint32 id, uint32 part_id, bool32 is_part)
{
    table_t *table = NULL;
    row_assist_t ra;
...
static status_t stats_check_empty_table_subpart(knl_session_t *session, knl_dictionary_t *dc,
    table_part_t *table_subpart, bool32 *is_empty)
{
    if (!table_subpart->heap.loaded) {
        if (dc_load_table_part_segment(session, dc->handle, table_subpart) != GS_SUCCESS) {
            return GS_ERROR;
    ...
    bool32 is_dynamic)
{
    ...
    tab_stats.is_dynamic = is_dynamic;
    // if the table is not empty, we just start one  auto session to commit statistics of columns when dynamic
    // statistics there are 3 sys tables(sys_columns,sys_histgram,sys_hist_abstr) will be written for columns
    // statistics, for empty table we only write sys_columns.
    if (stats_try_begin_auton_rm(session, is_dynamic) != GS_SUCCESS) {
        return GS_ERROR;
    }
...
static status_t stats_gather_each_table_part(knl_session_t *session, knl_dictionary_t *dc, stats_tab_context_t *tab_ctx,
...
status_t stats_gather_all_table_parts(knl_session_t *session, knl_dictionary_t *dc, stats_tab_context_t *tab_ctx)
{
    ...
    for (uint32 i = 0; i < part_table->desc.partcnt; i++) {
        ...
        if (stats_gather_each_table_part(session, dc, tab_ctx, table_part) != GS_SUCCESS) {
            ...
            cm_reset_error();
            GS_LOG_RUN_WAR(
                "GS-%d, fail to gather the %s partition %s, it or its subpartition has been dropped or truncated.",
    ...
    bool32 is_dynamic)
{
    ...
    if (db_delete_mon_sysmods(session, table->desc.uid, table->desc.id, GS_INVALID_ID32, is_dynamic) != GS_SUCCESS) {
        mtrl_release_context(mtrl_tab_ctx);
        CM_RESTORE_STACK(session->stack);
        return GS_ERROR;
    }
...
static heap_segment_t *stats_get_part_heap_segment(knl_session_t *session, table_t *table, table_part_t *table_part)
{
...
static btree_segment_t *stats_get_part_btree_segment(knl_session_t *session, index_t *idx, index_part_t *index_part)
{
    ...
    btree_segment_t *segment, bool32 is_dynamic)
{
    ...
    if (stats_idx->info.distinct_keys == 0) {
        ...
        stats_idx->avg_leaf_key = (double)stats_idx->info.leaf_blocks / (double)stats_idx->info.distinct_keys;
        stats_idx->avg_data_key = (double)(btree_get_segment_page_count(space, segment) - segment->del_pages.count) /
            (double)stats_idx->info.distinct_keys;
    }
...
static status_t stats_sample_stats_btree_subpart(knl_session_t *session, index_t *idx, index_part_t *index_part,
    double sample_ratio, bool32 is_dynamic)
{
    ...
    for (uint32 i = 0; i < index_part->desc.subpart_cnt; i++) {
        ...
        if (stats_sample_stats_one_btree(session, &index_subpart->btree, &stats_idx.info, sample_ratio) != GS_SUCCESS) {
            return GS_ERROR;
        }

        if (stats_save_btree_stats(session, idx, &stats_idx, (btree_segment_t *)index_subpart->btree.segment,
...
static status_t stats_sample_stats_btree_part(knl_session_t *session, index_t *idx, double sample_ratio,
    bool32 is_dynamic)
{
    index_part_t *index_part = NULL;
    stats_index_t stats_idx;
    ...
    for (uint32 i = 0; i < idx->part_index->desc.partcnt; i++) {
        ...
        if (!IS_PARENT_IDXPART(&index_part->desc)) {
            ...
            if (stats_sample_stats_one_btree(session, &index_part->btree, &stats_idx.info, sample_ratio) !=
                GS_SUCCESS) {
                return GS_ERROR;
            }

            if (stats_save_btree_stats(session, idx, &stats_idx, (btree_segment_t *)index_part->btree.segment,
                is_dynamic) != GS_SUCCESS) {
                return GS_ERROR;
            }
        } else {
            if (stats_sample_stats_btree_subpart(session, idx, index_part, sample_ratio, is_dynamic) != GS_SUCCESS) {
                return GS_ERROR;
            }
        }
    }
...
static status_t stats_sample_stats_btree(knl_session_t *session, index_t *idx, double sample_ratio, bool32 is_dynamic)
{
    ...
    if (!IS_PART_INDEX(idx)) {
        ...
        if (stats_save_btree_stats(session, idx, &stats_idx, (btree_segment_t *)idx->btree.segment, is_dynamic) !=
            GS_SUCCESS) {
            return GS_ERROR;
        }
...
static status_t stats_full_stats_btree_subpart(knl_session_t *session, index_t *idx, index_part_t *index_part,
    bool32 is_dynamic)
{
    index_part_t *index_subpart = NULL;
    stats_index_t stats_idx;
...
status_t stats_flush_monitor_normal(knl_session_t *session)
{
    ...
    for (;;) {
        ...
        if (knl_open_dc_by_id(session, uid, table_id, &dc, GS_TRUE) != GS_SUCCESS) {
            GS_LOG_RUN_WAR("Open %u.%u monitor statistic failed normal, please gather statistics manually", uid,
                table_id);
            cm_reset_error();
            continue;
        }
...
static status_t stats_seg_insert(knl_session_t *session, knl_cursor_t *cursor, knl_seg_stats_desc_t *desc)
{
    ...
    if (knl_open_sys_temp_cursor(session, cursor, CURSOR_ACTION_INSERT, SYS_TMP_SEG_STAT_ID, GS_INVALID_ID32) !=
        GS_SUCCESS) {
        return GS_ERROR;
    }
...
static status_t stats_update_record(knl_session_t *session, knl_cursor_t *cursor, knl_seg_stats_desc_t *desc)
{
    row_assist_t ra;

    if (knl_open_sys_temp_cursor(session, cursor, CURSOR_ACTION_UPDATE, SYS_TMP_SEG_STAT_ID, IDX_OBJECT_ID) !=
        GS_SUCCESS) {
        return GS_ERROR;
    }
...
static status_t stats_insert_record(knl_session_t *session, knl_cursor_t *cursor, dc_entity_t *entity)
{
    ...
    if (stats_update_record(session, cursor, &desc) != GS_SUCCESS) {
        GS_LOG_DEBUG_ERR("insert table %u.%u statistic info failed, org_scn: %llu", table->desc.uid, table->desc.id,
            table->desc.org_scn);
        return GS_ERROR;
    }
    ...
    if (IS_PART_TABLE(table)) {
        for (i = 0; i < table->part_table->desc.partcnt; i++) {
            ...
            stats_seg_init_desc(table_part->heap.stat, table->desc.uid, table->desc.id, table_part->desc.org_scn,
                &desc);
            if (stats_update_record(session, cursor, &desc) != GS_SUCCESS) {
                GS_LOG_DEBUG_ERR("insert table %u.%u part_table statistic info failed, org_scn: %llu", table->desc.uid,
                    table->desc.id, table_part->desc.org_scn);
                return GS_ERROR;
            }
        }
    }
    ...
    for (i = 0; i < table->index_set.count; i++) {
        ...
        if (stats_update_record(session, cursor, &desc) != GS_SUCCESS) {
            GS_LOG_DEBUG_ERR("insert table %u.%u index statistic info failed, org_scn: %llu", table->desc.uid,
                table->desc.id, idx->desc.org_scn);
            return GS_ERROR;
        }
        ...
        for (j = 0; j < idx->part_index->desc.partcnt; j++) {
            ...
            stats_seg_init_desc(index_part->btree.stat, table->desc.uid, table->desc.id, index_part->desc.org_scn,
                &desc);
            if (stats_update_record(session, cursor, &desc) != GS_SUCCESS) {
                GS_LOG_DEBUG_ERR("insert table %u.%u part_index statistic info failed, org_scn: %llu", table->desc.uid,
                    table->desc.id, index_part->desc.org_scn);
                return GS_ERROR;
            }
        }
    }
...
status_t stats_seg_load_entity(knl_session_t *session, knl_scn_t org_scn, seg_stat_t *seg_stat)
{
    ...
    if (knl_open_sys_temp_cursor(temp_session, cursor, CURSOR_ACTION_SELECT, SYS_TMP_SEG_STAT_ID, IDX_OBJECT_ID) !=
        GS_SUCCESS) {
        cm_revert_error(code, msg, &loc);
        CM_RESTORE_STACK(temp_session->stack);
        cm_unlatch(&g_stats_latch, NULL);
 ...
 * oid    table id
 */
static status_t stats_reset_sys_tablepart(knl_session_t *session, uint32 uid, uint32 oid, uint32 part_id)
{
    knl_cursor_t *cursor = NULL;
    row_assist_t ra;
    uint16 size;
 ...
 * oid    table id
 */
static status_t stats_reset_sys_indexpart(knl_session_t *session, uint32 uid, uint32 oid, uint32 idx_id, uint32 part_id)
{
    knl_cursor_t *cursor = NULL;
    row_assist_t ra;
    uint16 size;
...
status_t stats_reset_indexparts_stats(knl_session_t *session, knl_dictionary_t *dc)
{
    ...
    for (uint32 i = 0; i < table->index_set.count; i++) {
        ...
        for (uint32 j = 0; j < part_index->desc.partcnt; j++) {
            ...
            if (stats_reset_sys_indexpart(session, dc->uid, dc->oid, idx->desc.id, index_part->desc.part_id) !=
                GS_SUCCESS) {
                return GS_ERROR;
            }
        }
...
{
    global_mon->inserts =
        (global_mon->inserts >= part_mon->inserts) ? (global_mon->inserts - part_mon->inserts) : global_mon->inserts;
    global_mon->deletes =
        (global_mon->deletes >= part_mon->deletes) ? (global_mon->deletes - part_mon->deletes) : global_mon->deletes;
    global_mon->updates =
        (global_mon->updates >= part_mon->updates) ? (global_mon->updates - part_mon->updates) : global_mon->updates;
    global_mon->drop_segments = (global_mon->drop_segments >= part_mon->drop_segments) ?
        (global_mon->drop_segments - part_mon->drop_segments) :
        global_mon->inserts;
...
status_t stats_check_analyzing(knl_session_t *session, knl_dictionary_t *dc, bool32 *need_analyze, bool32 is_dynamic)
{
    dc_entity_t *entity = DC_ENTITY(dc);
    table_t *table = &entity->table;
...
void stats_init_stats_option(stats_option_t *stats_option, knl_analyze_tab_def_t *def)
{
    ...
    stats_option->sample_ratio =
        def->sample_ratio < GS_REAL_PRECISION ? STATS_FULL_TABLE_SAMPLE_RATIO : (def->sample_ratio / 100);
...
static void stats_verfiy_colunm_stats(knl_cursor_t *cursor, knl_column_set_stats_t *col_stats)
{
    ...
    if (col_stats->max_value.len == GS_NULL_VALUE_LEN) {
        ret = memcpy_sp(col_stats->max_value.str, STATS_MAX_BUCKET_SIZE, sys_stats.max_value.str,
            sys_stats.max_value.len);
        knl_securec_check(ret);
    ...
    if (col_stats->min_value.len == GS_NULL_VALUE_LEN) {
        ret = memcpy_sp(col_stats->min_value.str, STATS_MAX_BUCKET_SIZE, sys_stats.min_value.str,
            sys_stats.min_value.len);
        knl_securec_check(ret);
...
status_t stats_update_sys_histhead_force(knl_session_t *session, knl_dictionary_t *dc,
    knl_column_set_stats_t *col_stats, table_part_t *table_part, knl_column_t *column)
{
...
static void stats_update_sys_index_row(knl_session_t *session, knl_cursor_t *cursor, knl_index_set_stats_t *idx_stats,
    uint32 empty_block)
{
    row_assist_t ra;
    uint16 size;
    ...
    index_t *index)
{
    ...
    knl_set_scan_key(INDEX_DESC(cursor->index), key, GS_TYPE_INTEGER, &index->desc.uid, sizeof(uint32),
        IX_COL_SYS_INDEX_002_USER);
    knl_set_scan_key(INDEX_DESC(cursor->index), key, GS_TYPE_STRING, index->desc.name, (uint16)strlen(index->desc.name),
        IX_COL_SYS_INDEX_002_NAME);
...
static status_t stats_set_text_value(knl_session_t *session, dc_entity_t *entity, knl_column_t *column, text_t *src,
    text_t *dest)
{
...
static status_t stats_set_cbo_column_value(knl_session_t *session, dc_entity_t *entity, knl_column_t *column,
    knl_column_set_stats_t *col_stats, cbo_stats_column_t *cbo_column)
{
    cbo_column->num_distinct = col_stats->distnum;
    cbo_column->density = col_stats->density;
    cbo_column->num_null = col_stats->nullnum;
    if (stats_set_text_value(session, entity, column, &col_stats->max_value, &cbo_column->high_value) != GS_SUCCESS) {
        return GS_ERROR;
    }

    if (stats_set_text_value(session, entity, column, &col_stats->min_value, &cbo_column->low_value) != GS_SUCCESS) {
        return GS_ERROR;
    }
    ...
    table_part_t *table_part, knl_column_t *column)
{
    ...
    if (cbo_column == NULL) {
        GS_LOG_RUN_WAR("Failed to modify %s.%s the %d column stats, it is not analyzed", entity->entry->user->desc.name,
