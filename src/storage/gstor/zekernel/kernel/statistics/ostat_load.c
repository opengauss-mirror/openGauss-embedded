...
status_t cbo_load_index_subpart_stats(knl_session_t *session, dc_entity_t *entity, uint32 part_no, uint32 index_id,
    uint32 subpart_no)
{
    table_t *table = &entity->table;
    index_t *idx = NULL;
    ...
    if (idx == NULL) {
        GS_LOG_RUN_WAR("Load %s.%s the %d index stats falied,it is not existed ", entity->entry->user->desc.name,
    ...
    bool32 *valid)
{
    ...
    int32 result =
        stats_compare_data_ex(ret_bucket.str, ret_bucket.len, stats->low_value.str, stats->low_value.len, column);
    ...
    result =
        stats_compare_data_ex(ret_bucket.str, ret_bucket.len, stats->high_value.str, stats->high_value.len, column);
    ...
    cbo_stats_column_t *stats, cbo_stats_table_t *part_stats)
{
    ...
    ret =
        memset_sp(&endpoints, sizeof(uint32) * STATS_HISTGRAM_MAX_SIZE, 0xFF, sizeof(uint32) * STATS_HISTGRAM_MAX_SIZE);
    knl_securec_check(ret);
...
static status_t cbo_alloc_one_column_stats(knl_session_t *se, memory_context_t *memory, cbo_stats_table_t *stats,
    uint32 pos)
{
...
static status_t cbo_alloc_column_stats(knl_session_t *session, dc_entity_t *entity, memory_context_t *memory,
    cbo_stats_table_t *stats)
{
...
static status_t cbo_alloc_temp_cache_stats(knl_session_t *session, dc_entity_t *entity, knl_temp_cache_t *temp_cache)
{
    ...
    if (temp_cache->cbo_stats == NULL) {
        if (cbo_alloc_mem(session, memory, (void **)&temp_cache->cbo_stats, sizeof(cbo_stats_table_t)) != GS_SUCCESS) {
            return GS_ERROR;
        }
...
status_t cbo_alloc_tmptab_stats(knl_session_t *session, dc_entity_t *entity, knl_temp_cache_t *temp_cache,
    bool32 is_dynamic)
{
...
status_t cbo_delay_load_subpart_stats(knl_session_t *session, dc_entity_t *entity, table_part_t *table_subpart,
    uint32 part_pos)
{
    CM_SAVE_STACK(session->stack);
    knl_cursor_t *cursor = knl_push_cursor(session);
...
status_t cbo_refresh_statistics(knl_session_t *session, dc_entity_t *entity, stats_load_info_t load_info)
{
    ...
    if (cbo_load_table_stats(session, cursor, entity, load_info) != GS_SUCCESS) {
        ...
        GS_LOG_RUN_WAR("[DC] could not load table statistics %s.%s.", user->desc.name, entry->name);
        cm_unlatch(&entity->cbo_latch, NULL);
        CM_RESTORE_STACK(session->stack);
        return GS_SUCCESS;
    }
    ...
    stats_load_info_t load_info, bool32 load_columns)
{
    ...
    if (table_part == NULL) {
        GS_LOG_RUN_WAR("Load %s.%s the %d table part stats falied, it is not existed ", entity->entry->user->desc.name,
    ...
    if (load_columns) {
        if (cbo_load_table_part_columns(session, entity, part_stats, is_subpart) != GS_SUCCESS) {
            CM_RESTORE_STACK(session->stack);
            GS_LOG_RUN_WAR("Load %s.%s the %d table part columns stats falied", entity->entry->user->desc.name,
                entity->table.desc.name, part_no);
            return GS_ERROR;
        }
