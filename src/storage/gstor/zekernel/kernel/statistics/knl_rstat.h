...
#define STATS_GET_ROW_DATA(mtrl_cur) ((char *)(mtrl_cur)->row.data + (mtrl_cur)->row.offsets[0])

#define STATS_GET_ROW_SIZE(mtrl_cur) ((mtrl_cur)->row.lens[0])
...
#define STATS_DYNAMICAL_TRANS_GTT(type, is_dynamic) ((type) == DICT_TYPE_TEMP_TABLE_TRANS && (is_dynamic))
