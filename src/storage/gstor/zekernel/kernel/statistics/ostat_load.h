...
#define CBO_GET_TABLE_PART(object, id) ((object)->part_groups[(id) / PART_GROUP_SIZE]->entity[(id) % PART_GROUP_SIZE])
#define CBO_GET_INDEX_PART(object, id) \
    ((object)->part_index_groups[(id) / PART_GROUP_SIZE]->entity[(id) % PART_GROUP_SIZE])
#define CBO_NEED_LOAD_PART(part_id, scan_part_id) ((part_id) == GS_INVALID_ID32 || (part_id) == (scan_part_id))
