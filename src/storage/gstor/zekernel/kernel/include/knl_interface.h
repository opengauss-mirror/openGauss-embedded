...
typedef struct st_knl_table_desc {
    ...
    union {
        ...
        struct {
            ...
            bool8 is_memory : 1;    //memory table
    ...
    char retention_str[GS_MAX_RETENTION_LEN];  // retention
...
typedef status_t (*knl_alloc_rm_t)(struct st_instance *cc_instance, uint16 *rmid);
typedef void (*knl_release_rm_t)(struct st_instance *cc_instance, uint16 rmid);
...
typedef uint16 (*knl_get_xa_xid_t)(struct st_instance *cc_instance, knl_xa_xid_t *xa_xid);
typedef bool32 (*knl_add_xa_xid_t)(struct st_instance *cc_instance, knl_xa_xid_t *xa_xid, uint16 dst_rmid, uint8 status);
typedef void (*knl_delete_xa_xid_t)(struct st_instance *cc_instance, knl_xa_xid_t *xa_xid);
...
typedef void (*knl_mtrl_init_vmc_t)(knl_handle_t session, knl_handle_t *mtrl);
...
status_t knl_parse_default_from_text(knl_handle_t handle, knl_handle_t entity,
    knl_handle_t column, memory_context_t *mem, void **expr_tree, void **expr_update_tree, text_t parse_text);
