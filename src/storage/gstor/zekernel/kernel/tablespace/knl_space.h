 ...
 */
#define DOUBLE_WRITE_PAGES (uint32)(1 * 1024)
...
#define SPACE_IS_BITMAPMANAGED(space) (SPACE_CTRL_IS_BITMAPMANAGED(space) || IS_SWAP_SPACE_BITMAP(space))
...
typedef enum st_pages_boundary {
    ...
    EXT_SIZE_128_PAGE_BOUNDARY =
        EXT_SIZE_8_PAGE_BOUNDARY + (EXT_SIZE_128_BOUNDARY - EXT_SIZE_8_BOUNDARY) * EXT_SIZE_128,
    EXT_SIZE_1024_PAGE_BOUNDARY =
        EXT_SIZE_128_PAGE_BOUNDARY + (EXT_SIZE_1024_BOUNDARY - EXT_SIZE_128_BOUNDARY) * EXT_SIZE_1024,
} pages_boundary_t;
...
#define SPACE_PUNCH_HEAD_RESIDENT(space)                                                          \
    ((buf_check_resident_page_version(session, (space)->entry)) ? (SPACE_PUNCH_HEAD_PTR(space)) : \
                                                                  (SPACE_PUNCH_HEAD_PTR(space)))
...
{
    if (SPACE_IS_DEFAULT(space) || SPACE_IS_ENCRYPT(space) || IS_UNDO_SPACE(space) || IS_TEMP_SPACE(space) ||
        SPACE_IS_BITMAPMANAGED(space)) {
        return GS_FALSE;
    }
    return GS_TRUE;
}
