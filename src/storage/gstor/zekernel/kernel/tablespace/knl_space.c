 ...
 */
status_t spc_get_compart_space_id(knl_session_t *session, uint32 uid, uint32 table_id, uint32 part_id, uint32 *space_id)
{
    CM_SAVE_STACK(session->stack);
    knl_cursor_t *cursor = knl_push_cursor(session);
...
static inline status_t spc_precheck_create_parameter(knl_session_t *session, space_t *space, knl_device_def_t *def,
    int64 max_file_size)
{
    if (def->compress) {
...
status_t spc_drop_datafile(knl_session_t *session, space_t *space, knl_device_def_t *def)
{
    ...
    for (i = 0; i < GS_MAX_DATA_FILES; i++) {
        df = DATAFILE_GET(i);
        if (df->ctrl->used && cm_text_str_equal(&def->name, df->ctrl->name) && spc_contain_datafile(space->ctrl, i)) {
            break;
        }
    }
...
status_t spc_extend_undo_segments(knl_session_t *session, uint32 count, datafile_t *df)
{
    ...
    if (cm_alter_config(session->kernel->attr.config, "_UNDO_SEGMENTS", seg_count, CONFIG_SCOPE_BOTH, GS_TRUE) !=
        GS_SUCCESS) {
        return GS_ERROR;
    }
...
status_t spc_rename_space(knl_session_t *session, space_t *space, text_t *rename_space)
{
    ...
    if (space->ctrl->id == core_ctrl->temp_undo_space || space->ctrl->id == core_ctrl->sysaux_space ||
        space->ctrl->id == core_ctrl->system_space || space->ctrl->id == core_ctrl->undo_space) {
...
status_t spc_active_undo_encrypt(knl_session_t *session, uint32 space_id)
{
    ...
    if (space->ctrl->encrypt_version == NO_ENCRYPT) {
        ...
        if (page_cipher_reserve_size(session, space->ctrl->encrypt_version, &space->ctrl->cipher_reserve_size) !=
            GS_SUCCESS) {
            cm_spin_unlock(&space->lock);
            return GS_ERROR;
        }
...
static status_t spc_prepare_swap_encrypt(knl_session_t *session, space_t *space)
{
    ...
    space->ctrl->extent_size = MAX((GS_VMEM_PAGE_SIZE + max_cipher_len) / DEFAULT_PAGE_SIZE + 1, GS_SWAP_EXTENT_SIZE);
...
status_t spc_mount_space(knl_session_t *session, space_t *space, bool32 auto_offline)
{
    ...
    for (uint32 i = 0; i < space->ctrl->file_hwm; i++) {
        ...
        df->file_no = i;
        df->space_id = space->ctrl->id;
        // GS_LOG_RUN_INF("file_name=%s, fileID=%d \n", space->ctrl->name, space->ctrl->files[i]);
        if (!DATAFILE_IS_ONLINE(df)) {
            GS_LOG_RUN_INF("[SPACE] offline space %s, cause datafile %s is offline", space->ctrl->name, df->ctrl->name);
...
#else
        if (actual_size > df->ctrl->size && !kernel->db.recover_for_restore)
#endif
        {
...
void spc_validate_extents(knl_session_t *session, page_list_t *extents)
{
    ...
    knl_panic_log(count == extents->count,
        "The current record extents count is not as expected, panic info: "
        "count %u extents count %u",
        count, extents->count);
}
...
void spc_validate_undo_extents(knl_session_t *session, undo_page_list_t *extents)
{
    ...
    knl_panic_log(count == extents->count,
        "The current record extents count is not as expected, panic info: "
        "current count %u extents count %u",
        count, extents->count);
}
...
{
    if (space->punching) {
        GS_THROW_ERROR(ERR_OPERATIONS_NOT_ALLOW, "%s when space %s is punching", info, space->ctrl->name);
        return GS_TRUE;
    }
    return GS_FALSE;
}
...
static void spc_update_hwms(knl_session_t *session, space_t *space, uint32 *hwms)
{
    ...
    for (id = 0; id < space->ctrl->file_hwm; id++) {
        ...
        GS_LOG_RUN_INF("update hwm of file %u from %u to %u", space->ctrl->files[id],
            SPACE_HEAD_RESIDENT(space)->hwms[id], hwms[id]);
    }
}
...
static status_t spc_shrink_files(knl_session_t *session, space_t *space, knl_shrink_def_t *shrink)
{
    ...
    for (uint32 i = 0; i < space->ctrl->file_hwm; i++) {
        ...
        log_atomic_op_begin(session);
        if (spc_truncate_datafile(session, df, DATAFILE_FD(space->ctrl->files[i]), df_keep_size, GS_TRUE) !=
            GS_SUCCESS) {
            log_atomic_op_end(session);
            cm_spin_unlock(&space->lock);
            return GS_ERROR;
...
static void spc_try_remove_invalid_extent(knl_session_t *session, space_t *space, uint32 *hwms, page_id_t *prev_ext,
    page_id_t *curr_ext)
{
    page_id_t next;
...
static status_t spc_shrink_hwms_prepare(knl_session_t *session, space_t *space, uint32 *new_hwms, uint32 *start_hwms,
    uint64 *prev_extents)
{
    prev_extents[0] = 0;
    for (uint32 i = 0; i < space->ctrl->file_hwm; i++) {
...
static status_t spc_shrink_space_prepare(knl_session_t *session, space_t *space)
{
    ...
    if (space->ctrl->id == DB_CORE_CTRL(session)->undo_space) {
        GS_LOG_RUN_WAR("undo_shrink_segments(session) is calling");
...
static inline void spc_print_punch_log(knl_session_t *session, space_t *space, const char *info)
{
    ...
    GS_LOG_DEBUG_INF("[SPC PUNCH] %s: free extents count %u first %u-%u last %u-%u, punching extents count %u "
        "first %u-%u last %u-%u, punched extents count %u first %u-%u last %u-%u.",
        info, free->count, (uint32)free->first.file, free->first.page, (uint32)free->last.file, free->last.page,
    ...
    for (;;) {
        if (SPACE_HEAD_RESIDENT(space)->free_extents.count > 0) {
            ...
            if (extent->page >= space->head->hwms[DATAFILE_GET(extent->file)->file_no]) {
                GS_LOG_RUN_INF("ignore invalid extent(%u-%d), space %s, file no %u", extent->file, extent->page,
                    space->ctrl->name, DATAFILE_GET(extent->file)->file_no);
                continue;
            }
            extent->aligned = 0;
            cm_spin_unlock(&space->lock);
    ...
    for (;;) {
        if (spc_alloc_datafile_map_extent(session, space, extent_size, extent, is_compress) == GS_SUCCESS) {
            ...
            knl_panic_log(!IS_INVALID_PAGID(*extent),
                "alloce bitmap extent (%u-%u) assert, "
                "datafile id is out of range.",
                extent->file, extent->page);
            // page 0 is datafile head, allow can not be 0. verified when alloc success. same as below
...
status_t spc_try_alloc_extent(knl_session_t *session, space_t *space, page_id_t *extent, uint32 *extent_size,
    bool32 *is_degrade, bool32 is_compress)
{
...
status_t spc_free_extent_from_list(knl_session_t *session, space_t *space, const char *oper)
{
    ...
    if (!cm_spin_try_lock(&space->lock)) {
        if (oper != NULL) {
            GS_THROW_ERROR_EX(ERR_OPERATIONS_NOT_ALLOW, "%s when space %s is being locked", oper, space->ctrl->name);
        }
        log_atomic_op_end(session);
        return GS_ERROR;
    }
...
void spc_drop_segment(knl_session_t *session, space_t *space)
{
    ...
    knl_panic_log(space->head->segment_count > 0,
        "segment_count abnormal, panic info: page %u-%u type %u segment_count %u", space->entry.file, space->entry.page,
        ((page_head_t *)CURR_PAGE)->type, space->head->segment_count);
    space->head->segment_count--;
    if (need_redo) {
        log_put(session, RD_SPC_CHANGE_SEGMENT, &space->head->segment_count, sizeof(uint32), LOG_ENTRY_FLAG_NONE);
    }
...
static inline void spc_alloc_datafile_temp_extent(knl_session_t *session, space_t *space, uint32 id, page_id_t *extent,
    uint32 extent_size)
{
...
{
    knl_panic_log(IS_SWAP_SPACE(space), "[SPACE] space %u is not swap space, type is %u.", space->ctrl->id,
        space->ctrl->type);
    CM_POINTER3(session, space, extent);
...
status_t spc_alloc_swap_extent_map(knl_session_t *session, space_t *space, page_id_t *extent)
{
    ...
    if (spc_alloc_swap_map_extent(session, space, space->ctrl->extent_size, extent) == GS_SUCCESS) {
        knl_panic_log(!IS_INVALID_PAGID(*extent), "alloc bitmap extent (%u-%u) error, page id is invalid.",
            extent->file, extent->page);
        cm_spin_unlock(&space->lock);
        return GS_SUCCESS;
    }
    ...
    if (spc_extend_swap_datafile_map(session, space, space->ctrl->extent_size, extent) != GS_SUCCESS) {
        cm_spin_unlock(&space->lock);
        GS_LOG_RUN_ERR("[SPACE] space %u extend datafile failed, extend size is %u.", space->ctrl->id,
            space->ctrl->extent_size);
        return GS_ERROR;
    }
...
void spc_free_temp_extent_map(knl_session_t *session, space_t *space, page_id_t extent)
{
    knl_panic_log(IS_SWAP_SPACE(space), "[SPACE] space %u is not swap space, type is %u.", space->ctrl->id,
        space->ctrl->type);
    CM_POINTER2(session, space);
    knl_panic_log(!IS_INVALID_PAGID(extent), "alloc bitmap extent (%u-%u) error, page id is invalid.", extent.file,
        extent.page);
...
page_id_t spc_get_next_temp_ext(knl_session_t *session, page_id_t extent)
{
    ...
    knl_panic_log(!IS_INVALID_PAGID(extent), "get next temp extent error, page id %u-%u.", extent.file, extent.page);
...
status_t spc_rebuild_space(knl_session_t *session, space_t *space)
{
    ...
    if (cm_write_device(df->ctrl->type, session->datafiles[df->ctrl->id], DEFAULT_PAGE_SIZE, space_buf,
        DEFAULT_PAGE_SIZE)) {
        cm_pop(session->stack);
        return GS_ERROR;
    }
...
status_t space_head_dump(knl_session_t *session, page_head_t *page_head, cm_dump_t *dump)
{
    ...
    cm_dump(dump, "\tfree_extents: count %u \tfirst %u-%u \tlast %u-%u\n", space_head->free_extents.count,
        space_head->free_extents.first.file, space_head->free_extents.first.page, space_head->free_extents.last.file,
        space_head->free_extents.last.page);
...
void spc_punch_residual_extents(knl_session_t *session, uint32 extent_size, page_list_t *punch_exts)
{
    ...
    for (uint32 i = 0; i < punch_exts->count; i++) {
        ...
        if (!spc_punch_normal_verify_extent(&page_id)) {
            GS_LOG_RUN_WAR("punch extent(%u-%u) is invailed, extent list first is %u-%u.", page_id.file, page_id.page,
                punch_exts->first.file, punch_exts->first.page);
...
static inline void spc_punch_extents(knl_session_t *session, uint32 extent_size, page_id_t *ext_firsts, uint32 ext_num)
{
...
status_t spc_punch_fetch_bitmap_group(knl_session_t *session, space_t *space, datafile_t *df, uint32 hwm,
...
void spc_clean_punching_extents(knl_session_t *session, space_t *space)
{
    ...
    spc_init_page_list(&punch_head->punching_exts);
    log_put(session, RD_SPC_PUNCH_EXTENTS, &punch_head->punching_exts, sizeof(rd_punch_extents_t), LOG_ENTRY_FLAG_NONE);
