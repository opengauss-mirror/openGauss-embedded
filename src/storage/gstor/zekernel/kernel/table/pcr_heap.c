...
status_t pcrh_fetch(knl_handle_t handle, knl_cursor_t *cursor)
{
    ...
    for (;;) {
        ...
        if (!IS_SAME_PAGID_BY_ROWID(row_id, cursor->rowid)) {
            ...
            if (cursor->cleanout && session->kernel->ckpt_ctx.queue.count <= session->kernel->attr.ckpt_interval * 3) {
...
status_t pcrh_fetch_page(knl_handle_t handle , knl_cursor_t *cursor,int64_t* row_count) {
    knl_session_t *session = (knl_session_t *)handle;
    ...
    // 设置页缓存
    cursor->page_cache = LOCAL_PAGE_CACHE;
    heap_page_t* page = (heap_page_t*)cursor->page_buf;
