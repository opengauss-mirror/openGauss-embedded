...
static int32 part_compare_key_column(gs_type_t type, part_decode_key_t *key1, part_decode_key_t *key2, uint16 col)
{
    char *data1 = NULL;
    char *data2 = NULL;
    int32 result;
 ...
 */
int32 part_compare_border(knl_part_column_desc_t *desc, knl_part_key_t *locate_key, part_decode_key_t *part_key,
    bool32 is_left)
{
...
static bool32 part_compare_list_key(knl_part_column_desc_t *desc, part_decode_key_t *key1, part_decode_key_t *key2)
{
    char *data1 = NULL;
    char *data2 = NULL;
    uint32 i;
...
static uint32 part_locate_range_key(part_table_t *part_table, part_decode_key_t *decoder)
{
    ...
    if (PART_CONTAIN_INTERVAL(part_table)) {
        ...
        knl_panic_log(table_part->desc.groupcnt == 1,
            "table_part's groupcnt is abnormal, panic info: table_part %s groupcnt %u", table_part->desc.name,
            table_part->desc.groupcnt);
...
uint32 knl_locate_part_border(knl_handle_t session, knl_handle_t dc_entity, knl_part_key_t *locate_key, bool32 is_left)
{
    ...
    knl_panic_log(key->column_count == part_table->desc.partkeys,
...
static uint32 part_locate_hash_key(dc_entity_t *entity, part_table_t *part_table, knl_part_key_t *part_key)
{
    ...
    knl_panic_log(key->column_count < GS_MAX_COLUMNS,
        "the column_count is more than the max limit, panic info: table %s column_count %u", table->desc.name,
        key->column_count);
...
uint32 knl_locate_part_key(knl_handle_t dc_entity, part_key_t *key)
{
    ...
    knl_panic_log(key->column_count == part_table->desc.partkeys,
        "the column_count in key is not equal to "
...
uint32 knl_locate_subpart_key(knl_handle_t dc_entity, uint32 compart_no, part_key_t *key)
{
    ...
    knl_panic_log(key->column_count == part_table->desc.subpartkeys,
        "the column count of key is not equal to "
