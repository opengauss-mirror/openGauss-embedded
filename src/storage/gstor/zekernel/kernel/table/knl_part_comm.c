...
status_t db_switch_shadow_indexparts(knl_session_t *session, knl_cursor_t *cursor, index_t *index)
{
    ...
    knl_set_scan_key(INDEX_DESC(cursor->index), &cursor->scan_range.l_key, GS_TYPE_INTEGER, &idesc->uid, sizeof(uint32),
        IX_COL_SYS_SHW_INDEXPART001_USER_ID);
    ...
    knl_set_scan_key(INDEX_DESC(cursor->index), &cursor->scan_range.r_key, GS_TYPE_INTEGER, &idesc->uid, sizeof(uint32),
        IX_COL_SYS_SHW_INDEXPART001_USER_ID);
    ...
    if (db_delete_from_shadow_sysindexpart(session, cursor, index->desc.uid, index->desc.table_id, index->desc.id) !=
        GS_SUCCESS) {
        return GS_ERROR;
    }
...
uint32 part_generate_part_id(table_t *table, uint32 num)
{
    ...
    if (table->part_table != NULL) {
        if (table->part_table->desc.partcnt==0){
            num = 0;
        }else{
...
status_t db_update_part_flag(knl_session_t *session, knl_dictionary_t *dc, part_table_t *part_table, uint32 pid,
    part_flag_type_e part_flag)
{
    uint16 size;
    row_assist_t ra;
    ...
    knl_set_scan_key(INDEX_DESC(cursor->index), key, GS_TYPE_INTEGER, &part_table->desc.uid, sizeof(uint32),
        IX_COL_SYS_TABLEPART001_USER_ID);
    knl_set_scan_key(INDEX_DESC(cursor->index), key, GS_TYPE_INTEGER, &part_table->desc.table_id, sizeof(uint32),
        IX_COL_SYS_TABLEPART001_TABLE_ID);
    knl_set_scan_key(INDEX_DESC(cursor->index), key, GS_TYPE_INTEGER, &pid, sizeof(uint32),
...
status_t db_update_table_part_initrans(knl_session_t *session, knl_table_part_desc_t *desc, uint32 initrans)
{
    ...
    knl_set_scan_key(INDEX_DESC(cursor->index), key, GS_TYPE_INTEGER, (void *)&desc->uid, sizeof(uint32),
        IX_COL_SYS_TABLEPART001_USER_ID);
    knl_set_scan_key(INDEX_DESC(cursor->index), key, GS_TYPE_INTEGER, (void *)&desc->table_id, sizeof(uint32),
        IX_COL_SYS_TABLEPART001_TABLE_ID);
    knl_set_scan_key(INDEX_DESC(cursor->index), key, GS_TYPE_INTEGER, (void *)&desc->part_id, sizeof(uint32),
        IX_COL_SYS_TABLEPART001_PART_ID);
...
status_t db_update_table_subpart_initrans(knl_session_t *session, knl_table_part_desc_t *desc, uint32 initrans)
{
    ...
    knl_set_scan_key(INDEX_DESC(cursor->index), key, GS_TYPE_INTEGER, (void *)&desc->parent_partid, sizeof(uint32),
...
status_t db_update_part_count(knl_session_t *session, uint32 uid, uint32 tid, uint32 iid, bool32 is_add)
{
    ...
    knl_set_scan_key(INDEX_DESC(cursor->index), key, GS_TYPE_INTEGER, &uid, sizeof(uint32),
        IX_COL_SYS_PARTOBJECT001_USER_ID);
    knl_set_scan_key(INDEX_DESC(cursor->index), key, GS_TYPE_INTEGER, &tid, sizeof(uint32),
        IX_COL_SYS_PARTOBJECT001_TABLE_ID);
    knl_set_scan_key(INDEX_DESC(cursor->index), key, GS_TYPE_INTEGER, &iid, sizeof(uint32),
    ...
    GS_LOG_DEBUG_INF("update count: partcnt after update is: %d, the operation is(1:add, 0:drop): %d", partcnt, is_add);
    ...
    bool32 is_add)
{
    ...
    if (is_add) {
        ...
        if (part_no == part_table->desc.partcnt - 1) {
            for (uint32 i = part_no; i >= part_table->desc.transition_no; i--) {
                entity = PART_GET_ENTITY(part_table, i);
                if (entity != NULL && entity->is_ready) {
                    partcnt = entity->part_no;
                    break;
                }
            ...
            knl_panic_log(part_no < part_table->desc.transition_no,
                ...
                part_no, part_table->desc.transition_no);
    ...
    GS_LOG_DEBUG_INF("update interval count: uid: %d, tid: %d, iid: %d, partno: %d", table->desc.uid, table->desc.id,
        iid, part_no);
    GS_LOG_DEBUG_INF("update interval count: partcnt after update is: %d, the operation is(1:add, 0:drop): %d", partcnt,
        is_add);
    ...
    uint32 compart_no, bool32 is_left)
{
    ...
    knl_panic_log(key->column_count == part_table->desc.subpartkeys,
        "the column_count is not equal to part_table's "
        "subpartkeys, panic info: table %s column_count %u subpartkeys %u",
        table->desc.name, key->column_count, part_table->desc.subpartkeys);
...
status_t db_update_subtabpart_count(knl_session_t *session, uint32 uid, uint32 tid, uint32 compart_id, bool32 is_add)
{
    ...
    knl_set_scan_key(INDEX_DESC(cursor->index), &cursor->scan_range.l_key, GS_TYPE_INTEGER, &tid, sizeof(uint32),
        IX_COL_SYS_TABLEPART001_TABLE_ID);
    knl_set_scan_key(INDEX_DESC(cursor->index), &cursor->scan_range.l_key, GS_TYPE_INTEGER, &compart_id, sizeof(uint32),
        IX_COL_SYS_TABLEPART001_PART_ID);
    ...
    GS_LOG_DEBUG_INF("update count: subpartcnt after update is: %d, the operation is(1:add, 0:drop): %d", subpart_cnt,
        is_add);
...
status_t db_update_subpart_flag(knl_session_t *session, knl_dictionary_t *dc, table_part_t *compart, uint32 subpart_id,
    part_flag_type_e part_flag)
{
    row_assist_t ra;
    ...
    knl_set_scan_key(INDEX_DESC(cursor->index), &cursor->scan_range.l_key, GS_TYPE_INTEGER, &compart->desc.part_id,
        sizeof(uint32), IX_COL_SYS_TABLESUBPART001_PARENT_PART_ID);
    knl_set_scan_key(INDEX_DESC(cursor->index), &cursor->scan_range.l_key, GS_TYPE_INTEGER, &subpart_id, sizeof(uint32),
        IX_COL_SYS_TABLESUBPART001_SUB_PART_ID);
    ...
    uint32 subpart_no, bool32 reuse_storage)
{
    ...
    for (uint32 i = 0; i < entity->column_count; i++) {
        ...
        if (lob_part_segment_prepare(session, (lob_part_t *)lob_subpart, reuse_storage, LOB_TRUNCATE_PART_SEGMENT) !=
            GS_SUCCESS) {
            return GS_ERROR;
        }
    }
    ...
    uint32 compart_no, bool32 reuse_storage)
{
    ...
    if (heap_part_segment_prepare(session, (table_part_t *)table_subpart, reuse_storage, HEAP_TRUNCATE_PART_SEGMENT) !=
        GS_SUCCESS) {
        return GS_ERROR;
    }
    ...
    uint32 index_id, uint32 new_partid)
{
    ...
    knl_set_scan_key(INDEX_DESC(cursor->index), key, GS_TYPE_INTEGER, &index_id, sizeof(uint32),
    ...
    uint32 column_id, uint32 new_partid)
{
    ...
    knl_scan_key_t *key = &cursor->scan_range.l_key;
    knl_set_scan_key(INDEX_DESC(cursor->index), key, GS_TYPE_INTEGER, &table_part->desc.uid, sizeof(uint32),
        IX_COL_SYS_LOBPART001_USER_ID);
    knl_set_scan_key(INDEX_DESC(cursor->index), key, GS_TYPE_INTEGER, &table_part->desc.table_id, sizeof(uint32),
        IX_COL_SYS_LOBPART001_TABLE_ID);
    knl_set_scan_key(INDEX_DESC(cursor->index), key, GS_TYPE_INTEGER, &column_id, sizeof(uint32),
        IX_COL_SYS_LOBPART001_COLUMN_ID);
    knl_set_scan_key(INDEX_DESC(cursor->index), key, GS_TYPE_INTEGER, &table_part->desc.part_id, sizeof(uint32),
