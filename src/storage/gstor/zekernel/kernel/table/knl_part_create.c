...
static void part_init_table_desc(table_t *table, knl_part_obj_def_t *def, knl_part_desc_t *desc)
{
    ...
    desc->auto_addpart = def->auto_addpart;
    desc->is_crosspart = def->is_crosspart;
}
...
status_t db_write_sys_partobject(knl_session_t *session, knl_cursor_t *cursor, knl_part_desc_t *desc)
{
    ...
    (void)row_put_int32(&ra, desc->auto_addpart);
    (void)row_put_int32(&ra, desc->is_crosspart);
...
status_t part_init_table_part_desc(knl_session_t *session, table_t *table, knl_part_def_t *def, uint32 part_id,
    knl_table_part_desc_t *desc, bool32 not_ready)
{
    desc->uid = table->desc.uid;
    desc->table_id = table->desc.id;
    desc->part_id = part_id;
    desc->subpart_cnt = 0;
    (void)cm_text2str(&def->name, desc->name, GS_TS_PART_NAME_BUFFER_SIZE);
    ...
    if (def->storage_def.initial > 0) {
        ...
        desc->storage_desc.initial =
            CM_CALC_ALIGN((uint64)def->storage_def.initial, space->ctrl->block_size) / space->ctrl->block_size;
    }
    ...
    if (def->storage_def.maxsize > 0 && !dc_is_reserved_entry(table->desc.uid, table->desc.id)) {
        desc->storaged = GS_TRUE;
        desc->storage_desc.max_pages =
            CM_CALC_ALIGN((uint64)def->storage_def.maxsize, space->ctrl->block_size) / space->ctrl->block_size;
    }
...
static void part_init_index_desc(index_t *index, knl_part_desc_t *part_desc, knl_part_desc_t *desc)
{
    ...
    desc->auto_addpart = part_desc->auto_addpart;
    desc->is_crosspart = part_desc->is_crosspart;
}
...
status_t part_write_shadowindex_part(knl_session_t *session, knl_dictionary_t *dc, index_part_t *index_part,
    bool32 create_segment)
{
...
static status_t part_create_shadowindex_default(knl_session_t *session, knl_dictionary_t *dc, index_t *index,
    rebuild_info_t rebuild_info)
{
    ...
    for (uint32 i = 0; i < part_table->desc.partcnt; i++) {
        ...
        if (IS_PARENT_TABPART(&table_part->desc)) {
            if (subpart_create_shadowind_default(session, dc, index, table_part->part_no, rebuild_info) != GS_SUCCESS) {
                return GS_ERROR;
            }
        }
    ...
    knl_part_def_t *def, uint32 compart_no, bool32 is_alter)
{
    ...
    for (uint32 i = 0; i < def->subparts.count; i++) {
        ...
        if (part_init_index_part_desc(session, index, part_def, &table_subpart->desc, &index_subpart.desc) !=
            GS_SUCCESS) {
            return GS_ERROR;
        }
...
static status_t part_create_shadowindex_with_def(knl_session_t *session, knl_dictionary_t *dc, knl_part_obj_def_t *def,
    index_t *index, bool32 is_alter)
{
    index_part_t index_part;
    table_part_t *table_part = NULL;
    knl_part_def_t *part_def = NULL;
    ...
    for (uint32 i = 0; i < def->parts.count; i++) {
        ...
        if (IS_PARENT_TABPART(&table_part->desc)) {
            if (subpart_create_shwidx_with_def(session, dc, index, part_def, table_part->part_no, is_alter) !=
                GS_SUCCESS) {
                return GS_ERROR;
            }
        }
...
status_t db_update_sub_idxpart_status(knl_session_t *session, index_part_t *index_part, bool32 is_invalid,
    bool32 *is_changed)
{
    row_assist_t ra;
    knl_index_part_desc_t desc;
...
status_t db_update_idxpart_status(knl_session_t *session, index_part_t *index_part, bool32 is_invalid,
    bool32 *is_changed)
{
    row_assist_t ra;
    knl_index_part_desc_t desc;
...
status_t db_create_part_index(knl_session_t *session, knl_cursor_t *cursor, index_t *index, knl_part_obj_def_t *def)
{
    knl_part_desc_t object_desc;
    dc_entity_t *entity = index->entity;
    part_table_t *part_table = entity->table.part_table;
...
status_t part_lob_get_space_id(knl_session_t *session, lob_t *lob, knl_part_def_t *def, uint32 *space_id)
{
...
status_t subpart_init_table_part_desc(knl_session_t *session, knl_table_part_desc_t *comdesc, knl_part_def_t *def,
    uint32 subpart_id, knl_table_part_desc_t *desc)
{
