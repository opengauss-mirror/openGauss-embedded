...
static void temp_heap_revert_update(knl_session_t *session, undo_row_t *undo_info, row_head_t *row, uint16 *offsets,
    uint16 *lens);
...
void temp_mtrl_init_context(knl_session_t *session)
{
    ...
    g_knl_callback.init_vmc(session, (handle_t)ctx);
...
static void temp_heap_validate_page(knl_session_t *session, vm_page_t *vm_page)
{
    ...
    knl_panic_log(page->free_begin <= page->free_end,
        ...
        AS_PAGID(page->head.id).file, AS_PAGID(page->head.id).page, page->head.type, page->free_begin, page->free_end);
    ...
    for (uint16 i = 0; i < page->dirs; i++) {
        ...
        knl_panic_log(dir->offset >= sizeof(temp_heap_page_t),
            "dir's offset is abnormal, panic info: page %u-%u type %u dir offset %u", AS_PAGID(page->head.id).file,
            AS_PAGID(page->head.id).page, page->head.type, dir->offset);
        knl_panic_log(dir->offset < page->free_begin,
            ...
            AS_PAGID(page->head.id).file, AS_PAGID(page->head.id).page, page->head.type, page->free_begin, dir->offset);
...
static inline void temp_heap_init_row(knl_session_t *session, row_assist_t *ra, char *buf, uint32 column_count,
    uint16 flags)
{
    row_init(ra, buf, KNL_MAX_ROW_SIZE, column_count);
...
void temp_heap_insert_into_page(knl_cursor_t *cursor, temp_heap_page_t *page, row_head_t *row, undo_data_t *undo,
...
status_t temp_heap_fetch(knl_handle_t handle, knl_cursor_t *cursor)
{
    ...
    if (temp_table->table_type == DICT_TYPE_TEMP_TABLE_TRANS && temp_table->hold_rmid != GS_INVALID_ID32 &&
...
static void temp_batch_insert_rows(knl_session_t *session, knl_cursor_t *cursor, undo_data_t *undo, uint16 *rows_size)
{
    ...
    for (uint32 i = 0; i < row_count; i++) {
        ...
        if (row->size > TEMP_GS_MAX_ROW_SIZE || page->free_end < page->free_begin + max_cost_size + pct_free_size) {
            break;
        }
    }
...
static status_t temp_simple_insert(knl_session_t *session, knl_cursor_t *cursor)
{
    ...
    knl_panic_log(session->temp_mtrl->segments[CURSOR_TEMP_CACHE(cursor)->table_segid]->vm_list.count > 0,
        "vm page count is abnormal, panic info: page %u-%u type %u table %s vm_list's count %u", cursor->rowid.file,
    ...
    uint32 vm_slot)
{
    ...
    knl_panic_log(IS_SAME_PAGID(dir->undo_page, AS_PAGID(ud_page->head.id)),
        "dir's undo_page and ud_page are not same, panic info: page %u-%u type %u", AS_PAGID(page->head.id).file,
...
void temp_heap_undo_batch_insert(knl_session_t *session, undo_row_t *ud_row, undo_page_t *ud_page, int32 ud_slot)
{
    ...
    knl_temp_cache_t *knl_temp_cache =
        knl_get_temp_cache(session, batch_undo->obj_info.uid, batch_undo->obj_info.table_id);
...
void temp_heap_undo_delete(knl_session_t *session, undo_row_t *ud_row, undo_page_t *ud_page, int32 ud_slot)
{
    ...
    knl_panic_log(IS_SAME_PAGID(dir->undo_page, AS_PAGID(ud_page->head.id)),
        ...
        AS_PAGID(ud_page->head.id).page, ud_page->head.type);
    knl_panic_log(dir->undo_slot == ud_slot,
    ...
    undo_data_t *undo)
{
    ...
    knl_panic_log(migr_page->free_end - migr_page->free_begin >= cost_size,
        ...
        AS_PAGID(migr_page->head.id).file, AS_PAGID(migr_page->head.id).page, migr_page->head.type, migr_page->free_end,
        migr_page->free_begin, cost_size);
    ...
    knl_panic_log(owner_page->head.type == PAGE_TYPE_TEMP_HEAP,
        "owner_page type is abnormal, panic info: owner_page %u-%u type %u", AS_PAGID(owner_page->head.id).file,
        AS_PAGID(owner_page->head.id).page, owner_page->head.type);
    ...
    undo_data_t *undo)
{
    ...
    knl_panic_log(undo->size <= TEMP_GS_MAX_ROW_SIZE,
    ...
    uint16 *lens)
{
    ...
    knl_panic_log(new_ra.head->size <= row_size,
        "the new_ra's size is more than row_size, panic info: new_ra size %u row_size %u", new_ra.head->size, row_size);
...
void temp_heap_undo_update(knl_session_t *session, undo_row_t *ud_row, undo_page_t *ud_page, int32 ud_slot)
{
    ...
    dir = temp_heap_get_dir(page, (uint32)rid.vm_slot);
    knl_panic_log(IS_SAME_PAGID(dir->undo_page, AS_PAGID(ud_page->head.id)),
        "dir's undo_page and ud_page are not same, panic info: page %u-%u type %u", AS_PAGID(ud_page->head.id).file,
        AS_PAGID(ud_page->head.id).page, ud_page->head.type);
    knl_panic_log(dir->undo_slot == ud_slot,
    ...
    heap_update_assist_t *ua, temp_heap_page_t *page, uint16 slot)
{
    ...
    if (ua->inc_size > 0) {
        ...
        knl_panic_log(dir->offset < page->free_begin,
            ...
            AS_PAGID(page->head.id).file, AS_PAGID(page->head.id).page, page->head.type, dir->offset, page->free_begin);
        knl_panic_log(page->free_begin <= page->free_end,
