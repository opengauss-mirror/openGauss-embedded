...
table_accessor_t g_heap_acsor = {
    ...
    (knl_cursor_operator2_t)heap_fetch_page,
};
...
table_accessor_t g_pcr_heap_acsor = {
    ...
    (knl_cursor_operator2_t)pcrh_fetch_page,
};
...
static status_t db_init_table_desc_by_def(knl_session_t *session, knl_table_desc_t *desc, knl_table_def_t *def)
{
    ...
    if (desc->compress_algo > COMPRESS_NONE) {
        ...
        if (session->kernel->attr.default_compress_algo > COMPRESS_NONE) {
            if ((def->type == TABLE_TYPE_HEAP  && IS_SPACE_COMPRESSIBLE(space) && (def->part_def == NULL || !def->part_def->is_composite)))
            {
...
#endif
    desc->is_timescale = def->is_timescale;
    desc->has_retention = def->has_retention;
    if (desc->has_retention){
        text_t re;
        re.str = def->retention;
        re.len = strlen(def->retention);
        (void)cm_text2str(&re, desc->retention_str, GS_MAX_RETENTION_LEN-1);
    }
...
status_t db_init_table_desc(knl_session_t *session, knl_table_desc_t *desc, knl_table_def_t *def)
{
    ...
    if (def->columns.count > session->kernel->attr.max_column_count) {
        GS_THROW_ERROR(ERR_MAX_COLUMN_SIZE, session->kernel->attr.max_column_count);
        return GS_ERROR;
    }
...
static status_t db_create_cons(knl_session_t *session, knl_dictionary_t *dc, knl_constraint_def_t *def)
{
    ...
    if (def->name.str == NULL) {
        def->name.str = cm_push(session->stack, GS_NAME_BUFFER_SIZE);
        if (def->name.str == NULL) {
            return GS_ERROR;
        }
        PRTS_RETURN_IFERR(snprintf_s(def->name.str, GS_NAME_BUFFER_SIZE, GS_NAME_BUFFER_SIZE - 1,
            "%s_%u_%lld", table->desc.name, table->desc.id, cm_now()));
        def->name.len = (uint32)strlen(def->name.str);
    }
...
static status_t db_create_constraints(knl_session_t *session, knl_table_desc_t *desc, knl_table_def_t *def)
{
    ...
    for (i = 0; i < def->constraints.count; i++) {
        ...
        if (cons_def->name.str == NULL) {
            cons_def->name.str = cm_push(session->stack, GS_NAME_BUFFER_SIZE);
            if (cons_def->name.str == NULL) {
                return GS_ERROR;
            }
            PRTS_RETURN_IFERR(snprintf_s(cons_def->name.str, GS_NAME_BUFFER_SIZE, GS_NAME_BUFFER_SIZE - 1,
                "%s_%u_%u", desc->name, desc->id, i));
        ...
        // 2024-11-23   ENABLE FK
        bool32 IS_DISABLE_FK = GS_FALSE;
        if (IS_DISABLE_FK) {
...
status_t db_create_table(knl_session_t *session, knl_table_def_t *def, table_t *table)
{
    ...
    if (def->comment) {
        ...
        text_t COMMENT;
        COMMENT.str = def->comment;
        COMMENT.len = strlen(def->comment);
...
status_t db_drop_view(knl_session_t *session, knl_dictionary_t *dc)
{
    ...
    if (g_knl_callback.update_depender && g_knl_callback.update_depender(session, &obj_addr) != GS_SUCCESS) {
        CM_RESTORE_STACK(session->stack);
        return GS_ERROR;
    }

    if (g_knl_callback.pl_db_drop_triggers && g_knl_callback.pl_db_drop_triggers(session, (void *)dc) != GS_SUCCESS) {
        CM_RESTORE_STACK(session->stack);
        return GS_ERROR;
    }
    ...
    knl_commit(session);
    if (g_knl_callback.pl_drop_triggers_entry) {
                                     ...
                                     knl_cursor_t *cursor, knl_column_t *column)
{
    ...
    switch (column->datatype) {
        ...
        case GS_TYPE_UINT64:
            return row_put_int64(ra, VALUE(uint64, &value));
        case GS_TYPE_FLOAT:
        case GS_TYPE_REAL:
...
static bool32 check_varchar_column_size_changable(knl_session_t *session, knl_column_t *old_column,
                                                  knl_column_t *new_column,knl_dictionary_t *dc,uint64_t *row_num) {
    knl_cursor_t *cursor = NULL;
    ...
    // 遍历表，检查该列的长度是否都小于新的长度 
    while(!cursor->eof) {
        (*row_num)++;
        uint16_t len = CURSOR_COLUMN_SIZE(cursor, old_column->id);
        if(len != GS_NULL_VALUE_LEN && len > new_column->size) {
            knl_close_cursor(session, cursor);
            CM_RESTORE_STACK(session->stack);
            return GS_FALSE;
        }
                                         ...
                                         bool32 *rebuild_indexes, uint32 index_count)
{
    ...
    if (db_modify_column_null_only(old_column, new_column) || compatible) {
        ...
        if (!col_null) {
            // check if type is varchar length change 
            if(GS_IS_STRING_TYPE2(old_column->datatype, new_column->datatype) && old_column->size > new_column->size) {
                uint64_t row_num = 0;
                if(check_varchar_column_size_changable(session, old_column, new_column,dc,&row_num) != GS_TRUE) {
                    GS_THROW_ERROR(ERR_COLUMN_DATA_TRUNCATED, old_column->name, row_num);
                    return GS_ERROR;
                }
            } else {
...
// 找到与列相关的索引
static status_t db_find_related_index(knl_session_t *session, knl_dictionary_t *dc, knl_column_t *column, index_t **del_indexs)
{
    table_t *table = NULL;
    index_t *index = NULL;
    uint32 i , j;
    knl_icol_info_t *icol = NULL;
    bool32 related_index = GS_FALSE;
    uint32 del_index_idx = 0;
    table = DC_TABLE(dc);

    for (i = 0; i < table->index_set.total_count; i++) {
        related_index = GS_FALSE;
        index = table->index_set.items[i];
        for (j = 0; j < index->desc.column_count; j++) {
            if (index->desc.columns[j] < DC_VIRTUAL_COL_START) {
                if (column->id == (uint32)index->desc.columns[j]) {
                    related_index = GS_TRUE;
                }
            } else {
        ...
        del_indexs[del_index_idx++] = index;
    }
    return GS_SUCCESS;
}

static status_t db_drop_indexs(knl_session_t *session , knl_dictionary_t* dc, index_t **del_indexs, uint32 del_index_count)
{
    index_t *index = NULL;
    uint32 i;
    for (i = 0; i < del_index_count; i++) {
        index = del_indexs[i];
        knl_set_session_scn(session, GS_INVALID_ID64);
        if (db_drop_index(session, index, dc) != GS_SUCCESS) {
            return GS_ERROR;
...
status_t db_altable_drop_column(knl_session_t *session, knl_dictionary_t *dc, knl_altable_def_t *def)
{
    ...
    index_t* del_indexs[GS_MAX_TABLE_INDEXES] = {NULL};
    // 找到与列相关的索引
    if(db_find_related_index(session, dc, column, del_indexs) != GS_SUCCESS) {
        return GS_ERROR;
    }
    
    if(del_indexs[0] != NULL) {
        if(del_indexs[0]->desc.primary) {
            GS_THROW_ERROR(ERR_INVALID_OPERATION, ",can not drop the PRIMARY key column");
        } else if(del_indexs[0]->desc.unique) {
            GS_THROW_ERROR(ERR_INVALID_OPERATION, ",can not drop the column with UNIQUE constraint");
        } else {
            GS_THROW_ERROR(ERR_INVALID_OPERATION, ",column not dropable , an index depend on a column after it");
        }
        return GS_ERROR;
    }
...
bool32 db_user_has_objects(knl_session_t *session, uint32 uid, text_t *uname)
{
    ...
    do {
        ...
        bool32 HAS_SYS_PROC_ID = GS_FALSE;  // 2024-8-6
        if (HAS_SYS_PROC_ID) {
        ...
        bool32 HAS_SYS_LIBRARY_ID = GS_FALSE;   // 2024-8-6
        if (HAS_SYS_LIBRARY_ID) {
        ...
        bool32 HAS_SYS_SQL_MAP_ID = GS_FALSE;   // 2024-8-6
        if (HAS_SYS_SQL_MAP_ID) {
        ...
        bool32 HAS_SYS_DISTRIBUTE_RULE_ID = GS_FALSE;   // 2024-8-6
        if (HAS_SYS_DISTRIBUTE_RULE_ID) {
...
status_t db_drop_table_cascade(knl_session_t *session, text_t *user, knl_table_desc_t *desc)
{
    ...
    bool32 HAS_SYS_LOGIC_REP = GS_FALSE;    // 2024-9-3
    if (HAS_SYS_LOGIC_REP) {
...
static status_t db_match_ptrans(void *handle, bool32 *match)
{
    ...
    *match = GS_FALSE;
    ...
    if (cond->only_remained) {
        ...
        if (g_knl_callback.get_xa_xid(cc_instance, &ret_xa_xid) != GS_INVALID_ID16) {
            return GS_SUCCESS;
        }
...
status_t db_create_synonym(knl_session_t *session, knl_synonym_def_t *def)
{
    ...
    knl_securec_check(err);
    bool32 HAS_SYS_DEPENDENCY_ID = GS_FALSE;    // 2024-11-12
    if (HAS_SYS_DEPENDENCY_ID) {
