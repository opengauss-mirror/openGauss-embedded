...
{
    lob_chunk_t *chunk = (lob_chunk_t *)log->data;
    printf("insert_xid: xmap %u-%u, xnum %u, ", (uint32)chunk->ins_xid.xmap.seg_id, (uint32)chunk->ins_xid.xmap.slot,
        chunk->ins_xid.xnum);
    printf("delete_xid: xmap %u-%u, xnum %u, ", (uint32)chunk->del_xid.xmap.seg_id, (uint32)chunk->del_xid.xmap.slot,
        chunk->del_xid.xnum);
...
{
    page_head_t *head = (page_head_t *)log->data;
    printf("next_ext %u-%u\n", (uint32)AS_PAGID_PTR(head->next_ext)->file, (uint32)AS_PAGID_PTR(head->next_ext)->page);
}
...
void print_lob_change_seg(log_entry_t *log)
{
    ...
    printf("extents(count %u, first %u-%u, last %u-%u), free_list(count %u, first %u-%u, last %u-%u), ",
        ...
        (uint32)seg->extents.last.file, (uint32)seg->extents.last.page, seg->free_list.count,
        (uint32)seg->free_list.first.file, (uint32)seg->free_list.first.page, (uint32)seg->free_list.last.file,
        (uint32)seg->free_list.last.page);
    printf("ufp_count %u, ufp_first %u-%u, ufp_extent %u-%u\n", seg->ufp_count, (uint32)seg->ufp_first.file,
