...
static void rb_init_table_desc(knl_session_t *session, knl_table_desc_t *table_desc, knl_rb_desc_t *desc)
{
    ...
    ret = snprintf_s(desc->name, GS_NAME_BUFFER_SIZE, GS_NAME_BUFFER_SIZE - 1, "BIN$%u$%llX==$0", table_desc->id,
        desc->id);
    knl_securec_check_ss(ret);
 ...
 */
static void rb_init_table_part_desc(knl_session_t *session, table_t *table, knl_table_part_desc_t *part_desc,
    knl_rb_desc_t *desc)
{
    size_t name_len;
    errno_t ret;
    ...
    desc->id = session->curr_lsn;
    ret = snprintf_s(desc->name, GS_NAME_BUFFER_SIZE, GS_NAME_BUFFER_SIZE - 1, "BIN$%u$%llX==$0", part_desc->table_id,
        desc->id);
    knl_securec_check_ss(ret);
...
static void rb_init_index_desc(knl_session_t *session, knl_index_desc_t *index_desc, knl_rb_desc_t *desc)
{
    errno_t ret;
    desc->id = session->curr_lsn;
    ret = snprintf_s(desc->name, GS_NAME_BUFFER_SIZE, GS_NAME_BUFFER_SIZE - 1, "BIN$%u$%llX==$0", index_desc->table_id,
        desc->id);
    knl_securec_check_ss(ret);
...
{
    desc->id = session->curr_lsn;
    errno_t ret = snprintf_s(desc->name, GS_NAME_BUFFER_SIZE, GS_NAME_BUFFER_SIZE - 1, "BIN$%u$%llX==$0",
        part_desc->table_id, desc->id);
    knl_securec_check_ss(ret);
    ...
    if (IS_SUB_IDXPART(part_desc)) {
        ...
        knl_panic_log(index_compart != NULL, "the index_compart is NULL, panic info: rb_table %s index %s", desc->name,
            index->desc.name);
        ret = snprintf_s(desc->part_name, GS_NAME_BUFFER_SIZE, GS_NAME_BUFFER_SIZE - 1, "INDEX%uP%uSUBP%u",
            part_desc->index_id, index_compart->part_no, part_no);
    } else {
        ret = snprintf_s(desc->part_name, GS_NAME_BUFFER_SIZE, GS_NAME_BUFFER_SIZE - 1, "INDEX%uP%u",
            part_desc->index_id, part_no);
    }
    knl_securec_check_ss(ret);
...
static void rb_init_lob_desc(knl_session_t *session, knl_lob_desc_t *lob_desc, knl_rb_desc_t *desc)
{
    ...
    desc->id = session->curr_lsn;
    ret = snprintf_s(desc->name, GS_NAME_BUFFER_SIZE, GS_NAME_BUFFER_SIZE - 1, "BIN$%u$%llX==$0", lob_desc->table_id,
        desc->id);
    knl_securec_check_ss(ret);

    ret = snprintf_s(desc->org_name, GS_NAME_BUFFER_SIZE, GS_NAME_BUFFER_SIZE - 1, "LOB%uC%u", lob_desc->table_id,
        lob_desc->column_id);
    knl_securec_check_ss(ret);
    desc->part_name[0] = '\0';

    knl_panic_log(desc->oper == RB_OPER_TRUNCATE, "curr oper is abnormal, panic info: rb_table %s oper %u", desc->name,
        desc->oper);
    ...
    uint32 part_no, knl_rb_desc_t *desc)
{
    ...
    ret = snprintf_s(desc->org_name, GS_NAME_BUFFER_SIZE, GS_NAME_BUFFER_SIZE - 1, "LOB%uC%u", part_desc->table_id,
        part_desc->column_id);
    knl_securec_check_ss(ret);
    ...
    if (IS_SUB_LOBPART(part_desc)) {
        ...
        knl_panic_log(compart != NULL, "the compart is NULL, panic info: table %s rb_table %s", table->desc.name,
            desc->name);
        ret = snprintf_s(desc->part_name, GS_NAME_BUFFER_SIZE, GS_NAME_BUFFER_SIZE - 1, "LOB%uC%uP%uSUBP%u",
            part_desc->table_id, part_desc->column_id, compart->part_no, part_no);
    } else {
        ret = snprintf_s(desc->part_name, GS_NAME_BUFFER_SIZE, GS_NAME_BUFFER_SIZE - 1, "LOB%uC%uP%u",
            part_desc->table_id, part_desc->column_id, part_no);
    }
    knl_securec_check_ss(ret);
...
static status_t rb_drop_all_cons(knl_session_t *session, knl_dictionary_t *dc)
{
    ...
    while (!cursor->eof) {
        ...
        if (type != CONS_TYPE_REFERENCE) {
            ...
            knl_securec_check(ret);
            ret = snprintf_s(buf, name_buf_size, GS_NAME_BUFFER_SIZE - 1, "BIN$%u$%llX==$0", table->desc.id,
                session->curr_lsn);
            knl_securec_check_ss(ret);
...
#ifdef _STATISTICS
    if (GS_SUCCESS !=
        stats_drop_hists(session, table->desc.uid, table->desc.id, IS_NOLOGGING_BY_TABLE_TYPE(table->desc.type))) {
        CM_RESTORE_STACK(session->stack);
        return GS_ERROR;
    }
#endif
