...
#ifdef _CRYPTO
    res =
        EVP_DecryptUpdate(proc->encrypt_ctx.ctx, (unsigned char *)proc->encrypt_ctx.encrypt_buf.aligned_buf + left_size,
...
status_t bak_encrypt_data(bak_process_t *proc, const char *buf, int32 size)
{
    ...
    int32 res;
#ifdef _CRYPTO
    res = EVP_EncryptUpdate(proc->encrypt_ctx.ctx, (unsigned char *)proc->encrypt_ctx.encrypt_buf.aligned_buf, &outlen,
...
static status_t bak_update_record(knl_session_t *session, bak_record_t *record)
{
    ...
    knl_init_index_scan(cursor, GS_TRUE);
    knl_set_scan_key(INDEX_DESC(cursor->index), &cursor->scan_range.l_key, GS_TYPE_STRING, (void *)record->attr.tag,
...
status_t bak_init_uds(uds_link_t *link, const char *sun_path)
{
    ...
    socket_attr_t sock_attr = {
        .connect_timeout = GS_CONNECT_TIMEOUT,
        .l_onoff = 1,
        .l_linger = 1
    };
    ...
    for (uint32 i = 0; i * DEFAULT_PAGE_SIZE < size; i++) {
        ...
        if (!page_verify_checksum(page, DEFAULT_PAGE_SIZE)) {
            GS_LOG_RUN_ERR("[BACKUP] page corrupted(file %u, page %u). datafile page offset %llu, datafile name %s,"
                ...
                PAGE_SIZE(*page), PAGE_CHECKSUM(page, DEFAULT_PAGE_SIZE), size, knl_checksum_level(cks_level));
            GS_THROW_ERROR(ERR_CHECKSUM_FAILED, name);
            return GS_ERROR;
        }
    }
...
status_t bak_head_verify_checksum(knl_session_t *session, bak_head_t *head, uint32 size, bool32 is_check_file)
{
    ...
    if (is_check_file && org_cks_file != new_cks) {
        GS_LOG_RUN_ERR("[BACKUP] backupset file checksum. file %s org_cks %u new_cks %u, "
            ...
            ctx->bak.local.name, org_cks_file, new_cks, (uint32)is_check_file, knl_checksum_level(cks_level));
    ...
    if (!is_check_file && org_cks_head != new_cks) {
        GS_LOG_RUN_ERR("[BACKUP] backupset file checksum. file %s org_cks %u new_cks %u, "
            "check type %u,, checksum level %s",
            ctx->bak.local.name, org_cks_head, new_cks, (uint32)is_check_file, knl_checksum_level(cks_level));
        GS_THROW_ERROR(ERR_CHECKSUM_FAILED, ctx->bak.local.name);
        return GS_ERROR;
    }
    ...
    uint32 sec_id, char *file_name)
{
    ...
    if (bak->restore) {
        ret = snprintf_s(file_name, GS_FILE_NAME_BUFFER_SIZE, GS_MAX_FILE_NAME_LEN, "%s/data_%s_%u_%u.bak", path,
    ...
    uint32 sec_id, char *file_name)
{
    ...
    switch (type) {
        case BACKUP_CTRL_FILE:
            ret =
                snprintf_s(file_name, GS_FILE_NAME_BUFFER_SIZE, GS_MAX_FILE_NAME_LEN, "%s/ctrl_%d_%d.bak", path, 0, 0);
            knl_securec_check_ss(ret);
            break;
        case BACKUP_DATA_FILE:
            ...
            break;
        case BACKUP_LOG_FILE:
            ret = snprintf_s(file_name, GS_FILE_NAME_BUFFER_SIZE, GS_MAX_FILE_NAME_LEN, "%s/log_%u_%d.bak", path,
                file_id, 0);
            knl_securec_check_ss(ret);
            break;
        case BACKUP_ARCH_FILE:
            ret = snprintf_s(file_name, GS_FILE_NAME_BUFFER_SIZE, GS_MAX_FILE_NAME_LEN, "%s/arch_%u_%d.bak", path,
                file_id, 0);
            knl_securec_check_ss(ret);
            break;
    ...
    uint32 hwm_start)
{
    ...
    if (offset == DEFAULT_PAGE_SIZE) {
        if (contains_dw || file_size == SPACE_HEAD_END * DEFAULT_PAGE_SIZE) {
            ...
            read_size =
                hwm_start > 1 ? (hwm_start - 1) * DEFAULT_PAGE_SIZE : file_size - offset; /* skip file head page */
        }
    } else {
