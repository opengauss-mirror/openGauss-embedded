...
#define TEST_S (((knl_session_t *)session)->kernel->attr.page_size)
...
{
    if (page_type == PAGE_TYPE_HEAP_DATA || page_type == PAGE_TYPE_PCRH_DATA || page_type == PAGE_TYPE_BTREE_NODE ||
        page_type == PAGE_TYPE_PCRB_NODE || page_type == PAGE_TYPE_LOB_DATA || page_type == PAGE_TYPE_UNDO) {
        return GS_TRUE;
    }
    return GS_FALSE;
}
