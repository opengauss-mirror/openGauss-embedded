...
status_t page_cipher_reserve_size(knl_session_t *session, encrypt_version_t version, uint8 *cipher_reserve_size)
{
    ...
    *cipher_reserve_size = max_size;
    ...
    status_t status = cm_kmc_encrypt(GS_KMC_KERNEL_DOMAIN, version, plain_buf, plain_len, cipher_buf, &cipher_len);
...
static void check_ctrl_befor_encrypt(cipher_ctrl_t *cipher_ctrl, page_head_t *page, page_id_t page_id,
    uint8 cipher_reserve_size)
{
    if (page->encrypted != 0 || cipher_ctrl->cipher_expanded_size != 0 || cipher_ctrl->encrypt_version != 0 ||
        cipher_ctrl->offset != 0 || cipher_ctrl->reserved != 0) {
        knl_panic_log(GS_FALSE,
            "invalid cipher ctrl before encrypt: "
            ...
            cipher_ctrl->cipher_expanded_size, cipher_ctrl->offset, cipher_ctrl->plain_cks, cipher_reserve_size);
    }
}

static void check_ctrl_after_encrypt(cipher_ctrl_t *cipher_ctrl, page_head_t *page, page_id_t page_id,
    uint32 cipher_len, uint32 plain_len, uint8 cipher_reserve_size)
{
    if (cipher_ctrl->cipher_expanded_size + sizeof(cipher_ctrl_t) > cipher_reserve_size ||
        ...
        knl_panic_log(GS_FALSE,
            "invalid cipher ctrl after encrypt :"
            "page_info: page %u, file %u, page_type %u, "
            "cipher_ctrl: encrypted: %u, encrypt_version: %u, cipher_expanded_size: %u, offset: %u, plain_cks: %u, "
            "space->ctrl->cipher_reserve_size: %u ",
            page_id.page, page_id.file, page->type, page->encrypted, cipher_ctrl->encrypt_version,
...
static void check_reserve_ciper_buf(page_head_t *page, page_id_t page_id, cipher_ctrl_t *cipher_ctrl,
    uint8 cipher_reserve_size)
{
...
static void check_ctrl_before_decrypt(space_t *space, cipher_ctrl_t *cipher_ctrl, page_head_t *page, page_id_t page_id,
    uint32 cipher_len, uint32 org_plain_len)
{
    if (!page->encrypted || cipher_ctrl->cipher_expanded_size == 0 || cipher_ctrl->encrypt_version == NO_ENCRYPT ||
        ...
        knl_panic_log(GS_FALSE,
            "invalid cipher ctrl before decrypt : "
            "page_info: page %u, file %u, page_type %u,"
            "cipher_ctrl: encrypted: %u, encrypt_version: %u, cipher_expanded_size: %u, offset: %u, plain_cks: %u, "
            "space->ctrl->cipher_reserve_size: %u ",
            page_id.page, page_id.file, page->type, page->encrypted, cipher_ctrl->encrypt_version,
...
status_t page_encrypt(knl_session_t *session, page_head_t *page, uint8 encrypt_version, uint8 cipher_reserve_size)
{
    ...
    status_t status =
        cm_kmc_encrypt(GS_KMC_KERNEL_DOMAIN, encrypt_version, plain_buf, plain_len, cipher_buf, &cipher_len);
    if (status != GS_SUCCESS) {
    ...
    errno_t ret =
        memcpy_sp((char *)page + cipher_ctrl->offset, DEFAULT_PAGE_SIZE - cipher_ctrl->offset, cipher_buf, cipher_len);
    knl_securec_check(ret);
    ...
    cipher_ctrl_t *temp_ctrl, uint8 cipher_reserve_size)
{
    ...
    if (!page_verify_checksum(page, DEFAULT_PAGE_SIZE)) {
        knl_panic_log(GS_FALSE,
            "invaid page plain data: "
            "page_info: page %u, file %u, page_type %u, "
            "cipher_ctrl: encrypted: %u, encrypt_version: %u, cipher_expanded_size: %u, offset: %u, plain_cks: %u, "
            "space->ctrl->cipher_reserve_size: %u ",
            page_id.page, page_id.file, page->type, page->encrypted, temp_ctrl->encrypt_version,
            temp_ctrl->cipher_expanded_size, temp_ctrl->offset, temp_ctrl->plain_cks, cipher_reserve_size);
    }
...
status_t page_decrypt(knl_session_t *session, page_head_t *page)
{
...
#ifdef LOG_DIAG
    page_id_t page_id = AS_PAGID(page->id);
    knl_panic_log(page_type_suport_encrypt(page->type),
        "current page type is not suport encrypt, panic info: "
        "page %u-%u type %u",
    ...
    knl_panic_log(plain_len == org_plain_len,
        "the plain_len is not equal org_plain_len, panic info: "
