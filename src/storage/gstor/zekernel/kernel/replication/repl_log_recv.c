...
static status_t lrcv_flush_log(lrcv_context_t *lrcv, log_point_t *log_point, void *batch, uint32 size)
{
    ...
    if (cm_write_device(file->ctrl->type, file->handle, offset, batch, space_size) != GS_SUCCESS) {
        GS_LOG_RUN_ERR("[Log Receiver] failed to write log into %s[%u] with log size %u at point [%u-%u/%llu]",
            file->ctrl->name, log_ctx->curr_file, space_size, file->head.rst_id, file->head.asn, file->head.write_pos);
        return GS_ERROR;
    }
    ...
    GS_LOG_DEBUG_INF("[Log Receiver] Write space size %u into log file[%u] at point [%u-%u/%llu] to %llu "
        ...
        space_size, log_ctx->curr_file, file->head.rst_id, file->head.asn, file->head.write_pos, offset + space_size,
        batch_x->size, batch_x->head.magic_num, (uint64)batch_x->head.point.lfn, batch_x->raft_index, tail->magic_num,
        (uint64)tail->point.lfn);
...
status_t lrcv_fetch_archived_log(lrcv_context_t *lrcv, log_file_t *file, bool32 need_wait, char *arch_name,
    uint32 arch_name_buf_size)
{
...
static status_t lrcv_verify_checksum_batch(knl_session_t *session, log_batch_t *batch_input, uint32 size,
...
static status_t lrcv_process_batch(lrcv_context_t *lrcv)
{
    ...
    GS_LOG_DEBUG_INF("[Log Receiver] Received batch with size %u on log file[%u] at point [%u-%u/%u/%llu], "
        ...
        size, req->log_file_id, req->log_point.rst_id, req->log_point.asn, req->log_point.block_id,
        (uint64)req->log_point.lfn, log->curr_file);
    ...
    if (lrcv->status == LRCV_PREPARE) {
        ...
        if (LRCV_LOG_POINT_ON_POST_FILE(req->log_point, file->head) ||
            ...
            GS_LOG_RUN_INF("[Log Receiver] Invalidate current file, active %u current %u", log->active_file,
                log->curr_file);
            ...
            GS_LOG_RUN_INF("[Log Receiver] Reset current file asn %u status %d, active %u current %u", file->head.asn,
                file->ctrl->status, log->active_file, log->curr_file);
    ...
    if (req->log_file_id != log->curr_file || req->log_point.asn != log->files[log->curr_file].head.asn) {
        callback_t callback;
        ...
        if (log_switch_logfile(lrcv->session, (uint16)req->log_file_id, req->log_point.asn, &callback) != GS_SUCCESS) {
        ...
        if (file->ctrl->status == LOG_FILE_INACTIVE || file->ctrl->status == LOG_FILE_UNUSED) {
            ...
            if (LRCV_LOG_POINT_ON_CURR_FILE(*rcy_point, file->head)) {
                ...
                if (arch_get_archived_log_name(lrcv->session, file->head.rst_id, file->head.asn, ARCH_DEFAULT_DEST,
                    ...
                    if (log_get_file_offset(lrcv->session, arch_name, &lrcv->recv_buf.read_buf, &file_offset, &lfn,
                        &scn) != GS_SUCCESS) {
                    ...
                    if (rcy_point->lfn >= lfn || log_ctx->lfn >= lfn) {
                        GS_LOG_RUN_INF("[Log Receiver] rcy_point [%u-%u/%u/%llu] or replay lfn %llu has reached "
                            ...
                            log_ctx->lfn, file_id, file->head.rst_id, file->head.asn, file_offset, lfn);
                        log_get_next_file(lrcv->session, &file_id, GS_FALSE);
                    } else {
                        GS_LOG_DEBUG_INF("[Log Receiver] rcy_point [%u-%u/%u/%llu] has not reached the end of "
                            "log file[%u] [%u-%u/%llu/%llu], so continue",
                            rcy_point->rst_id, rcy_point->asn, rcy_point->block_id, (uint64)rcy_point->lfn, file_id,
...
status_t lrcv_prepare_log_files(lrcv_context_t *lrcv, rep_query_status_req_t *req)
{
    ...
    GS_LOG_RUN_INF("[Log Receiver] Standby current log file[%u] status %d log point is [%u-%u/%llu/%llu]",
        log_ctx->curr_file, file->ctrl->status, file->head.rst_id, file->head.asn, file->head.write_pos, curr_lfn);
    GS_LOG_RUN_INF("[Log Receiver] Standby lrp_point is [%u-%u/%u/%llu] rcy_point is [%u-%u/%u/%llu]",
        lrp_point->rst_id, lrp_point->asn, lrp_point->block_id, (uint64)lrp_point->lfn, rcy_point->rst_id,
        rcy_point->asn, rcy_point->block_id, (uint64)rcy_point->lfn);
    GS_LOG_RUN_INF("[Log Receiver] Primary current log point is [%u-%u/%u/%llu]", req->curr_point.rst_id,
        req->curr_point.asn, req->curr_point.block_id, (uint64)req->curr_point.lfn);
    ...
    if (LRCV_LOG_POINT_ON_CURR_FILE(*lrp_point, file->head)) {
        if (LRCV_LOG_POINT_ON_CURR_FILE(req->curr_point, file->head)) {
            if (!lrcv->reconnected) {
                ...
                GS_LOG_RUN_INF(
                    "[Log Receiver] Standby repaired current log file[%u] status %d log point is [%u-%u/%llu/%llu]",
                    log_ctx->curr_file, file->ctrl->status, file->head.rst_id, file->head.asn, file->head.write_pos,
                    curr_lfn);
            ...
            lrcv->flush_point.rst_id = file->head.rst_id;
            lrcv->flush_point.asn = file->head.asn;
            if (log_get_file_offset(lrcv->session, arch_name, &lrcv->recv_buf.read_buf, &write_pos_new, &curr_lfn,
                &scn) != GS_SUCCESS) {
                GS_LOG_RUN_ERR("[Log Receiver] failed to get file offset for archived log %s ", arch_name);
                return GS_ERROR;
            }
            ...
            GS_LOG_RUN_ERR("[Log Receiver] current log point [%u-%u/%u/%llu] from primary is "
                ...
                req->curr_point.rst_id, req->curr_point.asn, req->curr_point.block_id, (uint64)req->curr_point.lfn,
                file->head.rst_id, file->head.asn);
            return GS_ERROR;
        }
    ...
    GS_LOG_RUN_INF("[Log Receiver] Set flush point to [%u-%u/%u/%llu], log free size is %llu", lrcv->flush_point.rst_id,
        lrcv->flush_point.asn, lrcv->flush_point.block_id, (uint64)lrcv->flush_point.lfn, log_ctx->free_size);
    return GS_SUCCESS;
}
...
static bool32 lrcv_log_ctrl_check(lrcv_context_t *lrcv, rep_query_status_req_t *req)
{
    ...
    for (uint32 i = 0; i < log_ctx->logfile_hwm; i++) {
        ...
        if (logfile->ctrl->block_size != ctrl_pri->block_size || logfile->ctrl->file_id != ctrl_pri->file_id ||
            logfile->ctrl->flg != ctrl_pri->flg || logfile->ctrl->size != ctrl_pri->size ||
    ...
    for (uint32 i = 0; i < ctx->logfile_hwm; i++) {
        ...
        if (file->head.asn > rst_log->last_asn && file->head.rst_id < rst_log->rst_id) {
            GS_LOG_RUN_INF("[Log Receiver] logfile %s asn %u larger than resetlog asn %u, "
                ...
                file->ctrl->name, file->head.asn, rst_log->last_asn, file->head.rst_id, rst_log->rst_id,
                rst_log->rst_id);
...
static status_t lrcv_resetlog_check(lrcv_context_t *lrcv, rep_query_status_req_t *req)
{
    ...
    if (!db->ctrl.core.is_restored && LRCV_INVALID_DBID(req->dbid, db->ctrl.core.dbid)) {
        lrcv_needrepair(lrcv);
        GS_LOG_RUN_ERR("[Log Receiver] primary dbid [%u] is not equal to standby dbid [%u], need repair", req->dbid,
            db->ctrl.core.dbid);
        return GS_ERROR;
    }
    ...
    GS_LOG_RUN_INF("[Log Receiver] resetlog_check primary resetlog is [%u/%u/%llu], standby resetlog is [%u/%u/%llu], "
        ...
        req->rst_log.rst_id, req->rst_log.last_asn, req->rst_log.last_lfn, resetlog->rst_id, resetlog->last_asn,
        resetlog->last_lfn, req->curr_point.rst_id, req->curr_point.asn, (uint64)req->curr_point.lfn, point->rst_id,
        point->asn, (uint64)point->lfn, rcy_point->rst_id, rcy_point->asn, (uint64)rcy_point->lfn, lrp_point->rst_id,
        lrp_point->asn, (uint64)lrp_point->lfn, lrcv->flush_point.rst_id, lrcv->flush_point.asn,
        (uint64)lrcv->flush_point.lfn);
    ...
    if (req->curr_point.lfn < point->lfn || LOG_POINT_FILE_LT(req->curr_point, *point)) {
        ...
        GS_LOG_RUN_ERR("[Log Receiver] Standby current point [%u/%u/%llu] is faster "
            "than primary current point [%u/%u/%llu], need repair",
            point->rst_id, point->asn, (uint64)point->lfn, req->curr_point.rst_id, req->curr_point.asn,
            (uint64)req->curr_point.lfn);
        return GS_ERROR;
    }
    ...
    if (req->rst_log.rst_id != point->rst_id &&
        ...
        GS_LOG_RUN_ERR("[Log Receiver] Standby log lrp point [%u/%llu] current redo point [%u/%llu] is faster "
            ...
            point->rst_id, (uint64)lrp_point->lfn - 1, point->rst_id, (uint64)point->lfn, req->rst_log.rst_id,
            req->rst_log.last_lfn);
        return GS_ERROR;
    }
    ...
    *resetlog = req->rst_log;
    ...
    GS_LOG_RUN_INF("[Log Receiver] Resetlog check passed. Current reset log is [%u/%u/%llu], "
        ...
        req->rst_log.rst_id, req->rst_log.last_asn, req->rst_log.last_lfn, point->rst_id, point->asn, point->block_id,
        (uint64)point->lfn);
...
status_t lrcv_buf_alloc(knl_session_t *session, lrcv_context_t *lrcv)
{
    ...
    if (cm_aligned_malloc((int64)lrcv->d_ctx.buf_size, "lrcv compressed buffer", &lrcv->d_ctx.compressed_buf) !=
        GS_SUCCESS) {
...
static status_t lrcv_compress_receive(lrcv_context_t *lrcv, rep_batch_req_t *req, uint32 remain_size, int32 *recv_size)
{
    ...
    if (cs_read_stream(lrcv->pipe, lrcv->d_ctx.compressed_buf.aligned_buf, REPL_RECV_TIMEOUT, remain_size, recv_size) !=
        GS_SUCCESS) {
    ...
    switch (req->compress_alg) {
        case COMPRESS_ZSTD:
            if (lrcv_zstd_decompress(lrcv, lrcv->d_ctx.compressed_buf.aligned_buf, (uint32) * recv_size, &batches_size) !=
                GS_SUCCESS) {
                return GS_ERROR;
            }
            break;
        case COMPRESS_LZ4:
            if (lrcv_lz4_decompress(lrcv, lrcv->d_ctx.compressed_buf.aligned_buf, (uint32) * recv_size, &batches_size) !=
                GS_SUCCESS) {
                return GS_ERROR;
            }
            break;
    ...
    if (lrcv->header.type == REP_BATCH_REQ) {
        ...
        if (rep_batch_req->compress_alg == COMPRESS_NONE) {
            if (lrcv->header.size > lrcv->recv_buf.read_buf.buf_size) {
                ...
                if (cm_aligned_realloc((int64)new_buf_size, "lrcv recv buffer", &lrcv->recv_buf.read_buf) !=
                    GS_SUCCESS) {
            ...
            if (remain_size > lrcv->recv_buf.read_buf.buf_size) {
                GS_LOG_RUN_ERR("[Log Receiver] the remain data size %u exceeds the receive buffer size %u", remain_size,
                    (uint32)lrcv->extend_buf.read_buf.buf_size);
                return GS_ERROR;
            }
...
status_t lrcv_get_primary_server(knl_session_t *session, int32 retry_count, char *host, uint32 host_buf_size,
    uint16 *port)
{
    lrcv_context_t *lrcv = &session->kernel->lrcv_ctx;
