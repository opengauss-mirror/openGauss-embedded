...
#define LSND_SYNC_AFFIRM(ctx) \
    ((ctx)->dest_info.sync_mode == LOG_NET_TRANS_MODE_SYNC && (ctx)->dest_info.affirm_mode == LOG_ARCH_AFFIRM)
...
#define STANDBY_NO_NEED_WAIT(db, ctx, lsnd)                                                          \
    ((lsnd)->status < LSND_LOG_SHIFTING || (lsnd)->dest_info.sync_mode != LOG_NET_TRANS_MODE_SYNC || \
        (MODE_MAX_PROTECTION(db) && (ctx)->affirm_standy_num > 0 &&                                  \
        (lsnd)->dest_info.affirm_mode != LOG_ARCH_AFFIRM) ||                                         \
        (MODE_MAX_AVAILABILITY(db) && (ctx)->est_affirm_standy_num > 0 &&                            \
        (lsnd)->dest_info.affirm_mode != LOG_ARCH_AFFIRM))
...
static bool32 lsnd_rcv_msg_is_valid(lsnd_t *lsnd, uint32 type)
{
    switch (type) {
        ...
        case REP_RECORD_BACKUPSET_REQ: {
            ...
            if (rec->attr.tag[0] == '\0' || rec->path[0] == '\0' || rec->attr.backup_type < BACKUP_MODE_INVALID ||
                rec->attr.backup_type > BACKUP_MODE_FINISH_LOG || rec->attr.compress < COMPRESS_NONE ||
                rec->attr.compress > COMPRESS_LZ4 || rec->status < BACKUP_SUCCESS || rec->status > BACKUP_FAILED ||
                rec->device < DEVICE_DISK || rec->device > DEVICE_UDS ||
                log_point_is_invalid(&rec->ctrlinfo.rcy_point) || log_point_is_invalid(&rec->ctrlinfo.lrp_point)) {
                return GS_FALSE;
            }
            break;
        }
...
static status_t lsnd_receive(lsnd_t *lsnd, uint32 timeout, uint32 *type, int32 *recv_size)
{
    ...
    if (cs_read_stream(&lsnd->pipe, (char *)&message_header, timeout, sizeof(rep_msg_header_t), recv_size) !=
        GS_SUCCESS) {
...
static status_t lsnd_process_record_bak_request(lsnd_t *lsnd)
{
    ...
    errno_t err =
        memcpy_s(&bak_task->record, sizeof(bak_record_t), lsnd->recv_buf.read_buf.aligned_buf, sizeof(bak_record_t));
    knl_securec_check(err);
...
static inline void lsnd_process_batch_resp(lsnd_t *lsnd)
{
    ...
    GS_LOG_DEBUG_INF("[Log Sender] peer flush point [%u-%u/%u/%llu], peer rcy point [%u-%u/%u/%llu]",
        lsnd->peer_flush_point.rst_id, lsnd->peer_flush_point.asn, lsnd->peer_flush_point.block_id,
        (uint64)lsnd->peer_flush_point.lfn, lsnd->peer_rcy_point.rst_id, lsnd->peer_rcy_point.asn,
...
static void lsnd_process_query_status_ready(lsnd_t *lsnd)
{
    ...
    if (lsnd->send_point.lfn == rst_log.last_lfn) {
        ...
        GS_LOG_RUN_INF("[Log Sender] Peer flush point equals to last restlog[%u-%u/%llu], "
            ...
            rst_log.rst_id, rst_log.last_asn, rst_log.last_lfn, lsnd->send_point.rst_id, lsnd->send_point.asn,
...
void lsnd_process_abr_task(lsnd_t *lsnd)
{
    ...
    if (task->running && !task->executing) {
        ...
        if (abr_send_page_fetch_req(lsnd, task) != GS_SUCCESS) {
            ...
            GS_LOG_RUN_WAR("[ABR] failed to send ABR task to standby for page[%u-%u] with lsnd id %u", task->file,
                task->page, task->lsnd_id);
        } else {
            GS_LOG_RUN_INF("[ABR] succeed to send ABR task to standby for file %u page %u with lsnd id %u", task->file,
                task->page, task->lsnd_id);
        }
#endif
    }
...
static inline bool32 lsnd_need_notify_repair(lsnd_t *lsnd)
{
    ...
    if (DB_IS_PRIMARY(db) || (DB_IS_PHYSICAL_STANDBY(db) && lrcv->status == LRCV_READY)) {
        return GS_TRUE;
    }
...
static status_t lsnd_put_batch_message(lsnd_t *lsnd, log_point_t *point, uint32 file_id, uint64 size)
{
    ...
    if (log_need_realloc_buf(batch, &lsnd->send_buf.read_buf, "lsnd batch buffer", GS_MAX_BATCH_SIZE + SIZE_K(4))) {
        ...
        if (cm_aligned_realloc((int64)new_compress_buf_size, "lsnd compress buffer", &lsnd->c_ctx.compress_buf) !=
            GS_SUCCESS) {
    ...
    while (left_size >= sizeof(log_batch_t)) {
        ...
        GS_LOG_DEBUG_INF("[Log Sender] Put batch [%u-%u/%u/%llu] space size %u", lsnd->last_put_point.rst_id,
            lsnd->last_put_point.asn, lsnd->last_put_point.block_id, (uint64)batch->head.point.lfn, batch->space_size);
...
static status_t lsnd_read_arch_logfile(lsnd_t *lsnd)
{
    ...
    if (file->asn > point->asn) {
        GS_LOG_RUN_ERR("[Log Sender] invalid send point [%u-%u/%u/%llu], arch file asn is %u, name %s", point->rst_id,
            point->asn, point->block_id, (uint64)point->lfn, file->asn, file->file_name);
        return GS_ERROR;
    }
    ...
    if (file->asn != point->asn) {
        ...
        if (cm_open_device(file->file_name, DEV_TYPE_FILE, knl_redo_io_flag(session), &file->handle) != GS_SUCCESS) {
    ...
    if (file_id == GS_INVALID_ID32) {
        if (lsnd->last_read_asn == send_point->asn && lsnd->last_read_file_id != -1) {
            ...
            while (file_id == GS_INVALID_ID32 && send_point->asn < file->head.asn) {
                ...
                file_id =
                    log_get_id_by_asn(lsnd->session, (uint32)send_point->rst_id, send_point->asn, &loading_curr_file);
                log_unlock_logfile(lsnd->session);
            }
    ...
    if (lsnd->send_buf.write_pos == 0) {
        if (file_id == GS_INVALID_ID32 || !loading_curr_file) {
            GS_LOG_DEBUG_INF("[Log Sender] send point [%u-%u/%u/%llu], target point [%u-%u/%u/%llu], "
                ...
                send_point->rst_id, send_point->asn, send_point->block_id, (uint64)send_point->lfn, target_point.rst_id,
                target_point.asn, target_point.block_id, (uint64)target_point.lfn, reset_log->rst_id,
                reset_log->last_asn, reset_log->last_lfn);
    ...
    *sent = GS_FALSE;
    ...
    while (lsnd_need_send_batch(lsnd, &read_pos, &write_pos)) {
        ...
        GS_LOG_DEBUG_INF("[Log Sender] Ready to Send batch SYNC from read pos %u write pos %u with size %u "
            ...
            read_pos, lsnd->send_buf.write_pos, rep_msg_header->size, rep_batch_req->log_file_id,
            rep_batch_req->log_point.rst_id, rep_batch_req->log_point.asn, rep_batch_req->log_point.block_id,
            (uint64)rep_batch_req->log_point.lfn, batch->size, batch->head.magic_num, (uint64)batch->head.point.lfn,
            batch->raft_index, tail->magic_num, (uint64)tail->point.lfn);
        ...
        *sent = GS_TRUE;
        ...
        GS_LOG_DEBUG_INF("[Log Sender] Send batch SYNC from read pos %u with size %u on log file[%d] "
            ...
            read_pos, rep_msg_header->size, rep_batch_req->log_file_id, rep_batch_req->log_point.rst_id,
            rep_batch_req->log_point.asn, rep_batch_req->log_point.block_id, (uint64)rep_batch_req->log_point.lfn);
    ...
    if (lsnd->send_buf.write_pos > lsnd->send_buf.read_pos && lsnd->in_async) {
        ...
        *sent = GS_TRUE;
        ...
        GS_LOG_DEBUG_INF("[Log Sender] Send batch ASYNC from read pos %u with size %u at log point [%u-%u/%u/%llu]",
            lsnd->send_buf.read_pos, rep_msg_header->size, lsnd->send_point.rst_id, lsnd->send_point.asn,
            lsnd->send_point.block_id, (uint64)lsnd->send_point.lfn);
    }
...
static status_t lsnd_send_log(lsnd_t *lsnd, bool32 *sent)
{
    ...
    if (lsnd->dest_info.sync_mode == LOG_NET_TRANS_MODE_ASYNC || lsnd->tmp_async ||
...
static status_t lsnd_connect(lsnd_t *lsnd, uint32 *cs_fail_cnt)
{
    ...
    if (knl_login(lsnd->session, &lsnd->pipe, REP_LOGIN_REPL, (const char *)info->local_host, &login_err) !=
        GS_SUCCESS) {
...
static status_t lsnd_init_log_files(knl_session_t *session, lsnd_t *lsnd)
{
    ...
    for (uint32 i = 0; i < db->ctrl.core.log_hwm; i++) {
        ...
        if (cm_open_device(logfile->ctrl->name, logfile->ctrl->type, knl_redo_io_flag(session), &lsnd->log_handle[i]) !=
            GS_SUCCESS) {
...
static status_t lsnd_init_proc_context(knl_session_t *session, uint32 idx, arch_attr_t *arch_attr, lsnd_t **lsnd)
{
    ...
    if ((*lsnd) == NULL) {
        ...
        if ((*lsnd) == NULL) {
            GS_LOG_RUN_ERR("[Log Sender] failed to allocate %llu bytes for %s", (uint64)sizeof(lsnd_t),
                "lsnd_proc context");
            return GS_ERROR;
        }
    }
    ...
    if (arch_attr->compress_alg != COMPRESS_NONE) {
        ...
        if (cm_aligned_malloc((int64)(*lsnd)->c_ctx.buf_size, "lsnd compress buffer", &(*lsnd)->c_ctx.compress_buf) !=
            GS_SUCCESS) {
...
static status_t lsnd_init_each_proc(knl_session_t *session, uint32 idx)
{
    ...
    if (arch_ctx->arch_dest_state_changed) {
        GS_LOG_RUN_INF("[Log Sender] start new lsnd thread in free slot %u for archive dest state enabled", free_slot);
    }
...
void lsnd_try_clear_tmp_async(lsnd_t *lsnd, log_point_t *point)
{
    ...
    GS_LOG_ALARM_RECOVER(WARN_DEGRADE, "'peer-host':'%s', 'peer-port':'%u', 'status':'%s'}", lsnd->dest_info.peer_host,
        lsnd->dest_info.peer_port, "flush log");
    ...
    switch (type) {
        case DEGRADE_FLUSH_LOG:
            GS_LOG_RUN_INF("[Log Sender] LSND(%lu) send buffer is full, need to read log from file directly "
                "during flushing log",
                LSND_TID(lsnd));
            GS_LOG_ALARM(WARN_DEGRADE, "'peer-host':'%s', 'peer-port':'%u', 'status':'%s'}", lsnd->dest_info.peer_host,
                lsnd->dest_info.peer_port, "flush log");
            break;
        ...
        case DEGRADE_WAIT_RESP:
            ...
            GS_LOG_ALARM(WARN_DEGRADE, "'peer-host':'%s', 'peer-port':'%u', 'status':'%s'}", lsnd->dest_info.peer_host,
                lsnd->dest_info.peer_port, "waiting");
            break;
...
{
    return (bool32)(!lsnd->flush_completed && lsnd->status > LSND_STATUS_QUERYING &&
...
bool32 lsnd_copy_batch(lsnd_t *lsnd, log_batch_t *batch, log_file_t *file, log_point_t *point)
{
    ...
    GS_LOG_DEBUG_INF("[Log Sender] copy batch to write pos %u with size %u on log file[%u] at log point "
        ...
        lsnd->send_buf.write_pos, pkg_size, file->ctrl->file_id, point->rst_id, point->asn, point->block_id,
        (uint64)point->lfn, lsnd->peer_flush_point.rst_id, lsnd->peer_flush_point.asn, lsnd->peer_flush_point.block_id,
        (uint64)lsnd->peer_flush_point.lfn, batch->size, batch->head.magic_num, (uint64)batch->head.point.lfn,
        batch->raft_index, tail->magic_num, (uint64)tail->point.lfn);
...
static inline bool32 lsnd_sys_session_no_wait(knl_session_t *session)
{
    return (bool32)(IS_SYS_SESSION(session) &&
        (session->kernel->switch_ctrl.request != SWITCH_REQ_NONE || session->kernel->backup_ctx.bak.is_building));
}
...
void lsnd_wait_without_quorum(knl_session_t *session, uint64 curr_lfn, uint64 *quorum_lfn)
{
    ...
    for (;;) {
        ...
        for (uint32 i = 0; i < ctx->standby_num; i++) {
            ...
            if (MODE_MAX_AVAILABILITY(db) && (end_time - begin_time >= lsnd->timeout * REPL_WAIT_MULTI &&
                lsnd->dest_info.sync_mode == LOG_NET_TRANS_MODE_SYNC)) {
...
void lsnd_flush_log(knl_session_t *session, log_context_t *redo_ctx, log_file_t *file, log_batch_t *batch)
{
    ...
    GS_LOG_DEBUG_INF("[Log Sender] Try to flush batch %llu(%llu) in log %u asn %u offset %u", (uint64)point->lfn,
        (uint64)batch->head.point.lfn, file->ctrl->file_id, point->asn, point->block_id);
...
static void lsnd_set_lag_info(lsnd_t *lsnd, sync_info_t *sync_info, uint64 curr_lfn)
{
    ...
    if (lsnd->peer_flush_scn == 0) {
        ...
        sync_info->flush_lag =
            (uint64)(cm_timeval2date(local_time) - cm_timeval2date(peer_flush_time)) / MICROSECS_PER_MILLISEC;
    }
    ...
    if (curr_lfn == lsnd->peer_rcy_point.lfn) {
        ...
        sync_info->replay_lag =
            (uint64)(cm_timeval2date(local_time) - cm_timeval2date(peer_current_time)) / MICROSECS_PER_MILLISEC;
    }
}
...
status_t lsnd_check_protection_standby_num(knl_session_t *session)
{
    ...
    if (quorum_any > 0) {
        GS_LOG_RUN_INF("[Log Sender] config standby num is %d, ctx->standby_num is %d", standby_num, ctx->standby_num);
