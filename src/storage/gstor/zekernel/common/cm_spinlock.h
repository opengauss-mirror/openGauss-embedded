...
EXPORT_API void cm_spin_sleep_and_stat(spin_statis_t *stat);
EXPORT_API void cm_spin_sleep_and_stat2(uint32 ms);
EXPORT_API uint64 cm_total_spin_usecs();

#ifdef _WIN32

static inline uint32 cm_spin_set(spinlock_t *ptr, uint32 value)
{
    return (uint32)InterlockedExchange((long int *)ptr, value);
}
