...
static long __stdcall cm_crash_handler(EXCEPTION_POINTERS *excp)
{
    ...
    printf("eax   %x   ebx   %x   ecx   %x   edx   %x\n", (uint32)excp->ContextRecord->Rax,
        (uint32)excp->ContextRecord->Rbx, (uint32)excp->ContextRecord->Rcx, (uint32)excp->ContextRecord->Rdx);
#else
    printf("eax   %x   ebx   %x   ecx   %x   edx   %x\n", (uint32)excp->ContextRecord->Eax,
        (uint32)excp->ContextRecord->Ebx, (uint32)excp->ContextRecord->Ecx, (uint32)excp->ContextRecord->Edx);
#endif // WIN64
