...
status_t cm_tzoffset2text(timezone_info_t tz, text_t *text)
{
    ...
    PRTS_RETURN_IFERR(snprintf_s(text->str, TIMEZONE_OFFSET_STRLEN, TIMEZONE_OFFSET_STRLEN - 1, g_default_tzoffset_fmt,
        sign, abs(hour), minute));
...
static num_errno_t cm_text2tzoffset_core(text_t *text, timezone_info_t *tz)
{
    ...
    *((int16 *)tz) =
        (int16)((is_neg) ? (hour * TIMEZONE_MINUTES_PER_HOUR - min) : (hour * TIMEZONE_MINUTES_PER_HOUR + min));
