    ...
    while (*item != NULL) {
        if (cm_text_str_equal_ins(name, (*item)->name) || 
            ...
            if (!is_infile && ((*item)->flag & FLAG_ZFILE)) {
                /*  Based on the configuration file, no errors will be reported, only alarms
                GS_THROW_ERROR(ERR_DUPLICATE_PARAMETER, (*item)->name);
                return GS_ERROR;
                * [ERR_DUPLICATE_PARAMETER]     = "Duplicate or conflicting parameter %s",
                */
                GS_LOG_RUN_WAR("Duplicate or conflicting parameter %s, use the configuration file(gstor.ini).", (*item)->name);
            }
            break;
        }
        *item = (*item)->next;
...
static status_t cm_parse_config(config_t *config, char *buf, uint32 buf_len, bool32 is_ifile, bool32 set_alias)
{
    ...
    while (cm_fetch_text(&text, '\n', '\0', &line)) {
        ...
        // check the params is configable
        bool32 is_write_ini = GS_FALSE;
...
void cm_init_config(config_item_t *items, uint32 item_count, config_t *config)
{
    ...
    // because the file is not large than MAX_CONFIG_FILE_SIZE
    config->value_buf_size = GS_MAX_CONFIG_FILE_SIZE;
    for (i = 0; i < item_count; i++) {
        item = &config->items[i];
        item->pfile_value = item->value;
        item->next = NULL;
        if (i>0){
            config->items[i-1].next = item;
        }

        /* initialize hash map by name */
        // note: will cause conflict if hash value is the same ?
    ...
    if (item_count>0){
        config->first_item = &config->items[0];
        config->last_item = &config->items[item_count-1];
    }
}
    ...
    config_t *config, bool32 set_alias)
{
    ...
    if(config->items == NULL) { // config not init 
        cm_init_config(items, item_count, config);
    }

    if(file_name != config->file_name) { // address not equal
...
static status_t cm_alter_config_item(config_t *config, config_item_t *item, const char *value, config_scope_t scope)
{
    ...
    uint32 buf_size = (uint32)strlen(value) + 1;
    if (item->is_default) {
        if (cm_alloc_config_buf(config, buf_size , &item->value) != GS_SUCCESS) {
            return GS_ERROR;
        }
        if (scope != CONFIG_SCOPE_MEMORY && 
            cm_alloc_config_buf(config, buf_size , &item->pfile_value) != GS_SUCCESS) {
            return GS_ERROR;
        }
    }
    ...
    if (scope != CONFIG_SCOPE_DISK) {
        errcode = strncpy_s(item->value, buf_size , value, (size_t)value_len);
        if (errcode != EOK) {
            GS_THROW_ERROR(ERR_SYSTEM_CALL, errcode);
            return GS_ERROR;
        }
    ...
    item->flag &= ~FLAG_INFILE;
    if (scope != CONFIG_SCOPE_MEMORY) {
        errcode = strncpy_s(item->pfile_value, buf_size , value, (size_t)value_len);
        if (errcode != EOK) {
            GS_THROW_ERROR(ERR_SYSTEM_CALL, errcode);
            return GS_ERROR;
        }
