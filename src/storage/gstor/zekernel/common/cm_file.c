...
status_t cm_create_dir(const char *dir_name) {
    return cm_create_dir_mode(dir_name, S_IRWXU);
}

status_t cm_create_dir_mode(const char *dir_name, uint32 mode) {
    if (make_dir(dir_name, mode) != 0) {
...
status_t cm_create_dir_ex_mode(const char *dir_name, uint32 mode) {
    ...
    for (i = 0; i < dir_len; i++) {
        ...
        if(dir[i] == '/') {
            ...
            if (cm_create_dir_mode(dir,mode) != GS_SUCCESS) {
                return GS_ERROR;
            }
            dir[i] = '/';
        }
...
status_t cm_create_dir_ex(const char *dir_name) {
   return cm_create_dir_ex_mode(dir_name, S_IRWXU); 
}
