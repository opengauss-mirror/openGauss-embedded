...
typedef enum gs_default_value_type {
    DEFAULT_VALUE_TYPE_INVALID = 0,
    DEFAULT_VALUE_TYPE_LITERAL = 1,
    DEFAULT_VALUE_TYPE_FUNC = 2,
} gs_default_value_type_t;

#pragma pack(push, 1)
typedef struct internal_default_value {
    uint8_t type;
    uint32_t size;
    char data[0];
} internal_default_value_t;
#pragma pack(pop)

#define SIZE_K(n) (uint32)((n)*1024)
#define SIZE_M(n) (1024 * SIZE_K(n))
#define SIZE_G(n) (1024 * (uint64)SIZE_M(n))
#define SIZE_TB(n) (1024 * (uint64)SIZE_G(n))
...
#define GS_MIN_CONN_NUM (uint32)1
#define GS_MAX_CONN_NUM (uint32)4096
...
#define GS_MAX_RETENTION_LEN (uint32)16
#define GS_MAX_TS_PART_INTERVAL_LEN (uint32)16
#define GS_TS_PART_NAME_BUFFER_SIZE (uint32)CM_ALIGN4(GS_MAX_NAME_LEN + GS_MAX_TS_PART_INTERVAL_LEN + 1)
...
#define GS_FULL_PATH_BUFFER_SIZE (uint32)256
...
#define GS_SQL_ENGINE_MEMORY_SIZE SIZE_M(1024*2)
...
#define GS_MIN_SYSTEM_DATAFILE_SIZE SIZE_M(1)
#define GS_MIN_SYSAUX_DATAFILE_SIZE SIZE_M(1)
#else
#define GS_MIN_SYSTEM_DATAFILE_SIZE SIZE_M(2) // 128->2
#define GS_MIN_SYSAUX_DATAFILE_SIZE SIZE_M(16) // 128->96->16
#endif
...
#define GS_MAX_ARCH_FILES_SIZE SIZE_TB(32)
...
#define GS_SERIAL_CACHE_COUNT (uint32)1
...
#define GS_SYS_SESSIONS (uint32)11
#define GS_MAX_AUTON_SESSIONS (uint32)256
#define GS_MAX_UNDO_SEGMENTS (uint32)1024
#if defined(INTARK_LITE)
#define GS_MAX_SESSIONS (uint32)100
#define GS_MAX_RMS (uint32)100
#define GS_MAX_AGENTS (uint32)16  // 主要用来控制redo文件的大小
#else
#define GS_MAX_SESSIONS (uint32)1024
#define GS_MAX_RMS (uint32)16320
#define GS_MAX_AGENTS (uint32)16  // 1024->512->128->16
#endif
...
#define GS_EXTEND_RMS (uint32)16
...
#define GS_MAX_STORAGE_MAXSIZE (uint64)SIZE_TB(1)
#define GS_MIN_STORAGE_MAXSIZE (uint64)SIZE_M(1)
#define GS_MAX_STORAGE_INITIAL (uint64)SIZE_TB(1)
...
#if defined(INTARK_LITE)
    #define GS_MIN_CR_POOL_SIZE (int64)SIZE_K(16)    /* 16K */
    #define GS_MIN_TEMP_BUFFER_SIZE (int64)SIZE_M(1) /* 1M */
...
#if defined(INTARK_LITE)
#define GS_MAX_BUF_POOL_NUM (uint32)4
#define GS_MIN_DATA_BUFFER_SIZE (int64)SIZE_M(1) /* 1M */
#elif defined(DCC_LITE)
#define GS_MAX_BUF_POOL_NUM (uint32)4
#define GS_MIN_DATA_BUFFER_SIZE (int64)SIZE_M(10) /* 10M */
#else
#define GS_MAX_BUF_POOL_NUM (uint32)16
#define GS_MIN_DATA_BUFFER_SIZE (int64)SIZE_M(64) /* 64M */
#endif

#define GS_MAX_CR_POOL_COUNT (uint32)256
#define GS_MAX_TEMP_POOL_NUM (uint32)16

#define GS_MAX_TEMP_BUFFER_SIZE \
    (int64)SIZE_TB(4) /* 4T < (GS_VMEM_PAGE_SIZE[128K] + VM_PAGE_CTRL_SIZE[12]) * VM_MAX_CTRLS[32K*5461] */
...
#if defined(INTARK_LITE)
#define GS_XPURPOSE_BUFFER_SIZE SIZE_K(512)
#define GS_LARGE_PAGE_SIZE SIZE_K(256)
#else
...
#if defined(INTARK_LITE)
#define GS_MIN_SQL_PAGES (uint32)16
#define GS_MIN_DICT_PAGES (uint32)320
#define GS_CKPT_GROUP_SIZE (uint32)32
#elif defined(DCC_LITE) 
#define GS_MIN_SQL_PAGES (uint32)64
#define GS_MIN_DICT_PAGES (uint32)320
#define GS_CKPT_GROUP_SIZE (uint32)512
#else
...
#define GS_MAX_SGA_BUF_SIZE SIZE_TB(32)
...
#if defined(INTARK_LITE)
#define GS_VMEM_PAGE_SIZE SIZE_K(64)
#else
...
#define GS_MAX_LOG_FILES (uint32)20
#define GS_MAX_SPACES (uint32)20
#define GS_SPACES_BITMAP_SIZE (uint32)(GS_MAX_SPACES / UINT8_BITS)
#define GS_MAX_DATA_FILES (uint32)20             /* 2^10 - 1 */
...
#define GS_MAX_SPACE_FILES (uint32)20
#if defined(INTARK_LITE)
#define GS_EXTENT_SIZE (uint32)1
#else
...
#define GS_MAX_LOCK_WAIT_TIMEOUT (uint32)1000000    // ms
#define GS_MIN_LOCK_WAIT_TIMEOUT (uint32)1000       // ms
...
#define GS_MAX_ARCH_NUM (uint32)2832 // ((CTRL_MAX_BUF_SIZE / sizeof(arch_ctrl_t)) * (CTRL_MAX_PAGE - db->ctrl.arch_segment))
...
#define GS_JSON_MAX_DYN_BUF_SIZE (uint64)SIZE_TB(32)
...
#define GS_MAX_UINT64_HEX_STR_LEN (uint32)(sizeof(uint64) * 2 + 3)
...
#define GS_RETURN_IF_FALSE(cond) \
    do {                         \
        if (!(cond)) {           \
            return GS_ERROR;     \
        }                        \
...
#define CS_CHECK_NULL_PTR(pointer)                                        \
    do {                                                                  \
        if (SECUREC_UNLIKELY((pointer) == NULL)) {                        \
            GS_THROW_ERROR(ERR_NULL_PTR, (uint64)0, "the value is null"); \
            return GS_ERROR;                                              \
        }                                                                 \
    } while (0)
...
#define DEFAULT_LOG_LEVEL                                                                                    \
    (LOG_RUN_ERR_LEVEL | LOG_RUN_WAR_LEVEL | LOG_RUN_INF_LEVEL | LOG_DEBUG_ERR_LEVEL | LOG_DEBUG_WAR_LEVEL | \
        LOG_DEBUG_INF_LEVEL | LOG_MEC_LEVEL | LOG_TRACE_LEVEL | LOG_PROFILE_LEVEL | LOG_LONGSQL_LEVEL |      \
        LOG_OPER_LEVEL | LOG_ODBC_ERR_LEVEL | LOG_ODBC_WAR_LEVEL | LOG_ODBC_INF_LEVEL)

#define LOG_FATAL_LEVEL 0xFFFFFFFF

#define MAX_LOG_LEVEL                                                                                                  \
    ((LOG_RUN_ERR_LEVEL) | (LOG_RUN_WAR_LEVEL) | (LOG_RUN_INF_LEVEL) | (LOG_DEBUG_ERR_LEVEL) | (LOG_DEBUG_WAR_LEVEL) | \
        (LOG_DEBUG_INF_LEVEL) | (LOG_LONGSQL_LEVEL) | (LOG_OPER_LEVEL))
...
#define BUDDY_INIT_BLOCK_SIZE SIZE_M(1)
...
typedef enum en_altable_action {
    ...
    ALTABLE_MODIFY_TABLE_COMMENT = 42,
    ALTABLE_MODIFY_COLUMN_COMMENT = 43,
} altable_action_t;
