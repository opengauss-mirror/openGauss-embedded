...
double cm_dec4_to_real(const dec4_t *dec)
{
    ...
    if (i >= 5) {
        u64 = (uint64)dec->cells[0] * DEC_POW3_MASK
            ...
            + (uint64)dec->cells[3];
        if(u64 > DEC_POW3_MASK * 10) { // u64 > 10^13
            dval = (double)(int64)u64;
            // 针对高精度做的调整，不改变其他分支的逻辑
            // 避免 u64 太大 , u64 -> dval 就已经出现精度丢失
            dval += (double)dec->cells[4] / (double)DEC4_CELL_MASK;
            if(i > 5) {
                dval = dval * (double)DEC4_CELL_MASK + (double)dec->cells[5]/(double)DEC4_CELL_MASK;
            }
            --i; // note: 需要少除一个等级
        } else {
            // u64 is less than 10^13 , 可以直接转换
            u64 = u64* DEC4_CELL_MASK + (uint64)dec->cells[4];
            dval = (double)(int64)u64;
            if( i > 5 ) {
                dval = dval * DEC4_CELL_MASK + (double)dec->cells[5];
            }
        }
    } else if (i == 4) {
        u64 = (uint64)dec->cells[0] * DEC_POW3_MASK
