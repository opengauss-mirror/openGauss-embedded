...
static inline status_t cm_ptlist_add(ptlist_t *list, pointer_t item)
{
    ...
    if (list->count >= list->capacity) { /* extend the list */
        ...
        if (list->items != NULL) {
            if (list->capacity != 0) {
                errcode =
                    memcpy_sp(new_items, (size_t)buf_size, list->items, (size_t)(list->capacity * sizeof(pointer_t)));
                if (errcode != EOK) {
                    CM_FREE_PTR(new_items);
                    GS_THROW_ERROR(ERR_RESET_MEMORY, "extending list");
                    return GS_ERROR;
...
#define cm_list_for_each(item, head) for ((item) = (head)->next; (item) != (head); (item) = (item)->next)
...
#define cm_list_entry(item, type, member) ((type *)((char *)(item) - (char *)(&((type *)0)->member)))
