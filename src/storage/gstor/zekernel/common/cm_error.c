...
const char *g_error_desc[] = {
    ...
    //SERVER [2800, 2899]
    [ERR_SERVER_STARTED         ] = "Server is already started",
    [ERR_SERVER_NOT_STARTED     ] = "Server is not Started",

    [ERR_COLUMN_DATA_TRUNCATED] = "The column '%s' will be truncated at row %d",
    /*
...
status_t cm_set_sql_error(const char *file, uint32 line, gs_errno_t code, const char *format, va_list args)
{
    ...
    last_file = cm_get_last_filename(file);
    ...
    if (!g_tls_error.is_ignore_log) {
        GS_LOG_DEBUG_ERR("GS-%05d : %s [%s:%u]", code, log_msg, last_file, line);

        if (log_param->log_instance_startup || code == ERR_SYSTEM_CALL) {
            GS_LOG_RUN_ERR("GS-%05d : %s [%s:%u]", code, log_msg, last_file, line);
        }
    }
