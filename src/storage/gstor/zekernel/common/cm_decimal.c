...
int64 cm_truncate_bigint(int64 val, uint32 prec) {
    if (val >= 0) {
 ...
 */
uint32 cm_count_u16digits(uint16 u16) {
    // Binary search
    if (u16 >= 1000u) {
...
uint32 cm_count_u32digits(uint32 u32) {
    // Binary search
    if (u32 >= 100000u) {
        if (u32 >= 10000000u) {
...
double cm_round_real(double val, round_mode_t mode) {
    switch (mode) {
        case ROUND_TRUNC:
            return trunc(val);
 ...
 */
uint32 cm_celltext2uint32(const text_t *cellt) {
    uint32 val = 0;
 ...
 */
status_t cm_dec_4_to_8(dec8_t *d8, const dec4_t *d4, uint32 sz_byte) {
    if (sz_byte == 0) {
        cm_zero_dec8(d8);
        return GS_SUCCESS;
    }
    ...
    if ((uint32)(cm_dec4_stor_sz(d4)) > sz_byte) {
        GS_THROW_ERROR_EX(ERR_ASSERT_ERROR, "cm_dec4_stor_sz(d4)(%u) <= sz_byte(%u)", (uint32)(cm_dec4_stor_sz(d4)),
                          sz_byte);
        return GS_ERROR;
    }
...
status_t cm_dec_8_to_4(dec4_t *d4, const dec8_t *d8) {
...
uint32 cm_dec8_stor_sz(const dec8_t *d8) {
    dec4_t d4;
    (void)cm_dec_8_to_4(&d4, d8);
    return cm_dec4_stor_sz(&d4);
}

status_t cm_adjust_double(double *val, int32 precision, int32 scale) {
...
status_t cm_dec8_add_int64(const dec8_t *dec, int64 i64, dec8_t *result) {
    dec8_t i64_dec;
    cm_int64_to_dec8(i64, &i64_dec);
    return cm_dec8_add(dec, &i64_dec, result);
}

status_t cm_dec8_add_int32(const dec8_t *dec, int32 i32, dec8_t *result) {
    dec8_t i32_dec;
    cm_int64_to_dec8(i32, &i32_dec);
    return cm_dec8_add(dec, &i32_dec, result);
}

status_t cm_int64_sub_dec(int64 i64, const dec8_t *dec, dec8_t *result) {
    dec8_t i64_dec;
    cm_int64_to_dec8(i64, &i64_dec);
    return cm_dec8_subtract(&i64_dec, dec, result);
}

status_t cm_dec8_mul_int64(const dec8_t *dec, int64 i64, dec8_t *result) {
    dec8_t i64_dec;
    cm_int64_to_dec8(i64, &i64_dec);
    return cm_dec8_multiply(dec, &i64_dec, result);
}

status_t cm_dec8_div_int64(const dec8_t *dec, int64 i64, dec8_t *result) {
    dec8_t i64_dec;
    cm_int64_to_dec8(i64, &i64_dec);
    return cm_dec8_divide(dec, &i64_dec, result);
}

status_t cm_int64_div_dec8(int64 i64, const dec8_t *dec, dec8_t *result) {
    dec8_t i64_dec;
    cm_int64_to_dec8(i64, &i64_dec);
...
status_t cm_dec8_add_real(const dec8_t *dec, double real, dec8_t *result) {
    dec8_t real_dec;
...
status_t cm_real_sub_dec8(double real, const dec8_t *dec, dec8_t *result) {
    dec8_t real_dec;
    if (cm_real_to_dec8(real, &real_dec) != GS_SUCCESS) {
        return GS_ERROR;
    }
...
status_t cm_dec8_mul_real(const dec8_t *dec, double real, dec8_t *result) {
    dec8_t real_dec;
    if (cm_real_to_dec8(real, &real_dec) != GS_SUCCESS) {
        return GS_ERROR;
    }
...
status_t cm_dec8_div_real(const dec8_t *dec, double real, dec8_t *result) {
    dec8_t real_dec;
    if (cm_real_to_dec8(real, &real_dec) != GS_SUCCESS) {
        return GS_ERROR;
    }
...
status_t cm_real_div_dec8(double real, const dec8_t *dec, dec8_t *result) {
    dec8_t real_dec;
    if (cm_real_to_dec8(real, &real_dec) != GS_SUCCESS) {
        return GS_ERROR;
    }
