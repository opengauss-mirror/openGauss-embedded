...
static status_t cm_log_create_directory(const char *log_dir)
{
    ...
    //     }
    // }
    return cm_create_dir_ex_mode(log_dir, g_log_param.log_path_permissions);
}
    ...
    const char *module_name, bool32 need_rec_filelog, const char *format, ...)
{
    ...
    last_file = cm_get_last_filename(code_file_name);
...
void cm_write_audit_log(const char *format, ...)
{
    ...
    va_start(args, format);
    buf_text.str = date;
    buf_text.len = (uint32)strlen(date);
    cm_log_fulfil_write_buf(log_file_handle, &buf_text, sizeof(date), GS_TRUE, format, args);
    va_end(args);
}
