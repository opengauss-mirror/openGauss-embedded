...
static inline bool32 knl_session_in_trans(knl_session_t *knl_session)
{
    ...
    return (status != XACT_END || knl_xa_xid_valid(&knl_session->rm->xa_xid) ||
        knl_session->rm->query_scn != GS_INVALID_ID64 || knl_session->rm->svpt_count > 0);
}
...
void knl_free_session(knl_session_t *knl_session)
{
    ...
    knl_destroy_session(&cc_instance->kernel, id);
    CM_FREE_PTR(knl_session->stack);
    CM_FREE_PTR(knl_session);
    cc_instance->kernel.sessions[id] = NULL;
}

void knl_free_sys_sessions(instance_t *cc_instance)
{
    for (uint32 i = 0; i < GS_SYS_SESSIONS; i++) {
        knl_session_t *session = cc_instance->kernel.sessions[i];
        if (session != NULL) {
...
static void knl_init_new_session(instance_t *cc_instance, knl_session_t *knl_session, cm_stack_t *stack, char *plog_buf)
{
    knl_instance_t *kernel = &cc_instance->kernel;

    cm_spin_lock(&cc_instance->lock, NULL);
    ...
    cm_spin_unlock(&cc_instance->lock);
...
static status_t alloc_session_memory(knl_instance_t *kernel, knl_session_t **knl_session)
{
    ...
    GS_LOG_RUN_INF("memory of single temp_table:%d\n", sizeof(mtrl_segment_t)+sizeof(knl_temp_cache_t)+sizeof(void *));
...
static inline void calc_private_buff_len(instance_t *cc_instance, uint32 *stack_size, uint32 *plog_size, uint32 *buf_size)
{
    knl_attr_t *attr = &cc_instance->kernel.attr;

    *buf_size = sizeof(cm_stack_t);

    *stack_size = cc_instance->attr.stack_size;
    *buf_size += *stack_size;
...
static status_t alloc_private_buff(instance_t *cc_instance, cm_stack_t **stack, char **plog_buf, char **update_buf)
{
    char *stack_buf = NULL;
    uint32 stack_size, plog_size, buf_size;

    calc_private_buff_len(cc_instance, &stack_size, &plog_size, &buf_size);
    if (buf_size == 0) {
...
status_t knl_alloc_session(instance_t *cc_instance, knl_session_t **knl_session)
{
    uint16 rmid;
    char *plog_buf = NULL;
    ...
    if (knl_alloc_rm(cc_instance, &rmid) != GS_SUCCESS) {
        CM_FREE_PTR(*knl_session);
        return GS_ERROR;
    }

    if (alloc_private_buff(cc_instance, &stack, &plog_buf, &update_buf) != GS_SUCCESS) {
        knl_release_rm(cc_instance, rmid);
        CM_FREE_PTR(*knl_session);
        return GS_ERROR;
    }
    ...
    (*knl_session)->rm = cc_instance->rm_pool.rms[rmid];
    knl_init_new_session(cc_instance, *knl_session, stack, plog_buf);
    knl_set_update_info(kernel, *knl_session, update_buf);
    return GS_SUCCESS;
}

status_t knl_alloc_sys_sessions(instance_t *cc_instance)
{
    knl_session_t *knl_session = NULL;
    for (uint32 i = 0; i < GS_SYS_SESSIONS; i++) {
        GS_RETURN_IFERR(knl_alloc_session(cc_instance, &knl_session));
    }
#ifdef _LIBAIO
    // SESSION_ID_AIO = 11
    GS_RETURN_IFERR(knl_alloc_session(cc_instance, &knl_session));
...
status_t knl_alloc_cursor(instance_t *cc_instance, knl_cursor_t **cursor)
{
    knl_instance_t *kernel = &cc_instance->kernel;
    uint32 ext_size = (uint32)(kernel->attr.max_column_count * sizeof(uint16));
    // kernel->attr.cursor_size:2 pages(row + page_buf) + 2 ext_size(offsets + lens) + 1 page(data)
    uint32 mem_size = kernel->attr.cursor_size + 3 * ext_size + kernel->attr.page_size;
    ...
    *cursor = (knl_cursor_t *)malloc(mem_size);
    ...
    KNL_INIT_CURSOR(*cursor); //the order of KNL_INIT_CURSOR and update_info.data can not be changed !!!!!!!

    ext_buf += ext_size;
    (*cursor)->update_info.data = (uint16 *)ext_buf;
    return GS_SUCCESS;
}
