...
void rm_pool_deinit(instance_t *cc_instance, rm_pool_t *rm_pool)
{
    for (uint32 i = 0; i < cc_instance->rm_pool.page_count; i++) {
        CM_FREE_PTR(cc_instance->rm_pool.pages[i]);
    }
    cc_instance->rm_pool.page_count = 0;
}
...
static status_t rm_pool_extend(instance_t *cc_instance, rm_pool_t *pool)
{
    char *buf = NULL;
    size_t alloc_size;
    errno_t ret;

    if (pool->capacity >= cc_instance->kernel.attr.max_rms) {
        GS_THROW_ERROR(ERR_TOO_MANY_RM_OBJECTS, cc_instance->kernel.attr.max_rms);
...
static status_t rm_alloc(instance_t *cc_instance, rm_pool_t *rm_pool, uint16 *rmid)
{
    knl_rm_t *rm = NULL;

    if (rm_pool->free_list.count == 0 && rm_pool->hwm == rm_pool->capacity) {
        if (rm_pool_extend(cc_instance, rm_pool) != GS_SUCCESS) {
            return GS_ERROR;
        }
    }
    ...
    if (rm_pool->free_list.count == 0) {
        *rmid = rm_pool->hwm;
        ...
        cc_instance->kernel.rms[rm_pool->hwm] = rm;
        rm_pool->hwm++;
        cc_instance->kernel.rm_count++;
    } else {
...
status_t knl_alloc_rm(instance_t *cc_instance, uint16 *rmid)
{
    rm_pool_t *rm_pool = &cc_instance->rm_pool;
    knl_rm_t *rm = NULL;

    cm_spin_lock(&rm_pool->lock, NULL);
    if (rm_alloc(cc_instance, rm_pool, rmid) != GS_SUCCESS) {
...
void knl_release_rm(instance_t *cc_instance, uint16 rmid)
{
    rm_pool_t *rm_pool = &cc_instance->rm_pool;
...
uint16 knl_get_xa_xid(instance_t *cc_instance, knl_xa_xid_t *xa_xid)
{
    rm_pool_t *rm_pool = &cc_instance->rm_pool;
...
bool32 knl_add_xa_xid(instance_t *cc_instance, knl_xa_xid_t *xa_xid, uint16 rmid, uint8 status)
{
    rm_pool_t *rm_pool = &cc_instance->rm_pool;
    rm_bucket_t *bucket = NULL;
    uint16 temp;
...
void knl_delete_xa_xid(instance_t *cc_instance, knl_xa_xid_t *xa_xid)
{
    rm_pool_t *rm_pool = &cc_instance->rm_pool;
    rm_bucket_t *bucket = NULL;
    uint16 rmid;
...
static inline void assign_trans_to_bg_rollback(instance_t *cc_instance, knl_rm_t *rm)
{
    undo_t *undo = &cc_instance->kernel.undo_ctx.undos[rm->tx_id.seg_id];
    cc_instance->kernel.tran_ctx.rollback_num = cc_instance->kernel.attr.tx_rollback_proc_num;
    undo->items[rm->tx_id.item_id].rmid = cc_instance->kernel.sessions[SESSION_ID_ROLLBACK]->rmid;
}
    ...
    for (uint16 i = 0; i < rm_pool->hwm; i++) {
        ...
        if (rm->xa_status == XA_PENDING) {
            if (force) {
                ...
                assign_trans_to_bg_rollback(cc_instance, rm);
                knl_tx_reset_rm(rm);
        ...
        if (release_rm) {
            knl_delete_xa_xid(cc_instance, &rm->xa_xid);
            cm_spin_lock(&rm_pool->lock, NULL);
...
static bool32 knl_attach_rm(knl_session_t *knl_session, knl_xa_xid_t *xa_xid, uint8 exp_status, uint8 status,
    bool8 release)
{
    instance_t *cc_instance = knl_session->kernel->server;
    rm_pool_t *rm_pool = &cc_instance->rm_pool;
    ...
    rmid = knl_get_xa_xid(cc_instance, xa_xid);
    if (rmid == GS_INVALID_ID16) {
        return GS_FALSE;
    }
