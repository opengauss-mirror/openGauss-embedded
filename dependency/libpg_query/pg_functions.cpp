...
void pg_parser_init() {
  ...
  if(pg_parser_state.malloc_ptrs == nullptr) {
...
void pg_parser_reset() {
	pg_parser_state.pg_err_code = PGUNDEFINED;
	pg_parser_state.pg_err_msg[0] = '\0';
	pg_parser_state.pg_err_pos = 0;
	pg_parser_state.malloc_pos = 0;
	pg_parser_state.malloc_ptr_idx = 1; // pg_parser_state.malloc_ptrs[pg_parser_state.malloc_ptr_idx - 1]
}
