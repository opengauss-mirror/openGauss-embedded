 ...
 *
 *****************************************************************************/
CreateStmt:	CREATE_P OptTemp TABLE qualified_name '(' OptTableElementList ')' OptPartitionSpec
			OptTimescale OptInterval OptRetention OptAutopart OptCrosspart OptComment 
		...
		| CREATE_P OptTemp TABLE IF_P NOT EXISTS qualified_name '('
			OptTableElementList ')' OptPartitionSpec OptTimescale 
            OptInterval OptRetention OptAutopart OptCrosspart OptComment 
            OptWith OnCommitOption
				{
					PGCreateStmt *n = makeNode(PGCreateStmt);
					$7->relpersistence = $2;
		...
		| CREATE_P OR REPLACE OptTemp TABLE qualified_name '('
			OptTableElementList ')' OptPartitionSpec OptWith
			OnCommitOption
				{
					PGCreateStmt *n = makeNode(PGCreateStmt);
					$6->relpersistence = $4;
...
ColConstraintElem:
			...
			| COMMENT b_expr
				{
					PGConstraint *n = makeNode(PGConstraint);
					n->contype = PG_CONSTR_EXTENSION_COMMENT;
					n->location = @1;
					n->raw_expr = $2;
					n->cooked_expr = NULL;
					$$ = (PGNode *)n;
			...
			| AUTOINCREMENT
				{
					PGConstraint *n = makeNode(PGConstraint);
					n->contype = PG_CONSTR_EXTENSION_AUTOINCREMENT;
					n->location = @1;
					$$ = (PGNode *)n;
				}
		;
...
PGPartitionBoundSpec:
			...
			FOR VALUES WITH '(' hash_partbound ')'
				{
					ListCell   *lc;
					PGPartitionBoundSpec *n = makeNode(PGPartitionBoundSpec);
...
OptPartitionSpec: PGPartitionSpec	{ $$ = $1; }
			| /*EMPTY*/			{ $$ = NULL; }
		;

PGPartitionSpec: PARTITION BY ColId '(' part_params ')'
				{
					PGPartitionSpec *n = makeNode(PGPartitionSpec);
...
part_elem: ColId opt_collate opt_qualified_name
				{
					PGPartitionElem *n = makeNode(PGPartitionElem);
