...
AlterTableStmt:
		...
		|	ALTER TABLE IF_P EXISTS relation_expr partition_cmds
				{
					PGAlterTableStmt *n = makeNode(PGAlterTableStmt);
...
partition_cmds:
            partition_cmd							{ $$ = list_make1($1); }
			| partition_cmds ',' partition_cmd	    { $$ = lappend($1, $3); }

partition_cmd:
            /* ALTER TABLE <name> ADD PARTITION <partition name> */
			ADD_P PARTITION qualified_name
				{
					PGAlterTableCmd *n = makeNode(PGAlterTableCmd);
                    PGPartitionCmd *cmd = makeNode(PGPartitionCmd);

					n->subtype = PG_AT_AttachPartition;
					n->missing_ok = false;
                    cmd->name = $3;
			...
			| DROP PARTITION qualified_name
				{
					...
					n->subtype = PG_AT_DetachPartition;
					n->missing_ok = false;
                    cmd->name = $3;
                    cmd->concurrent = false;
					n->def = (PGNode *) cmd;
