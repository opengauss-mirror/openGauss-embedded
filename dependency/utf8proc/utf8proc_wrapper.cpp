...
size_t Utf8Proc::RenderWidth(const char *s, size_t len, size_t pos) {
    int sz;
    auto codepoint = intarkdb::utf8proc_codepoint(s + pos, sz);
    auto properties = intarkdb::utf8proc_get_property(codepoint);
...
size_t Utf8Proc::RenderWidth(const std::string &str) {
	...
	while(pos < str.size()) {
		int sz;
		auto codepoint = intarkdb::utf8proc_codepoint(str.c_str() + pos, sz);
		auto properties = intarkdb::utf8proc_get_property(codepoint);
